...
def build_phase1_fact_registry(phase1_result):
    ...
    일치시킨다. 값 추출 로직·라벨 목록·TF 섹션 분리 로직은 변경하지 않는다."""
    registry = {}
...
FRACTAL_TF_BLOCK_PATTERN = re.compile(
    r"정식 모드 보조 지표\s*—\s*TF\s*:\s*(?P<tf>\S+).*?"
...
def verify_fractal_supplement_math(text):
    ...
    warnings = []
    for m in FRACTAL_TF_BLOCK_PATTERN.finditer(text or ""):
        tf = m.group("tf")
        zone_raw = m.group("zone").strip()
        pos = _parse_decimal(m.group("pos"))
        kz = _parse_decimal(m.group("kz"))
        kp = _parse_decimal(m.group("kp"))
        orig = _parse_decimal(m.group("orig"))
        corr = _parse_decimal(m.group("corr"))
        buffer_flag = m.group("buffer")
        if None in (pos, kz, kp, orig, corr):
