...
def _ema_series(values, period):
    """지수이동평균 전체 시계열. values 길이가 period 미만이면 전부 None."""
...
def _sma_series(values, period):
    """단순이동평균 전체 시계열. 앞쪽 (period-1)개는 계산 불가로 None."""
    n = len(values)
    if n < period:
        return [None] * n
    out = [None] * (period - 1)
...
def tier1_volume_dry_price_high(daily_bars, lookback):
    """#7 거래량 하위권 + 가격 구간 상단 유지: 물량 소화가 끝나가는데 가격은 안 밀리는 상태.
    [보충, 2026-09-04] Phase A 원 설계 목록(18개)에 있었으나 구현이 누락됐던 항목 —
    사용자 지적으로 확인, 이번에 추가."""
    completed = _completed_bars(daily_bars)
    window = completed[-lookback:]
    if len(window) < 10:
    ...
    today_vol = vols[-1]
    lower_count = sum(1 for v in vols if v < today_vol)
    vol_pctile = lower_count / len(vols)  # 낮을수록 거래량 하위
    hi = max(b[2] for b in window)
    lo = min(b[3] for b in window)
    today_close = window[-1][4]
    price_position = (today_close - lo) / (hi - lo) if hi > lo else 0.5
    is_dry_high = (vol_pctile <= 0.3) and (price_position >= 0.7)
    is_partial = (vol_pctile <= 0.5) and (price_position >= 0.5)
    return {
        "status": "ok",
        "volume_percentile": round(vol_pctile, 4),
        "price_position_in_range": round(price_position, 4),
        "normalized": 1.0 if is_dry_high else (0.5 if is_partial else 0.0),
    }
...
def tier1_macd(daily_bars, lookback, fast=12, slow=26, signal=9):
    """MACD — 표준값 12/26/9(업계 정의, 임의 발명 아님). 히스토그램 부호전환·확대축소 산출.
    [보충, 2026-09-04] 사용자 요청 '보조지표' 3종 중 하나."""
    completed = _completed_bars(daily_bars)
    if len(completed) < slow + signal + 5:
        return {"status": "insufficient_data"}
    closes = [b[4] for b in completed]
    ...
    if len(histogram) < 3:
        return {"status": "insufficient_data"}
    today_hist, prev_hist = histogram[-1], histogram[-2]
    cross_up = prev_hist <= 0 < today_hist
    cross_down = prev_hist >= 0 > today_hist
    expanding = abs(today_hist) > abs(prev_hist)
    return {
        "status": "ok",
        "macd": round(macd_line[-1], 8),
        "signal": round(signal_line[-1], 8),
        "histogram": round(today_hist, 8),
        "histogram_expanding": expanding,
        "cross_up": cross_up,
        "cross_down": cross_down,
        "normalized": 1.0 if (cross_up or (today_hist > 0 and expanding)) else (0.5 if today_hist > 0 else 0.0),
    }


def tier1_moving_average_structure(daily_bars, lookback, short_p=5, mid_p=20, long_p=60):
    """이동평균 구조 3종 — 표준 기간(5/20/60, 업계 관행) 사용.
    1) 골든/데드크로스: 단기(5)가 중기(20) 위/아래로 뚫은 지 며칠째인가
    2) 밀집&확산: 5/20/60 세 선의 간격이 자기이력상 좁은 편인가(낮을수록 밀집)
    3) 지지반등: spring_reclaim(#17)과 동일 구조를 이평선(20일선) 기준으로 재적용
       — 판단로직 재발명 아님, 기준선만 교체."""
    completed = _completed_bars(daily_bars)
    if len(completed) < long_p + 10:
        return {"status": "insufficient_data"}
    closes = [b[4] for b in completed]
    sma_short = _sma_series(closes, short_p)
    sma_mid = _sma_series(closes, mid_p)
    sma_long = _sma_series(closes, long_p)
    if sma_short[-1] is None or sma_mid[-1] is None or sma_long[-1] is None:
        return {"status": "insufficient_data"}

    cross_state = "above" if sma_short[-1] > sma_mid[-1] else "below"
    days_since_cross = 0
    for i in range(len(closes) - 1, -1, -1):
        if sma_short[i] is None or sma_mid[i] is None:
            break
        state_i = "above" if sma_short[i] > sma_mid[i] else "below"
        if state_i != cross_state:
            break
        days_since_cross += 1
    golden_cross = cross_state == "above"

    spreads = []
    for i in range(long_p - 1, len(closes)):
        if sma_short[i] is None or sma_mid[i] is None or sma_long[i] is None or closes[i] <= 0:
            continue
        vals = [sma_short[i], sma_mid[i], sma_long[i]]
        spreads.append((max(vals) - min(vals)) / closes[i] * 100.0)
    if len(spreads) < 10:
        return {"status": "insufficient_data"}
    today_spread = spreads[-1]
    lower_count = sum(1 for s in spreads if s < today_spread)
    spread_pctile = lower_count / len(spreads)

    today = daily_bars[-1]
    today_open, today_low, today_close = today[1], today[3], today[4]
    ma_support = sma_mid[-1]
    strict_bounce = (today_low < ma_support) and (today_close >= ma_support)
    loose_bounce = (today_close <= ma_support * 1.03) and (today_close > today_open)

    return {
        "status": "ok",
        "golden_cross": golden_cross,
        "days_since_cross": days_since_cross,
        "ma_spread_pctile": round(spread_pctile, 4),
        "ma_bounce_strict": strict_bounce,
        "ma_bounce_loose": loose_bounce,
        "normalized": round(
            (1.0 if golden_cross else 0.0) * 0.4
            + (1 - spread_pctile) * 0.3
            + (1.0 if strict_bounce else (0.5 if loose_bounce else 0.0)) * 0.3,
            4,
        ),
    }


def tier1_ma20_cross_bullish(daily_bars, lookback, mid_p=20):
    """#21 20일선 돌파 양봉 [판정61번 신설 — Cowork 재검토]. 사용자가 이 시스템과 별개로
    기억하고 있던 지표(handoff 문서 기록, 코드 미구현 상태였음): 전일 종가가 20일선 아래,
    당일 종가가 20일선 위, 당일 봉이 양봉(종가>시가) — 셋 다 성립해야 신호.
    [tier1_moving_average_structure의 ma_bounce와 구분] ma_bounce는 "당일 저가"가 20일선
    아래로 뚫렸다가 종가로 복귀하는 조건(이탈 후 복귀)이고, 이 지표는 저가 이탈 여부를
    보지 않고 순수하게 "전일 종가 vs 당일 종가"의 종가 크로스오버만 본다 — 서로 다른
    신호이며 대체 관계가 아님(중복 아님).
    [MA 기준일 — SSOT, 기존 관례 재사용] 20일선 값은 ma_bounce와 동일하게 "당일을
    제외한(_completed_bars) 최근 mid_p봉" 평균을 쓴다(당일 자신의 종가를 그 종가와
    비교할 기준선 계산에 섞지 않기 위함 — 판정59번에서 이미 확인된, 당일봉을 이전이력창에
    섞으면 안 된다는 원칙과 동일 근거). 새 MA 계산 관례를 만드는 것이 아니라 기존 관례를
    그대로 재사용.
    [Phase C 원칙 준수] 이 함수는 통과/탈락을 만들지 않는다 — 관측(참고표시)용 신호만
    산출하며, S_scout 등 실제 스코어링에는 반영하지 않는다(임계값·가중치 실측 없이
    편입하지 않는다는 원칙 — 데이터가 쌓여 사후사건연구로 유의성이 확인된 뒤 별도 판단)."""
    completed = _completed_bars(daily_bars)
    if len(completed) < mid_p + 1:
        return {"status": "insufficient_data"}
    closes = [b[4] for b in completed]
    sma_mid = _sma_series(closes, mid_p)
    if sma_mid[-1] is None:
        return {"status": "insufficient_data"}
    ma20_level = sma_mid[-1]
    prev_close = closes[-1]
    today = daily_bars[-1]
    today_open, today_close = today[1], today[4]
    crossed_up = (prev_close < ma20_level) and (today_close > ma20_level)
    bullish_candle = today_close > today_open
    signal = crossed_up and bullish_candle
    return {
        "status": "ok",
        ...
        "signal": signal,
        "normalized": 1.0 if signal else 0.0,
    }


def tier1_bollinger(daily_bars, lookback, period=20, num_std=2):
    """볼린저밴드 — 표준값 20/2(업계 정의). 밴드폭 자기이력 백분위(표준편차 기준 —
    range_pctile_self_history의 고저폭 기준과는 다른 계산식, 같은 지표 중복 아님) +
    상단밴드워킹(연속 상단근접) 산출."""
    completed = _completed_bars(daily_bars)
    if len(completed) < period + 20:
        return {"status": "insufficient_data"}
    closes = [b[4] for b in completed]
    ...
    def band_at(i):
        ...
        width_pct = (upper - lower) / mean * 100.0 if mean > 0 else 0.0
    ...
    lower_count = sum(1 for w in widths if w < today_width)
    width_pctile = lower_count / len(widths)

    walking_streak = 0
    for i in range(len(closes) - 1, period - 2, -1):
        u, _, _ = band_at(i)
        if u > 0 and closes[i] >= u * 0.98:
            walking_streak += 1
        else:
            break
    ...
    return {
        "status": "ok",
        "band_width_pctile": round(width_pctile, 4),  # 낮을수록 역사적으로 좁음(스퀴즈)
        "upper_band_walking_streak": walking_streak,
        "normalized": round((1 - width_pctile) * 0.5 + min(walking_streak / 5.0, 1.0) * 0.5, 4),
    }
...
def compute_tier1_observation(
    ...
    obs = {
        ...
        "volume_dry_price_high": tier1_volume_dry_price_high(daily_bars, lookback),
        ...
        "macd": tier1_macd(daily_bars, lookback),
        "moving_average_structure": tier1_moving_average_structure(daily_bars, lookback),
        "bollinger": tier1_bollinger(daily_bars, lookback),
        "ma20_cross_bullish": tier1_ma20_cross_bullish(daily_bars, lookback),  # [판정61번 신설]
    }
    n_total = len(obs)  # coverage_summary 추가 전 시점의 항목 수(판정61번 이후 17개 지표)
