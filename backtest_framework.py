...
# ============================================================
FRAMEWORK_VERSION = "experiment-v2-observability-dedup-20260816"
...
def run_backtest_for_exchange_streaming(ex_id, quote, start_date, end_date, csv_writer,
    ...
    """원본 행은 스트리밍 저장하고, 모든 조합의 실행·None·통과·중복제거 통계를 누적한다."""
    ...
    for sym in symbols:
        try:
            daily = fetch_ohlcv_range(ex, sym, "1d", fetch_since_ms) if fetch_since_ms is not None else ex.fetch_ohlcv(sym, timeframe="1d", limit=200)
        except Exception:
            continue
...
def execute_and_report(chat_id, start_date=None, end_date=None):
    ...
    try:
        with open(tmp_path, "w", newline="", encoding="utf-8") as f:
            ...
            for ex_id, cfg in SUPPORTED_EXCHANGES.items():
                try:
                    ...
                    total_rows += n
                    send_telegram_message(chat_id, f"✓ {ex_id} 완료 (원본 진단행 {n}건)")
                except Exception as e:
        ...
        send_telegram_document(
            chat_id, f"candleview_backtest_summary_{run_id}.csv", summary_bytes,
            caption="실험 요약: 전체·거래소별 평가/None/진단성립/통과/중복제거 통계"
        )

        with open(tmp_path, "rb") as f:
            send_telegram_document(
                chat_id, f"candleview_backtest_raw_{run_id}.csv", f.read(),
                caption="원본 진단행: 개별 이벤트 분석용 (None 행은 summary CSV에서 집계)"
            )
        send_telegram_message(
            chat_id,
            "✅ 실험형 백테스트 완료. 상수 반영 전에는 탐색·검증·사후검증 기간을 분리해 summary CSV를 비교하세요."
        )
    finally:
