...
[SSOT 값 출처 — CandleView_API.txt 대조 완료, 2026-09 Cowork 재검토 시 라인번호 재정정]
- VOL_AVG_LOOKBACK=20봉(CandleView_API.txt L1099), VOL_ABSORPTION_MIN=1.5배(L1082)/
  BODY_MAX=30.0%(L1083)
- VOL_CLIMAX_MIN=3.0배(L1084), RSI 극단 30/70(L1504)
- COMPRESSION_RANGE_MAX=25.0%(판정55/58번 정정값 반영, 2026-09 Cowork 재검토 시 5.0→25.0
  동기화), FC_MIN_BARS_STANDARD=40(main.py L4104)
- RSI: main.py L605 calculate_rma_rsi 그대로 복제(14기간 Wilder RMA, 판정42 NaN→50.0 처리 포함)
- 노디맨드/노서플라이: "직전 2봉 각각 대비 거래량감소 + 몸통비율 30%이하"(CandleView_API.txt L1501)
- [주의] 위 라인번호는 이 정정 시점 기준 스냅샷이다. main.py·CandleView_API.txt는 계속
  커지는 활성 파일이라 향후 편집으로 다시 어긋날 수 있다 — 판단 근거는 항상 값(SSOT 원문
  대조) 우선이며, 라인번호는 참고용 보조정보로만 취급할 것.
...
VOL_AVG_LOOKBACK = 20
# [2026-09 Cowork 재검토 정정] 판정55/58번에서 main.py의 실제 COMPRESSION_RANGE_MAX가
# 25.0으로 확정됐으나 이 파일은 main.py를 import하지 못해(모듈 상단 설명 참조) 수동
# 동기화가 필요한 값이었음 — 여태 5.0으로 방치돼 있던 것을 정정. 다만 STATE1_GRID의
# 모든 항목이 comp_max를 명시적으로 지정하므로 rule_state1()의 params.get("comp_max",
# COMPRESSION_RANGE_MAX) 폴백은 현재도 도달하지 않는다(즉시 동작 변화 없음, 재발 방지용).
COMPRESSION_RANGE_MAX = 25.0
...
# 시세분출 개념이 적용되지 않는 스테이블코인 제외 — main.py FC_STABLECOIN_BASES 원문 그대로 복제
# (2026-09 재정정 시점 기준 L4213 — 활성 파일이라 향후 다시 이동할 수 있음, 값 우선 대조할 것).
...
# ============================================================
FRAMEWORK_VERSION = "experiment-v5-safe-timing-quality-scope-20260819"
EXPERIMENT_NAME = "state1_compression_then_condition"
# 라이브 FindCoin 전체가 아닌 결정론적 캔들·거래량 규칙 proxy다.
FRAMEWORK_SCOPE = "deterministic_rule_proxy"
# 시점별 상장·상폐 스냅샷이 제공되기 전에는 2026 고정 유니버스의 survivor proxy만 허용한다.
UNIVERSE_MODE = "survivor_proxy"
# 수수료·스프레드·슬리피지 입력 전에는 순수익률을 만들지 않는다.
RETURN_BASIS = "gross_return_costs_not_modeled"
EXECUTION_ASSUMPTION = "signal_on_completed_bar_enter_next_bar_open"
...
def validate_backtest_ohlcv(daily, timeframe="1d"):
    """과거 백테스트용 STAGE 0 무결성 게이트.
    최신성은 과거 표본에 적용하지 않고, 시간순·중복·일간 결손·OHLCV 논리만 검사한다.
    """
    reasons = []
    if not daily:
        return {"ok": False, "reasons": ["빈 OHLCV 배열"]}
    expected_ms = {"1d": 86_400_000}.get(timeframe)
    previous_ts = None
    for row in daily:
        if not isinstance(row, (list, tuple)) or len(row) < 6:
            reasons.append("OHLCV 행 형식 오류")
            break
        ts, o, h, l, c, v = row[:6]
        values = (ts, o, h, l, c, v)
        if not all(np.isfinite(float(x)) for x in values):
            reasons.append("비유한 OHLCV 값")
            break
        if previous_ts is not None:
            if ts <= previous_ts:
                reasons.append("timestamp 중복 또는 역행")
                break
            if expected_ms is not None and ts - previous_ts != expected_ms:
                reasons.append("기대 일간 간격 결손 또는 불일치")
                break
        if h < max(o, c) or l > min(o, c) or h < l:
            reasons.append("OHLC 논리 오류")
            break
        if v < 0:
            reasons.append("음수 거래량")
            break
        previous_ts = ts
    return {"ok": not reasons, "reasons": reasons}


def _add_data_quality_error(diag, symbol, reasons):
    diag["data_quality_reject_count"] += 1
    if len(diag["data_quality_examples"]) < COLLECTION_MAX_ERROR_EXAMPLES:
        diag["data_quality_examples"].append(f"{symbol} | {'; '.join(reasons)[:180]}")
...
CSV_FIELDNAMES = [
    "framework_version", "experiment_name", "framework_scope", "universe_mode", "return_basis", "execution_assumption", "experiment_stage", "signal_cooldown_bars",
...
SUMMARY_FIELDNAMES = [
    "framework_version", "experiment_name", "framework_scope", "universe_mode", "return_basis", "execution_assumption", "period_start", "period_end", "scope", "exchange",
...
COLLECTION_FIELDNAMES = [
    "framework_version", "experiment_name", "framework_scope", "universe_mode", "return_basis", "execution_assumption", "period_start", "period_end", "exchange",
    "configured_symbols", "fetch_attempted", "fetch_success", "api_error_count",
    "insufficient_bars_count", "data_quality_reject_count", "data_quality_examples", "usable_symbols", "evaluation_events", "raw_rows_written",
...
def _collection_row(diag, start_date, end_date):
    return {
        ...
        "data_quality_reject_count": diag["data_quality_reject_count"],
        "data_quality_examples": " || ".join(diag["data_quality_examples"]),
...
def build_backtest_bundle(bundle_path, raw_path, collection_bytes, summary_bytes, period_label):
    ...
    readme_text = (
        ...
        f"framework_scope: {FRAMEWORK_SCOPE} (라이브 FindCoin 전체 성과 검증 아님)\n"
        f"universe_mode: {UNIVERSE_MODE} (시점별 상장·상폐 유니버스 미반영)\n"
        f"return_basis: {RETURN_BASIS} (수수료·스프레드·슬리피지 미포함)\n"
        f"execution_assumption: {EXECUTION_ASSUMPTION}\n"
...
def summarize_collection_diagnostics(collection_diags):
    ...
    for ex_id in SUPPORTED_EXCHANGES:
        ...
        lines.append(
            ...
            f"품질제외{diag['data_quality_reject_count']} 평가{diag['evaluation_events']} 원본행{diag['raw_rows_written']}"
        )
...
def run_backtest_for_exchange_streaming(ex_id, quote, start_date, end_date, csv_writer,
    ...
    for sym in symbols:
        ...
        if len(daily) < min_bars_needed:
            ...
            continue
        quality = validate_backtest_ohlcv(daily, "1d")
        if not quality["ok"]:
            _add_data_quality_error(collection_diag, sym, quality["reasons"])
            continue
        ...
        for t in range(start_idx, len(daily) - FORWARD_WINDOW - 1):
            bar_ts = daily[t][0]
            ...
            window = daily[:t + 1]
            entry_price = daily[t + 1][1]  # t번 완성 신호 뒤 다음 봉 시가에만 진입 가능
            future_price = daily[t + 1 + FORWARD_WINDOW][4]
...
def summarize_from_stats(combo_stats, start_date, end_date):
    ...
    lines = [
        "<b>CandleView 결정론적 규칙 실험 요약</b>",
        f"버전: {FRAMEWORK_VERSION} | 범위: {FRAMEWORK_SCOPE}",
        f"기간: {period_label} | 유니버스: {UNIVERSE_MODE} | 수익률: gross(비용 미반영)",
        f"체결가정: 다음 봉 시가 진입 | 워밍업: {WARMUP_BARS}봉 | 중복제거: 동일 코인·조합 {SIGNAL_COOLDOWN_BARS}봉",
...
def execute_and_report(chat_id, start_date=None, end_date=None):
    ...
    send_telegram_message(
        chat_id,
        f"🔬 CandleView 결정론적 규칙 실험 시작 (라이브 FindCoin 성과 검증 아님)\n"
        f"기간: {period_label} | 유니버스: {UNIVERSE_MODE} | gross 수익률(비용 미반영)\n"
        f"State1: 압축률 단독(A) + 조건 단독(B), 다음 봉 시가 진입, 워밍업 {WARMUP_BARS}봉, 중복제거 {SIGNAL_COOLDOWN_BARS}봉\n"
    ...
    try:
        with open(tmp_path, "w", newline="", encoding="utf-8") as f:
            ...
            for ex_id, cfg in SUPPORTED_EXCHANGES.items():
                ...
                send_telegram_message(
                    ...
                    f"봉수미달{collection_diag['insufficient_bars_count']} 품질제외{collection_diag['data_quality_reject_count']} 원본행{n}"
                )
        ...
        send_telegram_message(
            chat_id,
            "✅ 결정론적 규칙 실험 완료. 결과는 survivor proxy의 gross 수익률이며 라이브 FindCoin 성과·순수익률이 아닙니다. 상수 반영 전에는 탐색·검증·사후검증 기간을 분리해 summary.csv를 비교하세요."
        )
    finally:
