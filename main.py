...
def _fc_notion_diagnose(chat_id):
    """[2026-09-04 신설 — 실사용 결함 대응] 사용자가 페이지 URL에서 직접 뽑은 ID로
    NOTION_FC_EVENT_DATA_SOURCE_ID를 설정했으나 404(object_not_found)가 발생함을 실제
    Render 로그로 확인. 원인: Notion 신형 멀티 데이터소스 구조에서 URL의 페이지/블록 ID와
    API가 요구하는 data_source_id가 다를 수 있음. 이 함수는 integration(candleview-bot)이
    실제로 접근 가능한 데이터베이스 전체를 조회해 정확한 data_source_id를 사용자에게
    텔레그램으로 직접 보여준다 — 브라우저 개발자도구 없이 해결 가능하게 하기 위함."""
    if not NOTION_TOKEN:
    ...
    if not results:
        send_telegram_message(
            chat_id,
            "연결된 데이터베이스가 하나도 없습니다.\n"
            "노션 표 페이지에서 공유(Share) → candleview-bot 연결이 실제로 됐는지 다시 확인해 주세요."
        )
        return
    lines = [f"🔎 candleview-bot이 접근 가능한 데이터베이스 {len(results)}개\n"]
    for db in results:
        title_arr = db.get("title") or []
        title = "".join(t.get("plain_text", "") for t in title_arr) or "(제목없음)"
        db_id = db.get("id", "?")
        ...
        if data_sources:
            for ds in data_sources:
                lines.append(f"  ✅ data_source_id: {ds.get('id')} ({ds.get('name','')})")
        else:
            lines.append("  (data_sources 없음 — database_id를 그대로 써보세요)")
    lines.append("\n\n환경변수엔 '✅ data_source_id' 값을 넣어야 합니다(database_id 아님).")
    full_text = "\n".join(lines)
