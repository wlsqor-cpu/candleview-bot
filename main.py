...
STATIC_APPROVED_MODELS = [
    {"model_id": "gemini-3.6-flash", "selection_source": "정적 승인 roster", "rank": 0},
    {"model_id": "gemini-3.5-flash-lite", "selection_source": "정적 승인 roster", "rank": 1},
]
    ...
    with MODEL_ROSTER_LOCK:
        ...
        try:
            ...
            for candidate in discovered:
                ...
                elif candidate_metrics.get("hard_gate_pass") and model_id not in approved_ids:
                    # 1차 우위가 증명되지 않은 통과 후보는 fallback으로만 등록한다.
                    approved.append({"model_id": model_id, "selection_source": "fixture 승인 fallback", "rank": len(approved)})
                    state["approved"] = approved
                    approved_ids.add(model_id)
...
def _run_model_approval_fixture(model_id):
    ...
    return {
        ...
        "tested_at_utc": datetime.utcnow().isoformat(),
    }


def model_candidate_dominates(challenger, incumbent):
    """품질·안정성 하드 게이트를 통과한 도전자만 기존 승인 모델을 대체할 수 있다.
    C: 근거·조건부 경로 충족률(높을수록 좋음), R: 재질의율, T: 중앙 총 토큰, L: 중앙 지연(낮을수록 좋음).
    비용·속도만 개선되고 근거 충실성 또는 구조 안정성이 저하된 후보의 승격을 금지한다."""
    required = ("hard_gate_pass", "coverage_rate", "retry_rate", "median_total_tokens", "median_latency_seconds")
    ...
    if not incumbent["hard_gate_pass"]:
        return True
    no_worse = (
        challenger["coverage_rate"] >= incumbent["coverage_rate"]
        and challenger["retry_rate"] <= incumbent["retry_rate"]
        and challenger["median_total_tokens"] <= incumbent["median_total_tokens"]
        and challenger["median_latency_seconds"] <= incumbent["median_latency_seconds"]
    )
    strictly_better = (
        challenger["coverage_rate"] > incumbent["coverage_rate"]
        or challenger["retry_rate"] < incumbent["retry_rate"]
        or challenger["median_total_tokens"] < incumbent["median_total_tokens"]
        or challenger["median_latency_seconds"] < incumbent["median_latency_seconds"]
    )
    return no_worse and strictly_better


def get_approved_model_roster():
    """승인된 후보만 순서대로 반환한다. 신규 발견은 Phase 3 fixture 승격 전까지 사용하지 않는다."""
    state = _refresh_model_roster_if_due()
    approved = [item for item in state.get("approved", []) if _model_id_from_url(_model_url(str(item.get("model_id", ""))))]
    if not approved:
        approved = [dict(item) for item in STATIC_APPROVED_MODELS]
    selected = []
    for item in sorted(approved, key=lambda row: (int(row.get("rank", 9999)), str(row.get("model_id", "")))):
...
def run_phase1(symbol_input, exchange_name, custom_tfs):
    ...
    try:
        ...
        if phase1_model_meta.get("failed"):
            return model_execution_hold_message(phase1_model_meta), None, None, None
