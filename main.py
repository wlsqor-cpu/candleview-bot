...
FC_EVENT_TRADE_MAX_CALLS = 2  # 체결데이터 하루당 최대 조회 횟수(500건/회) — 초과 시 capped=True로 명시
...
def _fc_event_trade_tape_features(exchange_class, symbol, day_start_ms, day_end_ms):
    """해당 하루(day_start_ms~day_end_ms) 체결 데이터에서 대형체결비중·누적매수매도Delta 산출.
    [가능한 이유] 호가창(order book)과 달리 체결 이력은 거래소가 영구 보관 — 과거 날짜 재현
    가능. D9 실측(2026-09-04) 확정사항인 ask_bid 필드(매수/매도 구분)·500건/회 상한을 그대로
    사용, 신규 가정 없음.
    [한계] 초고빈도 코인은 하루 체결이 수만 건일 수 있어 FC_EVENT_TRADE_MAX_CALLS로 상한을
    둔다. 상한 도달 시 'capped=True'로 명시해 완전집계인 척하지 않는다(coverage 정직성).
    [2026-09-05 진단로그 추가 — 실사용 결함 대응] 실제 배포 후 거래량이 폭증한 날짜에서도
    체결데이터가 계속 insufficient_data로 나오는 현상이 확인됨(사용자 보고). 원인 후보:
    (1) D9는 trades/ticks 엔드포인트의 '페이지네이션 방식'만 확인했지 '과거 보관기간
    한도'는 확인하지 못함 — 거래소가 최근 며칠치만 공개API로 보관하고 그 이전은 비워
    반환할 가능성. (2) ccxt의 since 파라미터가 업비트 내부적으로 다르게 해석될 가능성.
    원인 미확정 상태이며, 아래 print로 다음 실행부터 실제 batch 길이·예외 여부를 Render
    로그에서 직접 확인할 수 있게 한다(추측으로 결론내지 않기 위함)."""
    trades, since = [], day_start_ms
    for call_idx in range(FC_EVENT_TRADE_MAX_CALLS):
        try:
            batch = exchange_class.fetch_trades(symbol, since=since, limit=500)
        except Exception as e:
            print(f"[FC_EVENT][trade_tape] {symbol} since={since} 예외: {e}")
            return {"status": "error", "error": str(e)}
        print(f"[FC_EVENT][trade_tape] {symbol} call={call_idx} since={since} "
              f"수신건수={len(batch) if batch else 0} "
              f"첫timestamp={batch[0].get('timestamp') if batch else None} "
              f"끝timestamp={batch[-1].get('timestamp') if batch else None}")
        if not batch:
            break
        batch_filtered = [t for t in batch if t.get("timestamp") and t["timestamp"] < day_end_ms]
        if not batch_filtered:
            print(f"[FC_EVENT][trade_tape] {symbol} day_end_ms={day_end_ms} 이전 조건으로 "
                  f"필터링한 결과 0건(원본 {len(batch)}건은 있었음 — 날짜 범위 밖으로 전부 걸러짐)")
            break
        trades.extend(batch_filtered)
        last_ts = batch_filtered[-1].get("timestamp", since)
        if last_ts <= since:
            break
        since = last_ts + 1
        if len(batch_filtered) < 500:
            break
    if not trades:
        print(f"[FC_EVENT][trade_tape] {symbol} 최종 trades=0건 — insufficient_data 반환")
        return {"status": "insufficient_data"}
    buy_vol = sum(t.get("amount") or 0 for t in trades if t.get("side") == "buy")
    sell_vol = sum(t.get("amount") or 0 for t in trades if t.get("side") == "sell")
    total_vol = buy_vol + sell_vol
    if total_vol <= 0:
        return {"status": "insufficient_data"}
    avg_trade_size = total_vol / len(trades)
    large_threshold = avg_trade_size * 5  # 평균의 5배 이상을 '대형체결'로 정의(관측용 정의, 게이트 아님)
    large_vol = sum(t.get("amount") or 0 for t in trades if (t.get("amount") or 0) >= large_threshold)
    return {
        "status": "ok",
        "n_trades": len(trades),
        "buy_volume": round(buy_vol, 8),
        "sell_volume": round(sell_vol, 8),
        "delta_ratio": round((buy_vol - sell_vol) / total_vol, 4),  # 양수=매수우위, 음수=매도우위
        "avg_trade_size": round(avg_trade_size, 8),
        "large_trade_volume_pct": round(large_vol / total_vol, 4),
        "capped": len(trades) >= FC_EVENT_TRADE_MAX_CALLS * 500,
    }
...
def _fc_event_snapshot_full(daily_bars, as_of_idx, lookback, btc_daily_bars=None,
                             exchange_class=None, symbol=None):
    ...
    [2026-09-04 확장] exchange_class·symbol이 주어지면 체결데이터·구조적변곡·보조지표도
    함께 조회해 병합한다 — 선택적 인자라 기존 호출부(파라미터 미전달 시)는 영향 없음.
    ...
    지표(spring_reclaim·이동평균지지반등·체결데이터 등)는 마지막 봉을 직접 참조해 정확히
    as_of_idx를 보는 것과 하루 어긋남.
    해결: as_of_idx 봉을 복제해 맨 뒤에 한 번 더 붙인다. "완성봉 제외" 규칙 함수는 복제본만
    제외해 진짜 as_of_idx가 남고, "마지막봉 직접참조" 규칙 함수는 복제본이 as_of_idx와 값이
    동일해 결과가 같다 — 양쪽 다 정확히 as_of_idx를 보게 통일된다. 봉 1개 중복은 40~200봉
    창에서 무시 가능한 통계적 영향만 준다."""
    sliced = daily_bars[: as_of_idx + 1]
    ...
    fc_compute_compression(temp_item)
    fc_compute_box_range(temp_item)
    obs = fc_observation_tier1.compute_tier1_observation(
    ...
    obs["reused_compression_pct"] = temp_item.get("compression_pct")
    obs["reused_s_boxrange"] = temp_item.get("s_boxrange")
    obs["reused_box_status"] = temp_item.get("box_status")
    ...
    if exchange_class is not None and symbol is not None:
        day_start_ms = daily_bars[as_of_idx][0]
        day_end_ms = day_start_ms + 86400000
        obs["trade_tape"] = _fc_event_trade_tape_features(exchange_class, symbol, day_start_ms, day_end_ms)
    else:
        obs["trade_tape"] = {"status": "not_requested"}

    # [2026-09-04 신설] #25 결합판정 — 매수우위(체결Delta)이면서 동시에 가격은 정체된
    # '조용한 매집' 상태. trade_tape가 있어야만 판단 가능(delta_ratio 필요) — 원래 계획
    # (Part2 항목25)의 절반만 구현돼있던 것을 이번에 결합해 완성.
    if obs["trade_tape"].get("status") == "ok":
        today = sliced[-1]
        today_range = today[2] - today[3]
        today_body = abs(today[4] - today[1])
        body_pct = (today_body / today_range) if today_range > 0 else 1.0
        delta_ratio = obs["trade_tape"]["delta_ratio"]
        is_quiet_accum = (delta_ratio > 0.15) and (body_pct <= 0.35)
        obs["quiet_accumulation"] = {
            "status": "ok",
            "delta_ratio": delta_ratio,
            "body_pct_of_range": round(body_pct, 4),
            "is_quiet_accumulation": is_quiet_accum,
            "normalized": 1.0 if is_quiet_accum else 0.0,
        }
    else:
        obs["quiet_accumulation"] = {"status": "insufficient_data"}
...
def run_event_collection(chat_id, ex_name, symbol, ref_date_str):
    ...
    for offset in FC_EVENT_COLLECT_OFFSETS:
        ...
        obs = _fc_event_snapshot_full(bars, snap_idx, FC_MIN_BARS_STANDARD, btc_daily_bars,
                                       exchange_class=exchange_class, symbol=full_symbol)
