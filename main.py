...
# ============================================================
# [결함수정] Phase2 출력 사후검증 — R:R 재계산·정정 + FVG/OB 중첩서술 검증
# - 근거: 실사용 감사(STORJ/KRW)에서 P-06 선진입·돌파추격 손익비가 명시된 공식
#   어느 조합으로도 재현 불가한 순수 계산환각 확인, OB-FVG "상호중첩" 서술도
#   실제로는 겹침폭 0(경계접촉)인데 과장 서술된 사례 확인.
# - R:R은 숫자만 정밀 치환(문장구조 불변, 리스크 낮음). FVG/OB는 자연어 문장
#   재작성 대신 검증실패 시 경고 로그만 부착(자동 재작성은 새 왜곡 위험이 더 큼).
...
def verify_and_fix_phase2(text):
    ...
    entry = _f(r'진입\s*예상가\s*[:：]\s*([\d.]+)', text)
    inv = _f(r'무효화\s*손절선\s*[:：]\s*([\d.]+)', text)
    t1 = _f(r'1차\s*목표가\s*[:：]\s*([\d.]+)', text)

    if entry is not None and inv is not None and t1 is not None:
        # [P-06 정정 반영] 선진입/돌파추격 손익비의 Reward는 항상 P_target_1 기준(스펙 SSOT)
        m_early = re.search(r'(선진입\s*손익비\s*)([\d.]+)(\s*\(현재가\s*([\d.]+))', text)
        if m_early:
            p_cur = float(m_early.group(4))
            risk = abs(p_cur - inv)
            if risk > EPSILON:
                correct_rr = abs(t1 - p_cur) / risk
                stated = float(m_early.group(2))
                if abs(stated - correct_rr) > 0.05:
                    fixed = fixed.replace(m_early.group(0), f"{m_early.group(1)}{correct_rr:.2f}{m_early.group(3)}")
                    warnings.append(f"선진입 R:R 자동정정: {stated}→{correct_rr:.2f}")

        m_chase = re.search(r'(돌파\s*추격\s*손익비\s*)([\d.]+)(\s*\(돌파\s*추격\s*진입가\s*([\d.]+))', text)
        if m_chase:
            p_breakout = float(m_chase.group(4))
            chase_risk = abs(p_breakout - inv)
            if chase_risk > EPSILON:
                correct_rr = abs(t1 - p_breakout) / chase_risk
                stated = float(m_chase.group(2))
                if abs(stated - correct_rr) > 0.05:
                    fixed = fixed.replace(m_chase.group(0), f"{m_chase.group(1)}{correct_rr:.2f}{m_chase.group(3)}")
                    warnings.append(f"돌파추격 R:R 자동정정: {stated}→{correct_rr:.2f}")

    # FVG/OB "상호중첩" 서술 검증(수정은 안 함, 경고만 — 자연어 재작성 리스크 회피)
    m_overlap = re.search(
        r'\[OB\]\s*\(\s*([\d.]+)\s*~\s*([\d.]+)[^)]*\)[^가-힣]*(?:과|와)\s*가격\s*공백대\s*'
        r'\[FVG\]\s*\(\s*([\d.]+)\s*~\s*([\d.]+)[^)]*\)\s*가\s*상호\s*중첩',
        text)
    if m_overlap:
        ob_lo, ob_hi, fvg_lo, fvg_hi = map(float, m_overlap.groups())
        overlap_width = min(ob_hi, fvg_hi) - max(ob_lo, fvg_lo)
        if overlap_width <= 0:
            warnings.append(
                f"OB-FVG '상호중첩' 서술 검증실패: 실제 겹침폭={overlap_width:.2f}"
                f"(0 이하=경계접촉·인접일 뿐 중첩 아님)"
            )
...
def fc_compute_compression(item):
    ...
    if period_high <= 0:
        item["compression_pct"] = None
        return item
    item["compression_pct"] = abs(period_high - current_price) / period_high * 100.0
    return item
...
def fc_build_candidate_payload(exchange_class, candidates, btc_daily=None):
...
def run_findcoin(ex_name):
    ...
    candidate_payload, used_candidates = fc_build_candidate_payload(exchange_class, candidates, btc_daily)
