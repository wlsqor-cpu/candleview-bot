...
# ============================================================
VWAP_LOOKBACK_BARS = 20  # 엔진 스펙의 거래량 배율 기준(VOL_AVG_LOOKBACK=20봉)과 정합
...
def run_phase1(symbol_input, exchange_name, custom_tfs):
    ...
    try:
        ...
        for i, tf in enumerate(custom_tfs):
            ...
            tf_vwap = calculate_rolling_vwap(recent)
            payload += f"\n[{tf} 세력 평단 추정치 (Plugin 10)]\n"
            if tf_vwap is not None:
                payload += f"최근 {VWAP_LOOKBACK_BARS}봉 거래량가중평균가(VWAP): {tf_vwap:.8g}\n"
            else:
