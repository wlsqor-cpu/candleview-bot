...
STATIC_APPROVED_MODELS = [
    # Google Gemini API 공식 모델 문서(2026-08-18 확인): 현행 최상위 안정 Flash.
    {"model_id": "gemini-3.7-flash", "selection_source": "공식 기준 초기 roster", "rank": 0},
    # 1차가 API·구조 검증에서 실패할 때만 순차 사용한다.
    {"model_id": "gemini-3.6-flash", "selection_source": "공식 기준 fallback", "rank": 1},
    {"model_id": "gemini-3.5-flash-lite", "selection_source": "공식 기준 fallback", "rank": 2},
]
...
def _load_model_roster_state():
    try:
        with open(MODEL_ROSTER_STATE_FILE, "r", encoding="utf-8") as fh:
            state = json.load(fh)
        if state.get("schema_version") != 2 or not isinstance(state.get("approved"), list):
