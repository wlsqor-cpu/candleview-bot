...
# Render 웹 서비스용 포트 바인딩 (0.1초 만에 포트 즉시 오픈)
def run_port_server():
    port = int(os.environ.get("PORT", 10000))
    server = HTTPServer(("0.0.0.0", port), SimpleHTTPRequestHandler)
    print(f"포트 {port} 오픈 완료. Render 웹 서비스 수신 대기 중...")
...
print(
    f"🚀 78.15KB 원본 모듈({PROMPT_FILE_NAME}) 로드 성공! (글자 수:"
    f" {len(CANDLEVIEW_PROMPT_FULL)})"
)
