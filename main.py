...
def detect_regular_divergence(df):
    ...
    # RSI_DISTANCE_MAX는 두 피벗 "사이" 간격만 제한하고, 종료 피벗이 현재로부터 얼마나
    # 멀어졌는지는 제한하지 않았다. 그 결과 이미 가격이 크게 이동한 뒤에도 과거 바닥·천정
    # 다이버전스가 현재 국면의 확정 근거로 계속 인용됐다(실사용 확인: 종료 피벗 대비 +109%).
    # 신호가 유효한 관측 창은 이미 RSI_DISTANCE_MAX로 정의돼 있으므로 같은 값을 종료 피벗의
    # 경과 봉수에도 동일 적용한다. 신규 상수를 만들지 않는다.
    last_completed_index = len(work) - 1
    ...
    if len(highs_px) >= 2:
        for a in range(len(highs_px) - 1):
            ...
            if r2 < r1 and abs(delta) >= RSI_DELTA_MIN_DIV:
                elapsed = last_completed_index - i2
                if elapsed > RSI_DISTANCE_MAX_DIV:
                    continue
                tag = (
                    f"다이버전스: 일반하락 | 가격 {p1:.4g}→{p2:.4g} | "
                    f"RSI {r1:.1f}→{r2:.1f} (Δ{delta:+.1f}) | 간격 {dist}봉 | "
                    f"종료 피벗 경과 {elapsed}봉 | "
...
def _normalize_zero_width_range(value):
    """상단과 하단이 같은 구간은 범위가 아니라 단일 레벨이다.

    `49.8 ~ 49.8`처럼 폭이 0인 표기가 그대로 PHASE 2 경로에 인용되면 구간이 존재하는 것처럼
    읽힌다. 수치는 바꾸지 않고 중복된 경계 표기만 단일 값으로 접는다.
    """
    return re.sub(r"(\d+(?:\.\d+)?)\s*~\s*\1(?![\d.])", r"\1", str(value or ""))
    ...
    for section_match in re.finditer(section_pattern, text):
        ...
        for label in PHASE1_FACT_LABELS:
            ...
            if not fact_match:
                continue
            fact_ref = f"{tf}:{label}"
            registry[fact_ref] = {"fact_ref": fact_ref, "tf": tf, "label": label,
                                  "value": _normalize_zero_width_range(fact_match.group(1).strip())}
    return registry
...
def _load_confluence_area_min_from_spec():
    """중첩 최소 면적비는 코드에 재정의하지 않고 명세 SSOT에서만 읽는다."""
    match = re.search(r"CONFLUENCE_AREA_MIN\s*=\s*([0-9]+(?:\.[0-9]+)?)\s*%", CANDLEVIEW_PROMPT_FULL)
    return float(match.group(1)) if match else None


CONFLUENCE_AREA_MIN = _load_confluence_area_min_from_spec()
CONFLUENCE_UNCONFIRMED_TEXT = "미확인"


def _parse_price_range(value):
    """`18.37 ~ 22.00 (설명)` 형태에서 하단·상단만 읽는다. 범위 표기가 없으면 None."""
...
def _range_unit_suffix(value):
    """원본 범위 표기에서 통화 단위만 읽는다. 재계산 값이 단위 없이 인용되는 것을 막는다."""
    match = re.search(r"\d\s*([A-Z]{2,5})\b", str(value or ""))
    return match.group(1) if match else ""


def confluence_overlap_ratio(fvg_range, ob_range):
    """명세 7장 정의의 중첩 면적비(%)를 계산한다.

    분모는 두 구간 중 좁은 쪽의 폭이다. 넓은 쪽을 분모로 쓰면 좁은 구간이 완전히 포함돼도
    비율이 낮게 나와 `면적 교집합이 50% 이상 겹친다`는 정의와 어긋난다.
    """
    if not fvg_range or not ob_range:
        return None
    low = max(fvg_range[0], ob_range[0])
    high = min(fvg_range[1], ob_range[1])
    overlap = max(0.0, high - low)
    narrow = min(fvg_range[1] - fvg_range[0], ob_range[1] - ob_range[0])
    return None if narrow <= 0 else overlap / narrow * 100.0


def reconcile_confluence_zone(registry):
    """중첩 매물대를 명세 산식으로 재계산해 PHASE 2가 쓰는 registry 값을 확정한다.

    FVG·OB 구간은 이미 PHASE 1 카드에 수치로 표시돼 있고, 중첩 판정은 그 두 구간의
    산술 교집합이다. 그런데 이 항목은 Python 검증 없이 모델 판정에만 의존해 왔고,
    실사용에서 면적비 58.6%(기준 50%)인 구간이 `미확인`으로 표기된 사례가 확인됐다.
    새 수치를 만들지 않고 이미 표시된 두 구간만으로 재계산한다.
    """
    observations = []
    if CONFLUENCE_AREA_MIN is None or not isinstance(registry, dict):
        return registry, observations
    for tf in phase1_registry_timeframes(registry):
        fvg = _parse_price_range((registry.get(f"{tf}:가격 공백대") or {}).get("value"))
        ob = _parse_price_range((registry.get(f"{tf}:세력 매물대") or {}).get("value"))
        ratio = confluence_overlap_ratio(fvg, ob)
        if ratio is None:
            continue
        key = f"{tf}:중첩 매물대"
        current = str((registry.get(key) or {}).get("value") or "").strip()
        confirmed = ratio >= CONFLUENCE_AREA_MIN
        if confirmed:
            low = max(fvg[0], ob[0])
            high = min(fvg[1], ob[1])
            unit = _range_unit_suffix((registry.get(f"{tf}:세력 매물대") or {}).get("value")) \
                or _range_unit_suffix((registry.get(f"{tf}:가격 공백대") or {}).get("value"))
            expected = f"{_format_origin_fact_number(low)} ~ {_format_origin_fact_number(high)}"
            if unit:
                expected += f" {unit}"
        else:
            expected = CONFLUENCE_UNCONFIRMED_TEXT
        already = _parse_price_range(current) is not None
        if confirmed == already and (not confirmed or _parse_price_range(current) == (low, high)):
            continue
        registry[key] = {"fact_ref": key, "tf": tf, "label": "중첩 매물대",
                         "value": expected, "source": "python_confluence_recalc"}
        observations.append(f"CONFLUENCE_RECALC:{tf}:{ratio:.1f}%:{'중첩' if confirmed else '미확인'}")
    return registry, observations
...
# 명세 2장 [고정 승인 모델 운영] 1항: 승인 roster는 위 두 개로 고정하며 목록 밖 모델은
# 탐색·등록·실행하지 않는다. 모델 추가는 같은 항 4항의 수동 검증·명세 갱신 절차로만 허용한다.
...
def get_approved_model_roster():
    """분석 실행에는 명세 2장이 승인한 고정 모델만 순서대로 사용한다.
    뒤 순위 모델은 앞 순위가 quota/503/read timeout으로 실제 실패했을 때만 도달한다.
...
def call_gemini_api_with_retry(full_prompt, max_tokens=16384, preferred_url=None, return_metadata=False,
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        last_model_url = url
        # 명세 2장 2항의 429 단기 대기 재시도는 transport 예산과 별개로 1회를 추가 확보해야 한다.
        # range() 고정 루프에서는 PHASE 1(max_transport_attempts=1)일 때 대기만 하고 재호출이
        # 발생하지 않아 규정이 실행되지 않았다.
        transport_attempt = 0
        transport_budget = max_transport_attempts
        while transport_attempt < transport_budget:
            transport_attempt += 1
            try:
                ...
                if res.status_code == 200:
                    ...
                    time.sleep(_bounded_retry_delay_seconds(transport_attempt - 1))
                    ...
                    time.sleep(_bounded_retry_delay_seconds(transport_attempt - 1, _response_retry_after_seconds(res)))
                else:
    ...
    try:
        ...
        for i, tf in enumerate(custom_tfs):
            ...
            if len(recent) > 0:
                ...
                # 중심가는 고가·저가의 십진 중간값이다. float 나눗셈은 (9.36+7.81)/2를
                # 8.584999999999999로 만들고 그 값이 곧 최단 무손실 표현이라 표시단에서는
                # 되돌릴 수 없다. 원천 문자열을 Decimal로 받아 십진 연산으로 확정한다.
                # 고가·저가·현재가는 원천 값을 그대로 보존하며 이 계산에만 적용한다.
                origin_tf_states[tf] = {
                    ...
                    "current_candle": {
                        ...
                        "low": current_row["low"],
                        "midpoint": (Decimal(str(current_row["high"])) + Decimal(str(current_row["low"]))) / 2,
                    },
                }
            else:
                origin_tf_states[tf] = {"quality_status": quality["status"]}
        ...
        supplement["phase1_fact_registry"], confluence_observations = reconcile_confluence_zone(
            supplement["phase1_fact_registry"]
        )
        if confluence_observations:
            supplement["phase1_confluence_observations"] = confluence_observations
            print("[INFO] 중첩 매물대 재계산: " + ", ".join(confluence_observations))
...
def _evidence_strength(scores):
    """명세 9장 확률식의 근거강도 = MIN(1.0, |ΣS| / (축수 × CORE_SCORE_CAP)).

    우세등급은 표수(같은 방향 축 개수)로, 근거강도는 크기(순합 절대값)로 정해지는 별도 축이다.
    등급만 표시하면 축이 서로 상쇄된 상태에서도 확신도가 높은 것처럼 읽히므로 함께 표시한다.
    """
    if LEDGER_SCORE_LIMIT is None or not scores:
        return None
    maximum = Decimal(str(len(scores))) * Decimal(str(LEDGER_SCORE_LIMIT))
    if maximum <= 0:
        return None
    total = sum(Decimal(str(value)) for value in scores.values())
    return min(Decimal("1"), abs(total) / maximum).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
...
def build_phase2_display_contract(text):
    ...
    if direction == "횡보":
        return {"direction": direction, "prices": {}, "probabilities": None,
                "evidence_strength": _evidence_strength(scores)}, []
    ...
    return {"direction": direction, "prices": prices, "probabilities": probabilities,
            "evidence_strength": _evidence_strength(scores)}, warnings
...
# 경로 섹션 범위는 "다음 섹션 제목"을 종료 앵커로 삼지 않는다. 앵커가 한 번이라도 빠지면
# 본문 끝까지가 경로로 인식돼 3️⃣ 이후 정상 서술이 통째로 삭제되기 때문이다.
# 명세 11장의 경로 항목은 화살표 나열과 괄호 주석뿐이므로, 그 형태가 끝나는 지점에서 멈춘다.
MAIN_PATH_HEADER_PATTERN = re.compile(r"^[^\n]*📈\s*메인 시나리오 파동 경로[^\n]*$")
ALT_PATH_HEADER_PATTERN = re.compile(r"^[^\n]*📉\s*대체 시나리오 파동 경로[^\n]*$")
PATH_PROBABILITY_LINE_PATTERN = re.compile(r"\s*\(확률[^\n]*\)\s*")


def _is_path_body_line(line):
    """경로 항목 행인지 판정한다. 그 외는 다음 섹션·문단의 시작으로 본다.

    명세 11장의 경로 항목은 화살표 나열과 괄호 주석뿐이다. 일반 불릿(-, •)까지 경로로
    인정하면 경로 섹션 바로 뒤의 불릿 문단이 본문으로 흡수돼 조용히 삭제될 수 있으므로
    제외한다. 모델이 불릿으로 경로를 쓰면 카드 아래에 그대로 남을 뿐이며, 실패 방향이
    삭제가 아닌 미치환이 되도록 의도한 선택이다.
    """
    stripped = str(line or "").strip()
    return not stripped or stripped.startswith(("➔", "→", "(", "（"))


def _path_section_end(lines, header_index):
    """헤더 다음 줄부터 경로 항목 행만 소비하고, 말미 공백 줄은 다음 섹션 간격으로 되돌린다."""
    end = header_index + 1
    while end < len(lines) and _is_path_body_line(lines[end]):
        end += 1
    while end - 1 > header_index and not lines[end - 1].strip():
        end -= 1
    return end


def _find_path_header(lines, pattern):
...
def _classify_route_context_line(line):
    ...
    선행 기호는 화살표만 인정한다. 일반 불릿(-, •)까지 인정하면 경로 섹션 인접 문단의
    불릿 행이 미채움 template로 오분류돼 조용히 삭제될 수 있다.
    """
    text = str(line or "")
    if re.search(r"(확률|현재가|진입|목표|무효화|손절)", text) and re.search(r"\d", text):
        return "drop_unverified_decision"
    # 모델이 명세 라벨 `(확률 (수치)% — 참고용, 백테스트 검증치 아님)`을 두 줄로 쪼개 쓰면
    # 주석부만 단독 행으로 남아 카드 아래에 노출된다. 경로 항목이 아니므로 제거한다.
    if re.fullmatch(r"\s*\(\s*참고용,?\s*백테스트\s*검증치\s*아님\s*\)\s*", text):
...
# 명세 11장이 규정한 경로 나열 순서. 카드를 섹션 최상단에 붙이면 진입·목표가가 FVG·역할
# 전환선보다 앞서 나와 가격 진행 순서로 읽히지 않는다. 아래 순서로 안정 정렬한다.
MAIN_PATH_LINE_ORDER = (
    ("현재가",), ("근거리", "진입"), ("공백대", "FVG"), ("돌파선",), ("1차 목표",), ("2차 목표",),
)
ALT_PATH_LINE_ORDER = (
    ("현재가",), ("역할 전환", "Role Reversal"), ("무효화", "손절"), ("다음 매물대",),
)
UNORDERED_PATH_LINE_RANK = 90


def _path_line_rank(line, order_table):
    """행을 명세 순서표의 순위로 바꾼다. 표에 없는 행은 뒤쪽에 원순서로 남긴다."""
    text = str(line or "")
    if PATH_PROBABILITY_LINE_PATTERN.fullmatch(text) or text.strip().startswith("(확률"):
        return 0
    for index, keywords in enumerate(order_table, start=1):
        if any(keyword in text for keyword in keywords):
            return index
    return UNORDERED_PATH_LINE_RANK


def _render_path_block(header, card, context, direction_label=None, order_table=None):
    """헤더·카드·잔여 보조 근거를 명세 서식 순서로 조립한다.

    정렬은 행을 지우거나 합치지 않는다. 순서표에 없는 행은 원래 상대순서 그대로 뒤에 남으므로,
    분류에 실패해도 최악의 결과는 '정렬되지 않음'이며 정보 손실은 발생하지 않는다.
    """
    rendered_header = str(header).rstrip()
    if direction_label:
        rendered_header += f" [{direction_label}]"
    body = [line for line in str(card or "").splitlines() if line.strip()]
    body += [line for line in str(context or "").splitlines() if line.strip()]
    if order_table:
        body = [line for _, _, line in sorted(
            ((_path_line_rank(line, order_table), position, line) for position, line in enumerate(body)),
            key=lambda item: (item[0], item[1]),
        )]
    return [rendered_header] + body


def _append_decision_cards_without_edit(rendered, main_card, alt_card, main_direction, alt_direction):
    """경로 헤더를 찾지 못했을 때 본문을 한 글자도 지우지 않고 카드만 덧붙인다.

    이전 구현은 이 경우 본문 전체에서 결정값·template 행을 삭제했다. 삭제는 3️⃣~6️⃣의
    정상 서술까지 조용히 지울 수 있으므로, 손실 없는 추가 표시로 대체한다.
    """
    return (
        rendered.rstrip()
        + f"\n\n**📈 메인 시나리오 파동 경로 [{main_direction}]**\n" + main_card
        + f"\n\n**📉 대체 시나리오 파동 경로 [{alt_direction}]**\n" + alt_card
    )


def _strip_nonroute_probability_lines(lines):
    """경로 헤더 바로 다음이 아닌 확률 줄만 제거한다. 카드가 표시한 확률은 보존된다."""
    result, removed = [], False
    for line in lines:
        if PATH_PROBABILITY_LINE_PATTERN.fullmatch(line):
            previous = next((item for item in reversed(result) if item.strip()), "")
            if not (MAIN_PATH_HEADER_PATTERN.match(previous) or ALT_PATH_HEADER_PATTERN.match(previous)):
                removed = True
                continue
        result.append(line)
...
SIDEWAYS_PATH_NOTICE = "횡보 — 방향성 목표가 미확정"

# 아래 두 항목은 이상이 아니라 설계상 매 실행 기대되는 정상 치환의 부산물이다.
# PHASE 2 프롬프트가 모델에게 경로 섹션의 가격·확률 숫자를 쓰지 말라고 지시하므로 미채움
# template 제거는 항상 발생하고, 분석 대상 현재가도 origin close 표기로 항상 정규화된다.
# 이를 표시 결속 경고로 취급하면 정상 분석에서도 정격 검증 태그가 영구히 출력되지 않는다.
# 관측 로그(DISPLAY_CONTRACT_OBSERVED)에는 계속 남기고, 태그 차단에서만 제외한다.
NON_BLOCKING_DISPLAY_OBSERVATIONS = (
    "미채움 경로 template 제거",
    "분석 대상 현재가 source 정규화",
    "분석 대상 현재가 source 보완",
)


def _build_decision_cards(contract, quote, display_warnings):
    """검증된 ledger에서 메인·대체 카드 문자열을 만든다. 경고 부여 책임도 여기에 모은다.

    명세 9장 ④·11장은 횡보를 정상 결론으로 규정하므로, 횡보는 검증 실패가 아니라
    고정 표기로 처리하고 표시 결속 경고를 남기지 않는다.
    """
    if contract and contract.get("direction") == "횡보":
        return SIDEWAYS_PATH_NOTICE, SIDEWAYS_PATH_NOTICE
    if not contract:
        display_warnings.append("결정값 표시 보류")
        held = "(결정값 검증보류 — 가격경로 확인 필요)"
        return held, held
    ...
    return main_card, alt_card


VISIBLE_DECISION_PATTERN = re.compile(
    r"(?P<head>(?:상방|하방|횡보)\s*우세\s*\(\s*우세\s*등급\s*:\s*(?:강|보통|약함|횡보)(?![가-힣]))"
    r"(?P<tail>[^)\n]*\))"
)


def _annotate_visible_decision_strength(text, contract):
    """우세등급 표기 옆에 검증된 근거강도를 한 번만 병기한다.

    등급은 표수, 근거강도는 순합 크기라는 서로 다른 축이라 등급이 강이어도 축 상쇄로 확률이
    50%대에 머무를 수 있다. 두 값을 함께 보여야 확신도가 과대 전달되지 않는다.
    결론·등급 문자열 자체는 바꾸지 않고 괄호 안 표기만 보완한다.
    등급 뒤 구분자는 모델이 자유롭게 쓰므로(`, 근거강도:`, `/`, `—` 등) 특정 기호를 전제하지
    않고 닫는 괄호까지를 치환 대상으로 본다. 전제할 경우 모델이 먼저 쓴 검증되지 않은 값이
    그대로 발행된다. 등급 뒤 한글이 이어지면(예: `강함`) 등급 오인이므로 병기하지 않는다.
    """
    strength = (contract or {}).get("evidence_strength")
    if strength is None:
        return text
    return VISIBLE_DECISION_PATTERN.sub(
        lambda match: f"{match.group('head')} · 근거강도 {_format_display_number(strength)})",
        str(text or ""), count=1,
    )
...
def render_verified_phase2_decision_blocks(text, contract, quote, all_validation_warnings=None, display_warnings=None, phase1_fact_registry=None):
    ...
    rendered = _annotate_visible_decision_strength(rendered, contract)
    main_card, alt_card = _build_decision_cards(contract, quote, display_warnings)
    main_direction, alt_direction = _scenario_direction_labels(contract.get("direction") if contract else "횡보")

    lines = rendered.splitlines()
    main_index = _find_path_header(lines, MAIN_PATH_HEADER_PATTERN)
    alt_index = _find_path_header(lines, ALT_PATH_HEADER_PATTERN)
    if main_index is None or alt_index is None:
        display_warnings.append("결정값 경로 섹션 누락")
        return _append_decision_cards_without_edit(
            rendered, main_card, alt_card, main_direction, alt_direction
        ), display_warnings

    removed_kinds = set()
    # 뒤쪽 섹션부터 치환해야 앞쪽 치환으로 인덱스가 밀리지 않는다.
    for header_index, card, direction_label in sorted(
        ((main_index, main_card, main_direction), (alt_index, alt_card, alt_direction)), reverse=True
    ):
        section_end = _path_section_end(lines, header_index)
        context, kinds = _retain_route_context("\n".join(lines[header_index + 1:section_end]), return_metadata=True)
        removed_kinds |= kinds
        lines[header_index:section_end] = _render_path_block(
            lines[header_index], card, context, direction_label,
            ALT_PATH_LINE_ORDER if header_index == alt_index else MAIN_PATH_LINE_ORDER,
        )
    if "drop_empty_template" in removed_kinds:
        display_warnings.append("미채움 경로 template 제거")

    lines, nonroute_probability_removed = _strip_nonroute_probability_lines(lines)
    ...
    blocking_display_warnings = [
        ...
        if warning not in NON_BLOCKING_DISPLAY_OBSERVATIONS
    ]
    if not all_validation_warnings and not blocking_display_warnings:
        rendered = rendered.rstrip() + "\n\n시스템 무결성 검증 완료\n■ API Direct Data Parsing 완료\n■ Layer 5-B 인라인 검증 100% 통과"
    return rendered.rstrip(), display_warnings


# 명세 11장 4️⃣ 제목은 `**▶️ 1d 타임 프레임 분석**` 형태이므로 강조 표기를 선택 허용한다.
# 허용하지 않으면 볼드 제목에서 TF 근거 보완·완결성 관측이 통째로 동작하지 않는다.
TF_ANALYSIS_HEADING_PATTERN = re.compile(
    r"(?ms)(?P<heading>^\s*(?:\*\*)?\s*▶️\s*(?P<tf_heading>[^\n]*?)\s*타임\s*프레임\s*분석\s*(?:\*\*)?\s*\n)"
    r"(?P<body>.*?)(?=^\s*(?:\*\*)?\s*▶️\s*[^\n]*?\s*타임\s*프레임\s*분석\s*(?:\*\*)?\s*\n|^\s*(?:\*\*)?\s*5️⃣|\Z)"
)
PHASE2_INTEGRATION_SECTION_PATTERN = re.compile(
    r"(?ms)(?P<header>^\s*(?:\*\*)?\s*5️⃣[^\n]*\n)(?P<body>.*?)(?=^\s*(?:\*\*)?\s*6️⃣|\Z)"
)
# 종료 앵커를 특정 제목 하나에만 의존하면 모델이 그 제목을 달리 쓸 때 현재가 결속이 통째로
# 미작동한다. 다음 강조 제목·번호 섹션·문서 끝 중 먼저 오는 지점까지를 본문으로 본다.
PHASE2_ANALYSIS_TARGET_SECTION_PATTERN = re.compile(
    r"(?ms)(?P<header>^\s*(?:\*\*)?\s*분석 대상\s*(?:\*\*)?\s*\n)"
    r"(?P<body>.*?)(?=^\s*(?:\*\*)\s*\S|^\s*[1-6]\uFE0F\u20E3|^\s*[\U0001F4C8\U0001F4C9\U0001F9ED\u23F0]|\Z)"
)
...
def normalize_phase2_analysis_target_current_price(text, phase1_fact_registry):
    ...
    if price_match:
        ...
        observation = "분석 대상 현재가 source 정규화"
    else:
        # 모델이 명세 서식의 현재가 표기를 통째로 빠뜨리면 분석 기준가가 사라진다.
        # 치환할 대상이 없을 뿐 값은 PHASE 1 origin close로 확정돼 있으므로 그대로 보완한다.
        body_lines = match.group("body").splitlines()
        for index, line in enumerate(body_lines):
            if line.strip():
                body_lines[index] = line.rstrip() + f" {canonical_price}"
                break
        else:
            return rendered, []
        replacement_body = "\n".join(body_lines)
        observation = "분석 대상 현재가 source 보완"
    return rendered[:match.start()] + match.group("header") + replacement_body + rendered[match.end():], [observation]
...
# 백필 행의 고정 접두어. 중복 삽입 판정과 문구가 한 곳에서만 정의되도록 상수로 둔다.
BACKFILL_PROGRESS_PREFIX = "진행봉 관측: "


def _fact_numbers_already_narrated(value, source):
    """같은 사실이 이미 서술됐는지 문자열이 아니라 수치로 판정한다.

    모델은 같은 값을 다른 문장으로 쓴다(예: registry `고가 148.0 KRW / 저가 114.0 KRW` ↔
    본문 `진행봉(고가 148.0 KRW, 저가 114.0 KRW)`). 문자열 완전일치로 검사하면 항상
    미서술로 판정돼 같은 사실이 모든 TF에 중복 삽입된다.
    """
    numbers = re.findall(r"\d+(?:\.\d+)?", str(value or ""))
    if not numbers:
        return bool(str(value or "").strip()) and str(value) in str(source or "")
    text = str(source or "")
    # 부분문자열 대조는 148.0을 2148.05 안에서, 8.0을 58.0 안에서 찾아내 다른 수치를 같은
    # 수치로 오인한다. 앞뒤에 숫자나 소수점이 붙지 않은 경우에만 같은 값으로 인정한다.
    return all(
        re.search(r"(?<![\d.])" + re.escape(number) + r"(?![\d])", text)
        for number in numbers
    )


def _phase2_tf_missing_state_facts(tf, body, phase1_fact_registry, evidence_units=0, with_progress_note=True):
    """제목·방향만 남은 TF에 한해 현재 진행봉·미확정 경계를 같은 TF 사실로 보완한다.

    근거가 이미 충분한 TF(evidence_units 2 이상)에는 삽입하지 않는다. 본 보완은 얇은
    서술을 막기 위한 것이지 모든 TF에 같은 문장을 반복하기 위한 것이 아니다.
    """
    source = str(body or "")
    if evidence_units >= 2:
        return []
    ...
    facts = []
    if current_candle and not _fact_numbers_already_narrated(current_candle, source):
        # 대괄호 라벨을 겹쳐 쓰면 한 줄이 100자를 넘어 정작 TF 고유 근거보다 길어진다.
        # 값은 그대로 두고 라벨 표기만 압축한다.
        parts = [str(current_candle)]
        if current_price and not _fact_numbers_already_narrated(current_price, source):
            parts.insert(0, f"현재가 {current_price}")
        note = " — 확정 판단에는 사용하지 않음" if with_progress_note else ""
        facts.append(BACKFILL_PROGRESS_PREFIX + " / ".join(parts) + note)
    if structure_break and re.search(r"(미확정|보류|대기)", structure_break) and not _fact_numbers_already_narrated(structure_break, source):
        facts.append(f"구조 경계: {structure_break}")
    return facts
...
def _phase2_integration_missing_boundary(tf_registry, integration_body):
    ...
    선택 TF 전체가 같은 상태이면 TF별로 같은 문장을 반복 나열하지 않고 한 문장으로 요약한다.
    """
    ...
    for tf in phase1_registry_timeframes(tf_registry):
        ...
        if structure_break and re.search(r"(미확정|보류|대기)", structure_break) and structure_break not in source:
            ...
            states.append((tf, structure_break))
    if not boundaries:
        return ""
    distinct_states = {state for _, state in states}
    if len(states) > 1 and len(distinct_states) == 1:
        listed = ", ".join(tf for tf, _ in states)
        return f"통합 근거 경계: 선택 TF 전체({listed})가 [구조 돌파] {next(iter(distinct_states))}"
...
def ensure_phase2_tf_narrative_completeness(text, phase1_fact_registry):
    ...
    def replace(match):
        ...
        if direction_index is None:
            return match.group("heading") + body
        # 진행봉 주의 문구는 브리핑 전체에서 최초 1회만 붙인다. TF마다 반복하면 같은 경고가
        # 선택 TF 수만큼 늘어나 정작 TF별 고유 근거가 묻힌다.
        facts = _phase2_tf_missing_state_facts(
            tf, body, phase1_fact_registry, evidence_units=evidence_units,
            with_progress_note=not progress_note_used,
        )
        if any(fact.startswith(BACKFILL_PROGRESS_PREFIX) for fact in facts):
            progress_note_used.append(tf)
