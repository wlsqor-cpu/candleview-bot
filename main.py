...
def notion_create_prediction_row(payload):
    ...
    props = {
        ...
        "등급": {"select": {"name": payload.get("grade") or "미상"}},
...
def determine_first_touch_outcome(exchange_label, symbol, direction, invalidation, target_1, target_2, since_ms, timeframe="1h"):
    """판정 시각 이후 1시간봉으로 목표가·무효화가 중 먼저 터치한 쪽을 찾는다.
    같은 봉 안에서 무효화·목표가 동시에 걸치는 범위면 무효화를 우선한다(낙관적 왜곡 방지)."""
