...
STATIC_APPROVED_MODELS = [
    ...
    # 다음 최신 안정 Flash만 fallback으로 허용한다. 경량 모델로 품질을 낮춰 분석을 발행하지 않는다.
...
def _load_model_roster_state():
    try:
        with open(MODEL_ROSTER_STATE_FILE, "r", encoding="utf-8") as fh:
            state = json.load(fh)
        if state.get("schema_version") != 3 or not isinstance(state.get("approved"), list):
    ...
    with MODEL_ROSTER_LOCK:
        ...
        try:
            ...
            for candidate in discovered:
                ...
                # 우위가 증명되지 않은 후보는 metrics·discovered에만 보존한다. fallback roster에
                # 누적하면 오래된 또는 경량 모델이 1·2차 실패 뒤 분석을 발행할 수 있으므로 금지한다.
...
def get_approved_model_roster():
    ...
    approved = [
        ...
        if MODEL_ID_PATTERN.fullmatch(str(item.get("model_id", "")))
        and _model_id_from_url(_model_url(str(item.get("model_id", ""))))
    ]
    if not approved:
        approved = [dict(item) for item in STATIC_APPROVED_MODELS]
    selected = []
    # 1차와 다음 최신 안정 Flash 한 개만 호출 후보가 될 수 있다. 오래되었거나 경량인
    # 모델은 roster 파일에 남아 있어도 분석 fallback으로 사용하지 않는다.
