...
def _load_target_functions():
    ...
    for symbol in (
        ...
        "def _range_unit_suffix",
        "def confluence_overlap_ratio",
        "def reconcile_confluence_zone",
        "RSI_DELTA_MIN_DIV = ",
        "RSI_DISTANCE_MAX_DIV = ",
        "REGULAR_DIV_POINTS = ",
        "MAJOR_SWING_AMP_RATIO = ",
        "def _find_pivots",
        "def _filter_major_price_pivots",
        "def detect_regular_divergence",
        "def calculate_rma_rsi",
        "def _phase1_fact_value",
    ...
    ns["CONFLUENCE_AREA_MIN"] = ns["_load_confluence_area_min_from_spec"]()
    ns["CONFLUENCE_UNCONFIRMED_TEXT"] = "미확인"
...
class TestEvidenceStrengthAnnotation:
    ...
    def test_annotation_overrides_any_model_format(self, fns):
        """[DOS 실사례] 명세에 병기 조항을 넣자 모델이 자기 서식(`, 근거강도: 0.53`)으로
        먼저 써버렸고, 정규식이 `·`만 전제해 빗나가 검증되지 않은 값이 그대로 발행됐다."""
        from decimal import Decimal
        contract = {"evidence_strength": Decimal("0.19")}
        for tail in ("(우세 등급: 보통)", "(우세 등급: 보통, 근거강도: 0.53)",
                     "(우세 등급: 보통 · 근거강도 0.53)", "(우세 등급: 보통 / 근거강도 0.53)",
                     "(우세 등급: 보통 — 근거강도 0.53)"):
            out = fns["_annotate_visible_decision_strength"]("상방 우세 " + tail, contract)
            assert out == "상방 우세 (우세 등급: 보통 · 근거강도 0.19)", tail

    def test_annotation_is_idempotent(self, fns):
        from decimal import Decimal
        contract = {"evidence_strength": Decimal("0.19")}
        once = fns["_annotate_visible_decision_strength"](
            "상방 우세 (우세 등급: 보통, 근거강도: 0.53)", contract)
        assert fns["_annotate_visible_decision_strength"](once, contract) == once
        assert once.count("근거강도") == 1

    def test_malformed_decision_is_left_untouched(self, fns):
        """등급 오인·괄호 미닫힘·줄바꿈은 병기하지 않고 원문을 보존한다(손상 방지)."""
        from decimal import Decimal
        contract = {"evidence_strength": Decimal("0.19")}
        for text in ("상방 우세 (우세 등급: 강함)", "상방 우세 (우세 등급: 보통",
                     "상방 우세 (우세 등급: 보통,\n근거강도 0.5)", "상방 우세 (우세 등급: 미정)"):
            assert fns["_annotate_visible_decision_strength"](text, contract) == text
...
# ============================================================
# [엔진 판정 항목 검증] 실사용 5차 표본(SKR)에서 확인된 구조적 공백 고정
# 표시 계층이 아니라 판정을 만드는 항목 자체를 대상으로 한다.
# ============================================================
class TestDivergenceRecency:
    """[SKR 실사례] RSI_DISTANCE_MAX는 두 피벗 '사이' 간격만 제한하고 종료 피벗이
    현재로부터 얼마나 멀어졌는지는 제한하지 않아, 가격이 +109% 오른 뒤에도 바닥
    다이버전스가 '확정적 근거'로 인용됐다."""

    def _frame(self, fns, prices):
        import pandas as pd
        frame = pd.DataFrame({"open": prices, "high": [x * 1.005 for x in prices],
                              "low": [x * 0.995 for x in prices], "close": prices})
        frame["rsi"] = fns["calculate_rma_rsi"](frame["close"])
        return frame

    def _base_prices(self, fns):
        """실제로 Regular 다이버전스가 성립하는 계열을 고정 시드로 탐색해 재현성을 확보한다.
        (피벗·주요스윙·ΔRSI 조건을 모두 통과해야 하므로 임의 계열로는 재현되지 않는다)"""
        import random
        random.seed(7)
        for _ in range(200):
            prices = [100.0]
            for i in range(12):
                prices.append(prices[-1] * (1 - random.uniform(0.02, 0.09) if i % 3 else 1 + random.uniform(0.01, 0.04)))
            for _ in range(5):
                prices.append(prices[-1] * (1 + random.uniform(0.02, 0.06)))
            for i in range(12):
                prices.append(prices[-1] * (1 - random.uniform(0.01, 0.045) if i % 3 else 1 + random.uniform(0.005, 0.02)))
            for _ in range(4):
                prices.append(prices[-1] * (1 + random.uniform(0.02, 0.05)))
            if "미검출" not in fns["detect_regular_divergence"](self._frame(fns, prices)):
                return prices
        raise AssertionError("다이버전스 성립 계열을 생성하지 못함")

    def _series(self, fns, tail_bars=0):
        prices = list(self._base_prices(fns))
        for _ in range(tail_bars):
            prices.append(prices[-1] * 1.04)
        return self._frame(fns, prices)

    def test_fresh_divergence_is_detected(self, fns):
        tag = fns["detect_regular_divergence"](self._series(fns))
        assert "일반상승" in tag or "일반하락" in tag
        assert "종료 피벗 경과" in tag, "경과 봉수가 태그에 기록돼야 한다"

    def test_stale_divergence_is_dropped(self, fns):
        """종료 피벗이 RSI_DISTANCE_MAX봉을 넘겨 멀어지면 후보에서 제외된다."""
        tag = fns["detect_regular_divergence"](self._series(fns, tail_bars=40))
        assert "미검출" in tag, f"경과 초과인데 검출됨: {tag}"

    def test_recency_limit_reuses_existing_constant(self):
        """신규 상수를 만들지 않고 기존 SSOT를 재사용해야 한다."""
        main_src = _find_main_py().read_text(encoding="utf-8")
        assert "if elapsed > RSI_DISTANCE_MAX_DIV:" in main_src
        assert "DIVERGENCE_RECENCY" not in main_src

    def test_boundary_between_detect_and_drop(self, fns):
        detected = [n for n in (0, 10, 20) if "미검출" not in fns["detect_regular_divergence"](self._series(fns, n))]
        dropped = [n for n in (29, 35, 40) if "미검출" in fns["detect_regular_divergence"](self._series(fns, n))]
        assert detected == [0, 10, 20]
        assert dropped == [29, 35, 40]


class TestConfluenceRecalculation:
    """[SKR 실사례] 면적비 58.6%(기준 50%)인 1h 구간이 '미확인'으로 표기됐다.
    중첩 판정은 이미 표시된 FVG·OB 두 구간의 산술 교집합인데 Python 검증이 없었다."""

    def _registry(self, rows):
        registry = {}
        for tf, fvg, ob, conf in rows:
            for label, value in (("가격 공백대", fvg), ("세력 매물대", ob),
                                 ("중첩 매물대", conf), ("현재가", "22.37 KRW")):
                registry[f"{tf}:{label}"] = {"fact_ref": f"{tf}:{label}", "tf": tf,
                                             "label": label, "value": value}
        return registry

    def test_threshold_comes_from_spec(self, fns):
        assert fns["CONFLUENCE_AREA_MIN"] == 50.0
        spec = _find_spec_file().read_text(encoding="utf-8")
        assert "CONFLUENCE_AREA_MIN = 50.0%" in spec

    def test_ratio_uses_narrow_side_as_denominator(self, fns):
        assert fns["confluence_overlap_ratio"]((10.0, 30.0), (15.0, 20.0)) == 100.0
        assert fns["confluence_overlap_ratio"]((10.0, 20.0), (15.0, 25.0)) == 50.0
        assert fns["confluence_overlap_ratio"]((10.0, 20.0), (20.0, 30.0)) == 0.0

    def test_skr_case_is_corrected(self, fns):
        registry = self._registry([
            ("1w", "14.08 ~ 23.69 KRW", "23.61 ~ 25.94 KRW", "미확인"),
            ("1d", "12.01 ~ 13.08 KRW", "13.63 ~ 15.10 KRW", "미확인"),
            ("4h", "17.80 ~ 20.79 KRW", "15.95 ~ 18.74 KRW", "미확인"),
            ("1h", "18.37 ~ 22.00 KRW", "21.25 ~ 22.53 KRW", "미확인")])
        result, observations = fns["reconcile_confluence_zone"](registry)
        assert result["1h:중첩 매물대"]["value"] == "21.25 ~ 22 KRW", "58.6%인데 미확인 유지됨"
        for tf in ("1w", "1d", "4h"):
            assert result[f"{tf}:중첩 매물대"]["value"] == "미확인", tf
        assert observations == ["CONFLUENCE_RECALC:1h:58.6%:중첩"]

    def test_currency_unit_is_preserved(self, fns):
        registry = self._registry([("1h", "18.37 ~ 22.00 KRW", "21.25 ~ 22.53 KRW", "미확인")])
        result, _ = fns["reconcile_confluence_zone"](registry)
        assert result["1h:중첩 매물대"]["value"].endswith("KRW")

    def test_no_range_leaves_value_untouched(self, fns):
        registry = self._registry([("1h", "미확인", "21.25 ~ 22.53 KRW", "미확인")])
        result, observations = fns["reconcile_confluence_zone"](registry)
        assert result["1h:중첩 매물대"]["value"] == "미확인"
        assert observations == []

    def test_already_correct_value_is_not_rewritten(self, fns):
        registry = self._registry([("1h", "18.37 ~ 22.00 KRW", "21.25 ~ 22.53 KRW", "21.25 ~ 22 KRW")])
        _, observations = fns["reconcile_confluence_zone"](registry)
        assert observations == []

    def test_false_positive_is_corrected_to_unconfirmed(self, fns):
        """모델이 중첩으로 잘못 쓴 경우도 기준 미달이면 미확인으로 정정한다."""
        registry = self._registry([("4h", "17.80 ~ 20.79 KRW", "15.95 ~ 18.74 KRW", "17.80 ~ 18.74 KRW")])
        result, observations = fns["reconcile_confluence_zone"](registry)
        assert result["4h:중첩 매물대"]["value"] == "미확인"
        assert observations and observations[0].endswith("미확인")


class TestEngineAuditClauses:
    def test_spec_defines_new_engine_clauses(self):
        spec = _find_spec_file().read_text(encoding="utf-8")
        assert "[신호 최신성 제한 — 실사용 감사 결함수정]" in spec
        assert "[중첩 면적비 정의 및 Python 재계산 — 실사용 감사 결함수정]" in spec
