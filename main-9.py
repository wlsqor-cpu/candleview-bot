    ...
    try:
        ...
        if symbol not in exchange_class.markets:
            return (
                ...
                f"({ex_display}에 상장되지 않은 코인일 수 있습니다)",
                None, None, None
            )
...
def fc_prefilter_universe(exchange, quote):
    """FC-0: 스캔 대상 사전 정제. fetch_tickers() 1회 호출로 전종목 현재가·거래대금을
    가져온 뒤 거래정지·스테이블코인만 제외한다(거래대금 크기로는 배제하지 않음)."""
    try:
...
def run_findcoin(ex_name):
    ...
    fc_prompt = (
        ...
        f"FC-6의 고정 출력 서식대로 최종 결과를 완제 출력하십시오. "
...
def make_phase_keyboard(ex_name, symbol_raw, tfs):
    ...
    return {
        "inline_keyboard": [
            [{"text": "📋 Phase1 수집데이터 열람", "callback_data": f"phase1_view|{payload_tail}"}],
            [{"text": "📋 보간 지표 열람", "callback_data": f"supplement_view|{payload_tail}"}],
            [{"text": "📊 Phase2 코인 최종 분석내용 보기", "callback_data": f"phase2_run|{payload_tail}"}],
        ]
    }
    ...
    try:
        ...
        for update in updates.get("result", []):
            ...
            if callback:
                ...
                if action == "fc_detail":
                    ...
                    send_telegram_message(
                        chat_id,
                        f"🕯️ <b>CandleView</b>\n[{fc_ex_display}] {fc_symbol} — {', '.join(fc_tfs)} (자동 적용)\n"
            ...
            if len(parts) == 1:
                ...
                send_telegram_message(
                    chat_id,
                    f"🔎 <b>FindCoin</b> — {ex_display_fc} 전종목 스캔 시작\n"
                    f"전체 종목을 상대수급 기준으로 정밀 분석 중입니다.\n"
                    f"거래소 응답속도에 따라 1~3분 정도 소요될 수 있습니다."
                )
                ...
                watch_note = f"\n📌 참고: 데이터가 부족해 판정을 보류한 신규상장 코인 {n_watch}개 있음(배제 아님)" if n_watch > 0 else ""

                if n_gate2 == 0:
                    send_telegram_message(
                        chat_id,
                        f"🔎 <b>FindCoin 스캔 결과</b> — {ex_display_fc}\n\n"
                        f"총 스캔 {n_total}종목 → 유효 {n_valid}종목 → "
                        f"1차 통과(RTM·Percentile) {n_gate1}종목 → "
                        f"최종 통과(유동성) {n_gate2}종목"
                        f"{watch_note}\n\n"
                        f"⚠️ <b>관망 권고</b>\n"
                        f"현재 {ex_display_fc} 내 상대 수급·유동성 조건을 동시에 충족하는 "
                        f"고신뢰 분출 후보가 0개입니다.\n"
            ...
            status_msg = (
                f"🕯️ <b>CandleView</b>\n"
                f"[{ex_display}] {sym_mapped}/{quote} — {', '.join(tfs)} {tf_note}\n"
                f"Phase1 차트 데이터 수집 중..."
            )
