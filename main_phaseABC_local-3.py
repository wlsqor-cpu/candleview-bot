...
import fc_observation_tier1  # FindCoin Phase A~C — main.py를 참조하지 않는 독립 모듈(단방향 의존)
import fc_core_scoring  # FindCoin FC-2/FC-3 순수파이썬 State진단+8모듈(2026-09-05, 단방향 의존)
import fc_state3_structure  # FindCoin State3 옥석검증B: OB/FVG/Confluence/RoleReversal(단방향 의존)
...
# [PHASE2 실전 결과 추적용 — 2026-09 신설] Render free 플랜은 persistent disk를 지원하지 않고
# 파일시스템이 재배포·재시작·spin-down마다 초기화되므로(공식문서 확인), 판정 시각과 결과 확인
# 시각 사이(최소 며칠)에 반드시 최소 1회 재시작이 낀다 — 로컬 CSV/SQLite는 이 용도로 못 쓴다.
# Notion을 영속 계층으로 쓴다: 개인 워크스페이스는 무료로 페이지 무제한(Render 무료 Postgres처럼
# 30일 뒤 만료되지 않음). 토큰 미설정 시 추적 기능은 조용히 비활성화되고, 기존 텔레그램 발행
# 경로에는 전혀 영향을 주지 않는다(TRACKING_ENABLED로 매 호출마다 확인).
NOTION_TOKEN = os.environ.get("NOTION_TOKEN", "")
# [주의] 이 DB는 Notion 신형 멀티 데이터소스 구조로 생성됐다 — 여기 넣는 값은 데이터베이스
# 페이지 ID가 아니라 "데이터 소스 ID"다(생성 시 반환된 collection://<id> 중 <id> 부분).
...
def _extract_spec_numbers_source(full_prompt):
    """[Cowork 재검토 판정58번 신설] _spec_number()/CORE_SCORE_CAP 정규식이 검색할 본문에서
    판정 이력 대장(서술형 프로즈 — 과거 결함을 설명하며 "상수명=예시값" 형태를 인용문으로
    자주 사용)을 제외한다.

    [배경] re.search는 문서 전체에서 첫 매치를 반환하는데, 판정 이력 대장이 실제 Layer1/
    FindCon 정의보다 문서 앞쪽에 있어, 이력 서술이 상수명+숫자를 나란히 언급하면 그 서술이
    먼저 매치돼 조용히 오염될 수 있는 구조였다(실제 재현: BODY_RATIO_STRONG이 판정41번
    인용문에 정의부보다 먼저 등장 — 지금은 값이 우연히 일치해 무사했으나 상시 위험). 판정
    이력은 계속 누적되는 서술형 텍스트라 이 위험은 시간이 지날수록 커진다.
    [해법 — 기존 SSOT 경계마커 재사용, 신규 마커 발명 아님] build_standard_analysis_runtime_
    prompt가 이미 사용 중인 runtime_marker("(PA-VSA 전용 API 데이터 정밀 연산 엔진")로 판정
    이력 대장 전체(그 앞부분)를 잘라낸다. 단, 그 함수와 달리 FindCoin(14장) 이후 본문의
    상수도 검색 대상이어야 하므로 findcoin_marker에서 자르지 않고 문서 끝까지 유지한다.
    마커를 못 찾으면(스펙 구조 대개편 등) 안전하게 원본 전체로 폴백한다 — 조용한 실패 대신
    경고 로그만 남기고 기존 동작(전체 검색)을 유지해 회귀를 만들지 않는다."""
    ...
    if runtime < 0:
        print("[WARN] _spec_number 검색범위 경계마커를 찾지 못해 전체 문서(판정이력 포함)를 "
              "그대로 검색합니다 — 판정이력 오매칭 위험 방치 상태.")
        return full_prompt
    return full_prompt[runtime:]


CANDLEVIEW_SPEC_NUMBERS_SOURCE = _extract_spec_numbers_source(CANDLEVIEW_PROMPT_FULL)
...
# ============================================================
# [결함수정-파이프라인정합성] 한글 코인명 맵 주기적 갱신
# 근거: 기존에는 UPBIT_KOREAN_MAP/BITHUMB_KOREAN_MAP이 프로세스 기동 시 1회만 조회되고
# 이후 갱신되지 않았다. Render 서비스가 재배포 없이 장기간 떠 있는 동안 새로 상장된 코인은
# 한글명으로 영구히 인식되지 않아 "코인을 찾을 수 없습니다" 오탐이 발생하는 구조였다.
# 조회 실패 시 기존 정상 맵을 빈 dict로 덮어쓰지 않도록, 새 맵이 비어 있지 않을 때만 교체한다.
# ============================================================
KOREAN_MAP_REFRESH_INTERVAL_SECONDS = 6 * 60 * 60  # 6시간


def _refresh_korean_maps_once():
    global UPBIT_KOREAN_MAP, BITHUMB_KOREAN_MAP, _UPBIT_SYMBOL_TO_KOREAN, _BITHUMB_SYMBOL_TO_KOREAN
    new_upbit = fetch_upbit_korean_map()
    if new_upbit:
        UPBIT_KOREAN_MAP = new_upbit
        _UPBIT_SYMBOL_TO_KOREAN = {v: k for k, v in new_upbit.items()}
    else:
        print("[WARN] 업비트 한글 맵 갱신 실패 — 기존 맵 유지")
    new_bithumb = fetch_bithumb_korean_map()
    if new_bithumb:
        BITHUMB_KOREAN_MAP = new_bithumb
        _BITHUMB_SYMBOL_TO_KOREAN = {v: k for k, v in new_bithumb.items()}
    # 빗썸은 원래도 자체 한글명 미제공 거래소일 수 있어(빈 dict가 정상) 실패로 취급해 경고하지 않는다.
    print(f"[INFO] 한글 코인명 맵 갱신 완료: 업비트 {len(UPBIT_KOREAN_MAP)}개 / 빗썸 {len(BITHUMB_KOREAN_MAP)}개")


def _run_korean_map_refresh_loop():
    while True:
        time.sleep(KOREAN_MAP_REFRESH_INTERVAL_SECONDS)
        try:
            _refresh_korean_maps_once()
        except Exception as e:
            print(f"[WARN] 한글 코인명 맵 갱신 스레드 오류: {e}")


threading.Thread(target=_run_korean_map_refresh_loop, daemon=True).start()
...
# ============================================================
# [신규기능-A안 실효성보강] Plugin 10(세력 평단 추정)·Plugin 11(RSI 모멘텀 기울기)
# 사용자 요청 항목(1·3·4번) 중 정량 근거가 필요한 두 가지를 Python에서 결정적으로
# 계산해 Volume Delta(Plugin 7)와 동일한 방식으로 payload에 첨부한다. Gemini가 이
# 값을 스스로 추정하지 않고 SSOT로 그대로 인용하도록 하기 위함이다(사후검증 불가한
# 정성적 판단 대신 재현 가능한 산식 사용). 두 값 모두 Layer 3.5 "서술 전용" 플러그인
# 원칙을 따른다 — Force Score(S_raw) 가중합에 가산하지 않으며, 방향·확률·가격산식에
# 개입하지 않는다(CandleView_API.txt Layer 3.5 [1]항 원칙과 동일).
# ============================================================
VWAP_LOOKBACK_BARS = 20  # 엔진 스펙의 거래량 배율 기준(VOL_AVG_LOOKBACK=20봉)과 정합. 동적 판별
# (determine_vwap_dynamic_lookback) 실패 시 폴백값, 그리고 1w·1d 외 TF의 고정값으로 계속 쓰인다.
RSI_MOMENTUM_LOOKBACK_BARS = 5
RSI_MOMENTUM_FLAT_THRESHOLD = Decimal("2.0")  # |ΔRSI| < 2.0p는 "유지"로 판정

# ============================================================
# [실효성보강-VWAP동적창] 세력 평단(VWAP) 관측 구간을 1w·1d에 한해 "지금 진행 중인 매집구간"에
# 맞춰 동적으로 정한다(2026-09 세션, 사용자 지적).
# - 근거: 고정 20봉은 매집기간이 그보다 길면 일부만 반영해 실제 평단과 어긋나고, 반대로
#   가용 최대치(100봉) 전체를 그냥 다 쓰면 이미 끝난 이전 파동의 가격까지 섞여 왜곡된다
#   ("옛 데이터는 파동이 끝나고 새로 시작한 것이라 의미 없다"는 지적과 동일 문제).
# - 설계: main.py 자체에 CandleView용 박스권 판정 로직이 없어(스펙의 "박스권 확정"은 지금도
#   Gemini 자체판단 — 29번 항목 참조), 신규 전용 판정 하나만 최소 범위로 추가한다.
#   ① 최근 100봉 내에서 다이버전스 계산에 이미 쓰는 주요 스윙 피벗(_find_pivots +
#   _filter_major_price_pivots, 좌우 2봉 + 진폭 60% 필터)을 재사용해, 주요 SH끼리·주요 SL끼리
#   각각 독립적으로 "가장 최근 피벗 가격"을 고정 기준 삼아 BOX_TOLERANCE_RATIO(스펙
#   BOX_PRICE_TOLERANCE=±2.8%와 동일값) 이내인 피벗만 최근 시점부터 역순으로 계속 편입한다
#   (기준을 편입할 때마다 넓히는 방식은 적대적 테스트에서 옛 무관 구간까지 드리프트로
#   끌려들어오는 결함이 확인돼 채택하지 않음 — 고정기준 방식으로 교체). SH·SL 각각 2개 이상
#   편입되면 "박스권 확정"으로 보고 그 편입 시작점(둘 중 이른 쪽)을 매집구간 시작점으로 삼는다.
#   ② 박스 미확정(추세 진행형)이면 가장 최근 주요 스윙 피벗(현재 파동의 시작점)부터로 본다.
#   ③ 스윙 자체가 없으면 None을 반환해 호출측이 기존 고정 VWAP_LOOKBACK_BARS(20봉)로 폴백한다.
# - 범위: 1w·1d 한정(CandleView_API.txt Plugin 10 서술 통합 범위와 동일). 4h·1h는 호출측에서
#   이 함수 자체를 쓰지 않고 기존 고정 20봉을 그대로 쓴다.
# - 이 함수는 VWAP 관측 창 길이만 결정한다 — 박스권 narrative(상/하단 표기, 삼각수렴 등 전체
#   서술)를 대체하지 않으며, Force Score·확률%·가격산식에는 관여하지 않는다(Layer 3.5 원칙 동일).
# ============================================================
BOX_TOLERANCE_RATIO = 0.028  # 스펙 BOX_PRICE_TOLERANCE(±2.8%)와 동일값, 이 판별 전용 재사용
VWAP_DYNAMIC_MIN_BARS = VWAP_LOOKBACK_BARS  # 창이 너무 짧아지는 것 방지(노이즈 하한)
VWAP_DYNAMIC_MAX_BARS = 100  # 스펙 박스권 확정 스캔범위(최근 100봉)와 동일 상한


def determine_vwap_dynamic_lookback(df):
    """1w·1d 세력 평단(VWAP) 관측 구간(봉 수)을 동적으로 판별한다. 판별 불가 시 None
    (호출측이 VWAP_LOOKBACK_BARS로 폴백)."""
    if df is None:
        return None
    total = len(df)
    if total < VWAP_DYNAMIC_MIN_BARS + 1:
        return None
    work = df.iloc[:-1].reset_index(drop=True)  # 진행봉 제외 — detect_regular_divergence와 동일 원칙
    if len(work) < VWAP_DYNAMIC_MIN_BARS:
        return None
    ...
    highs_raw, lows_raw = _find_pivots(high_arr, low_arr, left=2, right=2)
    highs_px, lows_px = _filter_major_price_pivots(highs_raw, lows_raw, high_arr, low_arr)
    if not highs_px and not lows_px:
        return None

    def _connected_cluster_start(idxs, arr):
        """가장 최근 피벗 가격을 고정 기준으로 삼아, 그 기준과 BOX_TOLERANCE_RATIO 이내인
        피벗만 최근 시점부터 역순으로 계속 편입한다(기준을 매번 갱신하지 않는 이유: 편입할
        때마다 기준 범위를 넓히면 사소한 연쇄 이탈이 누적돼 이미 끝난 무관한 과거 구간까지
        끌려 들어오는 드리프트가 생김 — 실제 적대적 테스트로 발견). 끊기면 즉시 중단한다.
        2개 이상 편입되면 그 시작 인덱스를 반환, 아니면 None(박스 미확정)."""
        if len(idxs) < 2:
            return None
        ordered = sorted(idxs)
        ref_price = float(arr[ordered[-1]])
        if ref_price <= 0:
            return None
        lo, hi = ref_price * (1 - BOX_TOLERANCE_RATIO), ref_price * (1 + BOX_TOLERANCE_RATIO)
        start, count = ordered[-1], 1
    ...
    sh_start = _connected_cluster_start(highs_px, high_arr)
    sl_start = _connected_cluster_start(lows_px, low_arr)
    box_start = min(sh_start, sl_start) if (sh_start is not None and sl_start is not None) else None

    if box_start is not None:
        anchor = box_start
    else:
        # ② 박스 미확정 — 가장 최근 주요 스윙(현재 파동의 시작점)부터
        candidates = list(highs_px) + list(lows_px)
        anchor = max(candidates) if candidates else None

    if anchor is None:
        return None

    lookback = total - anchor
    return max(VWAP_DYNAMIC_MIN_BARS, min(lookback, VWAP_DYNAMIC_MAX_BARS))


def calculate_rolling_vwap(df, lookback=VWAP_LOOKBACK_BARS):
    """최근 lookback개 봉(진행봉 포함) 구간의 거래량가중평균가(VWAP, Typical Price 기준)를
    계산한다. [세력 매물대(OB)]의 캔들-기하학적 정의를 대체하지 않고, "세력 평단 추정치"라는
    별도의 정량 보조 지표로만 병행 제공한다. 거래량 합이 0이거나 데이터가 없으면 None."""
    ...
    return float((typical_price * window["volume"]).sum() / volume_sum)


def calculate_rsi_momentum_slope(df, lookback=RSI_MOMENTUM_LOOKBACK_BARS):
    """현재 RSI와 lookback봉 전 RSI의 차이(ΔRSI)로 모멘텀의 강화/유지/약화를 판별한다.
    추세 지속력을 직접 예측하지 않고, 최근 RSI 궤적의 기울기만 결정적으로 보고한다.
    반환: (delta 또는 None, 상태 문자열)."""
    if df is None or len(df) <= lookback:
        return None, "데이터부족"
    current_rsi = df["rsi"].iloc[-1]
    past_rsi = df["rsi"].iloc[-1 - lookback]
    if pd.isna(current_rsi) or pd.isna(past_rsi):
        return None, "데이터부족"
    delta = Decimal(str(round(float(current_rsi) - float(past_rsi), 4)))
    if abs(delta) < RSI_MOMENTUM_FLAT_THRESHOLD:
        state = "유지(횡보)"
    elif delta > 0:
        state = "강화(상승)"
    else:
        state = "약화(하락)"
    return float(delta), state
...
def _spec_number(name, integer=False):
    ...
    [2026-09-05 수정] ROLE_REVERSAL_TOLERANCE 등 '±1.5%'처럼 부호가 붙은 값은 기존 정규식이
    0건 매칭으로 조용히 실패(None 반환)하던 결함을 FindCoin State3 작업 중 발견 — ±? 추가로
    수정. 기존 부호없는 상수 전체 재검증 완료(회귀 없음).
    [Cowork 재검토 판정58번] 검색 대상을 CANDLEVIEW_PROMPT_FULL(판정이력 대장 포함 전체)에서
    CANDLEVIEW_SPEC_NUMBERS_SOURCE(판정이력 제외, 실제 엔진 스펙 본문만)로 변경 — 판정이력의
    서술형 인용문이 실제 정의보다 먼저 매치돼 값을 오염시킬 수 있던 구조적 위험 제거."""
    match = re.search(rf"{re.escape(name)}\s*=\s*±?([0-9]+(?:\.[0-9]+)?)", CANDLEVIEW_SPEC_NUMBERS_SOURCE)
    if not match:
        return None
...
# [2026-09 Cowork 재검토 — 판정67번] 2장 SSOT(예: "REF_PROXIMITY_LIMIT = ±1.0%")에서 그대로 추출.
# 값은 퍼센트(예: 1.0)이며 비율로 쓸 때는 호출측에서 /100.0 한다. 기존 근접판정(Whale Wall
# 매칭·구조 일치도 등)은 전부 Gemini 프롬프트 지시문일 뿐 Python이 직접 쓰지 않았는데, OB
# Tier1 검증에서 Python이 최초로 이 상수를 실측 비교에 쓰므로 SSOT 추출로 도입한다.
REF_PROXIMITY_LIMIT_PCT = _spec_number("REF_PROXIMITY_LIMIT")
...
def build_phase1_fact_registry(phase1_result):
    ...
    [결함수정-파이프라인정합성] 엔진 스펙 "[3] PHASE 1 최종 출력 양식"(예: "**현재가**")은
    라벨을 마크다운 굵게(**label**)로 감싸 출력하도록 지시하고, sanitize_html()이 그 "**"를
    실제로 <b>로 변환하는 것으로 봐도 모델이 문자 그대로 "**"를 산출한다는 사실이 확인된다.
    그런데 기존 정규식 ^{label}은 줄 시작이 "**"인 실제 출력과 절대 일치하지 않아, 정상 형식의
    PHASE 1 카드에서도 registry가 사실상 항상 비어 있었다. 그 결과:
      1) validate_phase1_fact_registry가 매번 "registry 누락" 경고를 냄
      2) build_phase2_evidence_slots의 모든 슬롯(구조 상태·1d 컨텍스트·4h/1h 구간 등)이 항상
         빈 문자열로 PHASE 2에 전달됨 — PHASE1 사실 누락 방지용 근거 슬롯이 무력화됨
      3) fact_refs_for_prompt가 항상 빈 리스트가 되어, PHASE 2 프롬프트가 "반드시 아래 목록의
         값 하나를 fact_ref로 사용하라"고 요구하면서 그 목록을 항상 비워서 줌 — Source Bundle의
         fact_ref 추적성 검증이 애초에 성립할 수 없는 상태였고, 이 불일치는 비차단(non-blocking)
         경고로만 관측되어 겉으로는 정상 발행되는 것처럼 보였음.
    라벨 앞뒤의 "**"(0개 이상) 및 선택적 콜론을 허용하도록 정규식을 완화해 실제 모델 출력과
    일치시킨다. 값 추출 로직·라벨 목록은 변경하지 않는다.

    [결함수정-파이프라인정합성 #2] TF 키 정규화. 실제 운영 응답을 대조해보면 섹션 헤더가
    "🔹 1w"로만 나올 때도 있고 "🔹 1w (주봉)"처럼 한글 설명이 붙을 때도 있다(모델 출력의
    자연스러운 변동이며 스펙 위반은 아니다). 과거에는 헤더 줄 전체를 그대로 tf 키로 써서,
    "(주봉)"이 붙는 순간 registry 키가 "1w (주봉):현재가"가 되어 이 registry를 "1w","1d",
    "4h","1h" 같은 정규 코드로 조회하는 모든 소비처와 어긋났다 — build_phase2_evidence_slots
    (근거 슬롯 전부 공백 반환), validate_phase1_completeness(카드가 실제로는 꽉 차 있는데
    "카드 자체가 응답에 없음"으로 오탐), validate_phase1_current_price_consistency(비교 대상을
    못 찾아 검증 자체가 조용히 생략)까지 모델의 헤더 표기 방식에 따라 간헐적으로 무력화되는
    구조였다. 헤더 선두의 표준 TF 코드(숫자+단위, 예: 1w/4h/1d/15m)만 추출해 키로 쓰도록
    고정하고, 그 형태의 코드가 선두에 없는 예외적인 경우에만 기존처럼 헤더 원문 전체를
    그대로 쓴다(기존 동작보다 나빠지지 않도록 하는 안전망)."""
    registry = {}
    text = str(phase1_result or "")
    section_pattern = r"(?ms)^🔹\s*([^\n]+)\n(.*?)(?=^🔹\s*|\Z)"
    for section_match in re.finditer(section_pattern, text):
        tf_header_raw = section_match.group(1).strip()
        tf_code_match = re.match(r"(\d+[mhdwM])\b", tf_header_raw)
        tf = tf_code_match.group(1) if tf_code_match else tf_header_raw
        body = section_match.group(2)
        for label in PHASE1_FACT_LABELS:
            fact_match = re.search(rf"(?m)^\**{re.escape(label)}\**\s*:?\s*\n([^\n]+)", body)
            if not fact_match:
                continue
            fact_ref = f"{tf}:{label}"
...
def validate_phase1_completeness(phase1_result, custom_tfs, fact_registry=None):
    """[결함수정-파이프라인정합성] PHASE 1 응답이 스펙 "[카드 완전성 규정]"(요청된 모든 TF에
    15개 항목 전부 기재, 결손은 [데이터결손] 태그로 대체)을 실제로 충족하는지 확인한다.

    기존 코드는 call_gemini_api_with_retry가 '비어 있지 않은 텍스트'만 성공으로 간주하고,
    PHASE 2는 근거원장·가격경로·Bundle 형식을 엄격히 재검증하는 반면 PHASE 1 자체의 응답
    완결성(요청한 TF 카드가 전부 있는지, 카드당 15개 항목이 전부 있는지)은 아무도 검증하지
    않았다 — 토큰 한도 도달 등으로 카드가 중간에 잘려도 잘린 그대로 사용자에게 표시되고
    PHASE 2 입력으로 그대로 전달되는 비대칭이 있었다. 값의 옳고 그름은 판단하지 않고
    "카드/항목 자체가 존재하는지"만 최소한으로 확인해 절단·누락을 사용자와 로그에 드러낸다."""
    registry = fact_registry if fact_registry is not None else build_phase1_fact_registry(phase1_result)
    reasons = []
    for tf in custom_tfs or []:
        missing_labels = [label for label in PHASE1_FACT_LABELS if f"{tf}:{label}" not in registry]
        if not missing_labels:
            continue
        if len(missing_labels) == len(PHASE1_FACT_LABELS):
            reasons.append(f"{tf}: 카드 자체가 응답에 없음(누락 또는 응답 절단 가능성)")
        else:
            reasons.append(f"{tf}: {len(missing_labels)}/{len(PHASE1_FACT_LABELS)}개 항목 누락 ({', '.join(missing_labels)})")
    if not reasons:
        return []
    return ["PHASE1 카드 완결성 결손: " + " | ".join(reasons)]


CURRENT_PRICE_NUMBER_PATTERN = re.compile(r"[-+]?[0-9][0-9,]*(?:\.[0-9]+)?")
CURRENT_PRICE_TOLERANCE_RATIO = Decimal("0.05")  # 5% — TF 간 수집 시차·표시 반올림 허용치


def validate_phase1_current_price_consistency(fact_registry, last_close_by_tf):
    """[결함수정-파이프라인정합성] "현재가"는 TF와 무관하게 같은 STAGE 0 스냅샷의 동일 시점
    시장가를 가리켜야 한다. 그런데 카드의 "현재가" 문구는 Python이 사전계산하지 않고 모델이
    직접 작성하므로, 카드 작성 중 엉뚱한 TF의 다른 수치(예: 그 TF의 저가·중심가)를 잘못 옮겨
    적는 사례가 실사용에서 확인됐다(예: 1w 카드만 실제 종가와 무관한 값을 "현재가"로 표기,
    나머지 TF는 정상). Python이 각 TF 수집 시 이미 확보한 실제 종가(last_close_by_tf)와
    카드에 적힌 값을 비교해, 짧은 수집 시차·표시 반올림을 넘어서는 큰 격차만 경고한다. 값을
    고쳐 쓰지 않고 다른 검증 함수들과 동일하게 불일치만 로그로 남긴다."""
    warnings = []
    registry = fact_registry or {}
    for tf, actual_close in (last_close_by_tf or {}).items():
        item = registry.get(f"{tf}:현재가")
        if not isinstance(item, dict) or not item.get("value"):
            continue
        match = CURRENT_PRICE_NUMBER_PATTERN.search(item["value"])
        if not match:
            continue
        reported = _parse_decimal(match.group(0))
        actual = _parse_decimal(actual_close)
        if reported is None or actual is None or actual == 0:
            continue
        deviation_ratio = abs(reported - actual) / abs(actual)
        if deviation_ratio > CURRENT_PRICE_TOLERANCE_RATIO:
            warnings.append(
                f"{tf}: 카드에 표기된 현재가({item['value'].strip()})가 실제 수신 종가({actual})와 "
                f"{deviation_ratio * 100:.1f}% 차이 — 다른 TF의 수치를 잘못 옮겨 적었을 가능성"
            )
    return warnings
...
# ============================================================
# [2026-09 Cowork 재검토 정정] 이전에는 이 세 값이 하드코딩 리터럴이었다 — 값 자체는 2장
# SSOT("WALL_RANGE_PCT = 5.0" 등)와 일치했지만, COMPRESSION_RANGE_MAX가 겪었던 것과 동일한
# "스펙-코드 이중선언" 구조라 스펙만 갱신되고 코드가 누락되면 조용히 어긋날 위험이 있었다.
# DATA_MIN_COMPLETED_BARS 등과 동일하게 _spec_number()로 전환해 SSOT를 단일화한다.
WALL_RANGE_PCT = _spec_number("WALL_RANGE_PCT")
WALL_MAX_COUNT = _spec_number("WALL_MAX_COUNT", integer=True)
WALL_MIN_SIZE_RATIO = _spec_number("WALL_MIN_SIZE_RATIO")
OI_HISTORY_LIMIT = 30
...
def format_phase2_model_provenance(phase2_meta, phase1_meta):
    """PHASE 2 표시용 모델 안내.

    평소에는 format_model_provenance와 동일하게 단일 모델 줄만 보여준다. 다만
    [PHASE2 막다른 골목 예외](phase2_retry에서 승인 fallback이 전혀 없던 PHASE 1 성공
    모델 대신 roster 앞 순위 모델로 예외 처리된 경우)에는 PHASE 1 근거수집 모델과
    PHASE 2 해석 모델이 실제로 달라졌다는 사실을 숨기지 않고 그대로 명시한다."""
    phase2_meta = phase2_meta or {}
    if not phase2_meta.get("dead_end_reverse_fallback_used"):
        return format_model_provenance(phase2_meta)
    phase1_meta = phase1_meta or {}
    phase1_label = _friendly_model_name(
        phase1_meta.get("model_id") or _model_id_from_url(phase1_meta.get("model_url", ""))
    )
    phase2_label = _friendly_model_name(
        phase2_meta.get("model_id") or _model_id_from_url(phase2_meta.get("model_url", ""))
    )
    return (
        f"분석 모델: PHASE 1 {phase1_label}(근거수집) → PHASE 2 {phase2_label}(해석 · 막다른 골목 예외 fallback)\n"
        f"⚠️ PHASE 1 성공 모델은 승인 fallback이 없어 재시도까지 모두 실패했습니다. "
        f"이번 PHASE 2만 다른 승인 모델로 예외 처리되어, PHASE 1과 PHASE 2 해석 모델이 서로 다릅니다."
    )
...
def call_gemini_api_with_retry(full_prompt, max_tokens=16384, preferred_url=None, return_metadata=False,
                               ...
                               max_transport_attempts=2, allow_dead_end_reverse_fallback=False):
    headers = {
    ...
    payload = {
        ...
        "generationConfig": {
            ...
            # [결함수정-파이프라인정합성] 기존에는 답변 절단·사고과정 노출을 막으려 "low"(공식
            # 문서 기준 "지연·비용 최소화" 등급)로 강제했으나, 이 엔진은 4개 TF·15개 항목을
            # 동시에 종합해 81조합 등급·근거원장·가격경로까지 산출하는 다단계 추론 작업이라
            # 사고 예산을 지나치게 줄이면 PHASE 2 근거원장 검증 실패(검증보류)나 얕은 판단이
            # 늘어날 가능성이 높다. Gemini 3 Flash 계열의 공식 기본값은 "high"(가장 신중한
            # 추론)이지만, PHASE 1은 request_timeout_seconds=60의 빠른 fallback 구조에 맞춰져
            # 있어 "high"로 한 번에 올리면 지연이 길어져 불필요한 roster fallback이 늘어날 수
            # 있다. 정확도와 지연 사이의 절충으로 "medium"(공식 문서: "대부분 작업에 균형 잡힌
            # 사고")을 기본값으로 한다. 지연이 문제되지 않으면 "high"로 추가 상향을 검토할 것.
    ...
    if preferred_url:
        ...
        if allow_preferred_fallback:
            ...
            # [PHASE2 막다른 골목 예외] PHASE 1 성공 모델이 승인 roster의 마지막 순위라 전진
            # fallback이 하나도 없는 경우에만, 그리고 호출자가 명시적으로 열어준 경우에만
            # (run_phase2는 phase2_retry 재시도 호출에서만 이를 연다) roster 앞 순위 모델을
            # 최후 후보로 연다. 전진 fallback이 하나라도 있으면 발동하지 않으므로 평소 동작은
            # 그대로 유지된다.
            if (
                allow_dead_end_reverse_fallback
                and len(candidates) == 1
                and preferred_rank is not None
                and preferred_rank > 0
            ):
                for approved in roster[:preferred_rank]:
                    reverse_candidate = dict(approved)
                    reverse_candidate["selection_source"] = "PHASE 2 막다른 골목 예외 fallback"
                    reverse_candidate["dead_end_reverse_fallback"] = True
                ...
                if res.status_code == 200:
                    ...
                    if answer_text:
                        ...
                        metadata = {
                            ...
                            "dead_end_reverse_fallback_used": bool(candidate.get("dead_end_reverse_fallback")),
                        ...
                        # [2026-09 Cowork 재검토 — 판정65번] 판정62번으로 gemini-3.5-flash가
                        # 3번째 긴급 fallback으로 공식 승인됐지만, 근본원인(3.7·3.6 할당량
                        # 제한)은 아직 해결되지 않은 임시조치다. 실제로 이 3번째 단계까지
                        # 얼마나 자주 도달하는지 알아야 근본원인 해결의 시급성을 판단할 수
                        # 있는데, 지금까지는 그 빈도를 추적할 방법이 없었다. 판정51번의 캐시
                        # 히트율 로그와 동일한 원칙(관측 전용, 신규 API 호출·과금·분기 없음)
                        # 으로 greppable 로그 한 줄만 추가한다.
                        if metadata["model_id"] == "gemini-3.5-flash":
                            print(f"[MODEL_FALLBACK_TIER3] 근본원인(할당량 제한) 미해결로 인한 "
                                  f"긴급 fallback 발동: selection_source={metadata['selection_source']}")
...
def run_phase1(symbol_input, exchange_name, custom_tfs):
    ...
    try:
        ...
        tf_quality_list = []
        snapshot_events = []
        major_swing_pivots = {}  # [Cowork 재검토 — 판정67번] OB Tier1 검증용, 1w·1d만 채움
        ...
        for i, tf in enumerate(custom_tfs):
            ...
            if len(recent) > 0:
                ...
                # [결함수정-파이프라인정합성] TF별 실제 수신 종가를 보존한다. "현재가"는 모든 TF
                # 카드에서 동일 시점의 시장가를 가리켜야 하는데, 모델이 카드 작성 중 엉뚱한 TF의
                # 수치(예: 그 TF의 저가)를 잘못 옮겨 쓰는 사례가 실사용에서 확인됐다(아래
                # validate_phase1_current_price_consistency 참조).
                last_close_by_tf[tf] = last_close
            ...
            # [신규기능-A안 실효성보강] Plugin 10: 세력 평단 추정치(VWAP) — TF마다 개별 계산
            # (PHASE1 카드15개와는 별개, Volume Delta와 동일한 서술 전용 플러그인 패턴)
            # [실효성보강-VWAP동적창] 1w·1d만 동적 관측창 판별(determine_vwap_dynamic_lookback)을
            # 적용한다 — CandleView_API.txt Plugin 10 서술 통합 범위(1w·1d 한정)와 동일 기준.
            # 4h·1h는 기존 고정 VWAP_LOOKBACK_BARS(20봉)를 그대로 쓴다(narrative 통합 대상이
            # 아니라 창을 정교화할 실익이 없고, 변경 범위를 최소화하기 위함).
            if tf in ("1w", "1d"):
                vwap_lookback = determine_vwap_dynamic_lookback(recent) or VWAP_LOOKBACK_BARS
                # [2026-09 Cowork 재검토 — 판정67번 OB Tier1 검증] determine_vwap_dynamic_lookback와
                # 완전히 동일한 산식(_find_pivots+_filter_major_price_pivots, 진행봉 제외)을 독립
                # 재호출해 가격 자체만 별도로 확보한다. 기존 함수의 반환 계약(lookback 정수만 반환)을
                # 바꾸면 VWAP 경로에 회귀 위험이 생기므로, 계산을 공유하지 않고 의도적으로 중복
                # 호출한다(순수함수·저비용이라 성능 영향 무시 가능). 세력 매물대(OB) 자체를
                # 재계산하는 것이 아니라, PHASE 2가 보고할 OB 구간이 실제 주요 스윙 피벗과 완전히
                # 무관한 값(환각 의심)인지만 사후 대조하는 참고용 데이터다.
                swing_work = recent.iloc[:-1].reset_index(drop=True)
                if len(swing_work) >= 5:
                    sw_high_arr = swing_work["high"].values
                    sw_low_arr = swing_work["low"].values
                    sw_highs_raw, sw_lows_raw = _find_pivots(sw_high_arr, sw_low_arr, left=2, right=2)
                    sw_highs_px, sw_lows_px = _filter_major_price_pivots(sw_highs_raw, sw_lows_raw, sw_high_arr, sw_low_arr)
                    major_swing_pivots[tf] = {
                        "highs": [float(sw_high_arr[i]) for i in sw_highs_px],
                        "lows": [float(sw_low_arr[i]) for i in sw_lows_px],
                    }
            else:
                vwap_lookback = VWAP_LOOKBACK_BARS
            tf_vwap = calculate_rolling_vwap(recent, lookback=vwap_lookback)
            payload += f"\n[{tf} 세력 평단 추정치 (Plugin 10)]\n"
            if tf_vwap is not None:
                payload += f"최근 {vwap_lookback}봉 거래량가중평균가(VWAP): {tf_vwap:.8g}\n"
            else:
                payload += "[데이터결손] VWAP 산출 불가 (거래량 데이터 없음)\n"

            # [신규기능-A안 실효성보강] Plugin 11: RSI 모멘텀 기울기 — TF마다 개별 계산
            rsi_delta, rsi_state = calculate_rsi_momentum_slope(recent)
            payload += f"\n[{tf} 모멘텀 기울기 (Plugin 11)]\n"
            if rsi_delta is not None:
                payload += f"RSI {RSI_MOMENTUM_LOOKBACK_BARS}봉 변화량: {rsi_delta:+.2f}p | 상태: {rsi_state}\n"
            else:
                payload += "[데이터결손] 모멘텀 기울기 산출 불가 (봉수 부족)\n"
        ...
        phase1_canonical = build_phase1_canonical(
            ...
            {"volume_delta": tf_delta_list, "oi": oi_info, "whale_wall": wall_info, "major_swing_pivots": major_swing_pivots},
        )
        ...
        # [결함수정-파이프라인정합성] AI 해석 자체가 보류된 경우(원천 STAGE0 덤프만 표시)는
        # 카드 형식이 아니므로 완결성 검사 대상이 아니다. 정상 응답에서만 카드 절단·누락을 검사한다.
        completeness_warnings = (
            [] if phase1_model_meta.get("failed")
            else validate_phase1_completeness(phase1_result, custom_tfs, fact_registry=phase1_fact_registry)
                 + validate_phase1_current_price_consistency(phase1_fact_registry, last_close_by_tf)
        )
        if completeness_warnings:
            phase1_result += "\n\n[자동검증 로그 — Python 사후검증]\n" + "\n".join(f"• {w}" for w in completeness_warnings)
            # 경고가 반영된 최종 표시본을 기준으로 registry·지문을 다시 확정해, 표시본과 지문이 어긋나지 않게 한다.
            phase1_fact_registry = build_phase1_fact_registry(phase1_result)
        supplement["phase1_fact_registry"] = phase1_fact_registry
        supplement["phase1_fact_registry_warnings"] = validate_phase1_fact_registry(phase1_fact_registry)
...
def _load_core_score_cap_from_spec():
    ...
    [Cowork 재검토 판정58번] _spec_number()와 동일하게 판정이력 대장을 제외한
    CANDLEVIEW_SPEC_NUMBERS_SOURCE에서 검색 — 오매칭 위험 동일 원인 동일 해법."""
    match = re.search(r"CORE_SCORE_CAP\s*=\s*±\s*([0-9]+(?:\.[0-9]+)?)", CANDLEVIEW_SPEC_NUMBERS_SOURCE)
...
# [2026-09 Cowork 재검토 — 판정66번] 10장[2] "정량 산정 등급 기준" 프로즈 수치를 그대로 옮긴
# 리터럴이다. RSI_MOMENTUM_FLAT_THRESHOLD와 같은 선례: 이 두 값은 스펙 내 단일 서술(10장[2])
# 외에 다른 곳에 이중선언되어 있지 않으므로(WALL_*·COMPRESSION_RANGE_MAX와 달리 코드-스펙
# 이중선언 위험이 없음) _spec_number() SSOT 추출까지는 하지 않는다. 10장[2] 문구가 바뀌면
# 이 두 값도 함께 갱신해야 한다.
CONFIDENCE_GRADE_HIGH_MIN = Decimal("5.0")  # 10장[2]: 이 값 이상 → '상'
CONFIDENCE_GRADE_MID_MIN = Decimal("2.5")   # 10장[2]: 이 값 이상 ~ HIGH_MIN 미만 → '중', 미만 → '하'


def _confidence_grade_from_score(score):
    """10장[2] 정량 산정 등급 기준(5.0 이상 상 / 2.5~5.0 중 / 2.5 미만 하)을 그대로 재현한다."""
    if score >= CONFIDENCE_GRADE_HIGH_MIN:
        return "상"
    if score >= CONFIDENCE_GRADE_MID_MIN:
        return "중"
    return "하"
...
def _expected_final_grade(scores, net_direction, adjustment):
    """[내부 판정값] 9장③ 스펙 원문: "다수결 주도 방향이 D_final과 다르거나... D_final=횡보이면
    G_final=약함으로 하향한다. 단 D_final=횡보의 사용자 표기는 기존대로 '횡보 우세(우세 등급:
    횡보)'로 고정한다." — 즉 내부 판정값(G_final)과 사용자 표시 문구는 D_final=횡보일 때 서로
    다른 값을 가지도록 명시적으로 이원화되어 있다. 이 함수는 내부 판정값만 반환한다(내부 원장
    "등급" 필드·Notion 추적 기록용). 표시 문구는 _expected_display_grade()가 별도로 정한다.

    [2026-09 Cowork 재검토 — 판정70번, 결함수정] 종전 버전은 net_direction=="횡보"일 때 이
    함수가 "횡보"를 그대로 반환했고, 그 값이 내부 원장 "등급" 필드와 사용자 표시 문구 양쪽에
    동일하게 쓰였다. 그 결과 4대축 다수결과 순합부호가 실제로 충돌해 D_final이 횡보로 떨어진
    "신호충돌형 횡보" 사례가, Notion 추적 DB에도 "횡보"(순수 관망장과 구분 불가)로만 남아
    스펙이 보존하려던 "약함"(충돌 발생) 정보가 소실되고 있었다. 아래로 정정한다."""
    majority_direction, candidate_grade = _candidate_grade_from_scores(scores)
    if net_direction == "횡보":
        # 81조합 자체가 횡보 후보(진짜 합의된 관망)이든, 다수결과 순합부호가 어긋나 D_final만
        # 횡보로 떨어진 경우(신호충돌)이든, 스펙 9장③ 규칙2 원문은 두 경우를 구분하지 않고
        # 내부 판정값을 "약함"으로 통일한다 — 사용자 표시만 별도로 "횡보"로 고정된다.
        return "약함"
...
def _expected_display_grade(net_direction, internal_grade):
    """[사용자 표시값] D_final=횡보이면 내부 판정값(약함)과 무관하게 항상 '횡보'로 고정 표시한다
    (9장③ 규칙2 단서, 판정70번). 그 외에는 내부 판정값을 그대로 표시한다."""
    return "횡보" if net_direction == "횡보" else internal_grade


def verify_and_strip_evidence_ledger(text, data_deficient_tfs=None):
    """내부 근거원장의 구조적 완전성만 검사하고 사용자 출력에서는 제거한다.

    data_deficient_tfs: [2026-09 Cowork 재검토 — 판정66번] STAGE0에서 이미 tf_quality_list로
    보유 중인 "데이터결손/판정불가" TF 코드 목록(선택). 10장[3] "결손 데이터 제한: [데이터결손]
    발생 시 해당 TF 신뢰도는 최대 '중'으로 하향 제한한다" 규정은 S_1~S_4 축점수·순합방향·확률%처럼
    이미 자동 대조되는 다른 규정과 달리 PHASE 2 출력과 대조하는 Silent Verification이 없었다
    (63번 항목에서 "발견 — 미조치, 결함 아님"으로 기록, 개발자 승인 후 이번에 조치). 코어 점수·
    등급 자체를 재계산·변경하지 않고, 경고만 추가하는 기존 패턴을 그대로 따른다.
    """
    warnings = []
    ...
    if data_deficient_tfs:
        confidence_score = _parse_decimal(_extract_ledger_value(ledger, "최종신뢰도점수"))
        if confidence_score is not None and confidence_score.is_finite() and _confidence_grade_from_score(confidence_score) == "상":
            warnings.append(
                "10장[3] 결손 데이터 제한 위반 의심: 데이터결손 TF(" + ", ".join(sorted(data_deficient_tfs)) +
                f")가 있음에도 최종신뢰도점수={confidence_score}로 '상' 등급 기준(5.0 이상)을 충족함 "
                "(규정: 데이터결손 TF 존재 시 신뢰도 최대 '중'으로 하향 제한)"
            )
...
# ============================================================
def _strip_duplicate_supply_block(text):
    """[2026-09 Cowork 재검토 — 판정71번] 12장 V002-15 "파이프라인 폐쇄 검증": PLUGIN_DELTA/OI/
    WALL_ENABLE가 켜져 있어도 PHASE 2 본문에 [API 수급 보조 관측] 형태의 별도 고정 블록이
    존재하면 안 된다(11장[4]항 main.py 보간지표 버튼과 중복 금지). run_phase2 프롬프트가 이미
    이 블록을 만들지 말라고 명시 지시하지만(근본원인 차단, 아래 compute_supply_gate_facts와
    한 세트), 지시 위반 시를 대비한 결정론적 안전망이다 — 판단이 아니라 발견 즉시 제거하는
    삭제이므로 OB Tier1처럼 '경고만 남기는' 사후판정 패턴과는 다르다."""
    lines = (text or "").split("\n")
    heading_re = re.compile(r'API\s*수급\s*보조\s*관측')
    section_marker_re = re.compile(r'^\s*[1-6]️⃣')
    out = []
    i = 0
    while i < len(lines):
        if heading_re.search(lines[i]):
            i += 1
            while i < len(lines) and lines[i].strip() != "" and not section_marker_re.match(lines[i]):
                i += 1
            continue
        out.append(lines[i])
        i += 1
...
def compute_supply_gate_facts(phase1_canonical, major_swing_pivots=None):
    """[2026-09 Cowork 재검토 — 판정71번] 12장 V002-15 Layer 5-B 수치 검증 규칙(Delta_Ratio
    범위, OI 변동률 범위, Whale Wall 구조매칭)을 PHASE2 생성 '이전에' Python이 기존
    phase1_canonical.plugins 원시값(신규 API 호출·신규 계산 없음, 이미 build_phase1_canonical이
    보유)으로 사전판정해 프롬프트에 사실로 주입한다. OB Tier1(판정67~69)이 택했던
    '사후 정규식 추출→경고'는 Gemini가 이미 틀리게 쓴 문장을 사후에 걸러내는 방식이라
    증상관리에 가깝다 — 여기서는 Gemini가 범위 밖 주장이나 무자격 Whale Wall을 애초에
    서술하지 못하도록 근본에서 차단한다(run_phase2 프롬프트 삽입과 한 세트, 아래 참조)."""
    plugins = (phase1_canonical or {}).get("plugins") or {}
    pivots = major_swing_pivots or {}

    oi_info = plugins.get("oi") or {}
    oi_anomaly, oi_detail = False, None
    if oi_info.get("status") == "ok":
        candidates = list(oi_info.get("change_pcts") or [])
        if oi_info.get("latest_change_pct") is not None:
            candidates.append(oi_info["latest_change_pct"])
        for pct in candidates:
            if pct is not None and abs(pct) > 50.0:
                oi_anomaly, oi_detail = True, pct
                break

    delta_anomaly_tfs = []
    for item in (plugins.get("volume_delta") or []):
        info = (item or {}).get("info") or {}
        ratio = info.get("last_delta_ratio")
        if info.get("status") == "ok" and ratio is not None and abs(ratio) > 1.0:
            delta_anomaly_tfs.append(item.get("tf") or "?")

    all_pivot_prices = []
    for tf_data in pivots.values():
        all_pivot_prices.extend((tf_data or {}).get("highs") or [])
        all_pivot_prices.extend((tf_data or {}).get("lows") or [])
    proximity_ratio = (REF_PROXIMITY_LIMIT_PCT or 0.0) / 100.0

    def _structurally_matched(price):
        # 피벗 데이터 자체가 없으면(1w/1d 스윙 계산 실패 등) 판정불가 — 임의로 자격을
        # 박탈하지 않는다(None). OB Tier1과 동일한 "판단 근거 없으면 관대하게" 원칙.
        if not all_pivot_prices or REF_PROXIMITY_LIMIT_PCT is None:
            return None
        return any(
            p * (1 - proximity_ratio) <= price <= p * (1 + proximity_ratio)
            for p in all_pivot_prices
        )

    wall_info = plugins.get("whale_wall") or {}
    qualified_walls, unmatched_walls = [], []
    for side_key, side_label in (("bid_walls", "매수벽"), ("ask_walls", "매도벽")):
        for wall in (wall_info.get(side_key) or []):
            price = wall.get("price")
            if price is None:
                continue
            matched = _structurally_matched(price)
            entry = f"{side_label} {price}"
            if matched is True:
                qualified_walls.append(entry)
            elif matched is False:
                unmatched_walls.append(entry)
            # matched is None(판정불가)은 두 목록 어디에도 넣지 않는다 — 임의 배제 금지.

    return {
        "oi_change_anomaly": oi_anomaly,
        "oi_change_anomaly_detail": f"{oi_detail:+.2f}%" if oi_detail is not None else "",
        "delta_ratio_anomaly_tfs": delta_anomaly_tfs,
        "whale_wall_structurally_qualified": qualified_walls,
        "whale_wall_structurally_unmatched": unmatched_walls,
    }


def verify_and_fix_phase2(text, data_deficient_tfs=None, major_swing_pivots=None):
    fixed, warnings = verify_and_strip_evidence_ledger(text, data_deficient_tfs=data_deficient_tfs)
    fixed = _strip_duplicate_supply_block(fixed)
    ...
    # [2026-09 Cowork 재검토 — 판정67번] OB(세력 매물대) Tier1 최소 타당성 검증 — 판정54번에서
    # "OB 자체 Python화는 보류, VWAP 교차검증만 보강"으로 범위를 좁혀둔 항목의 후속 조치(개발자
    # 승인). OB의 실제 정의(13장/8장: BOS/CHoCH 직전 마지막 반대방향 캔들의 가격 범위)는 구조
    # 판정이 선행돼야 해서 Python이 완전히 재현할 수 없다 — 이 검사는 OB 판정 자체의 정확성을
    # 검증하지 않는다(Tier2, 보류 상태 그대로). 대신 PHASE1이 이미 확보한 1w·1d 실측 주요 스윙
    # 피벗 가격(major_swing_pivots, 다이버전스·VWAP 동적창과 동일한 _find_pivots+
    # _filter_major_price_pivots 산식)과 PHASE2가 보고한 OB 구간을 대조해, 어떤
    # 실제 주요 스윙 피벗과도 REF_PROXIMITY_LIMIT(±1.0%, 2장 SSOT) 이내로 근접하지 않으면
    # "실제 시장 구조와 무관해 보이는 값(환각 의심)"이라는 최소 타당성 경고만 남긴다. 코어
    # 점수·등급·표시 문구는 전혀 바꾸지 않는다(기존 Silent Verification과 동일 원칙).
    # [2026-09 Cowork 재검토 — 판정68번] 최초 실전 테스트(UPBIT LA/KRW)에서 이 검사가 0건
    # 매치로 조용히 무동작했다 — 원인은 "[OB] (하단~상단)" 완전 괄호 표기를 가정한 정규식인데,
    # 실사용 Gemini 출력은 "세력 매물대 [OB]는 67.0 ~ 75.6 KRW에" 또는 "[OB]" 태그 자체를
    # 생략한 "세력 매물대는 77.4 ~ 84.5 KRW에" 형태로도 나온다는 것. 두 실제 사례 모두 괄호가
    # 없어 기존 패턴이 전혀 매치하지 않았다(재현 스크립트로 확인). 서술 문장 자체를 강제하지
    # 않고(자연어 서술 허용 원칙 유지) "세력 매물대" 또는 "[OB]" 앵커 뒤 20자 이내에 등장하는
    # 숫자~숫자 구간을 잡아내도록 완화했다 — 판정 자체를 바꾸지 않고 파싱 관용도만 넓힌 것.
    if major_swing_pivots and REF_PROXIMITY_LIMIT_PCT is not None:
        tf_section_pattern = re.compile(
            r"(?ms)^\s*▶️\s*(?P<tf>1w|1d|4h|1h|15m|5m)\s*타임\s*프레임\s*분석\s*\n(?P<body>.*?)"
            r"(?=^\s*▶️\s*(?:1w|1d|4h|1h|15m|5m)\s*타임\s*프레임\s*분석|^\s*5️⃣|\Z)"
        )
        proximity_ratio = REF_PROXIMITY_LIMIT_PCT / 100.0
        for tf_match in tf_section_pattern.finditer(fixed):
            pivots = major_swing_pivots.get(tf_match.group("tf")) or {}
            candidate_prices = list(pivots.get("highs") or []) + list(pivots.get("lows") or [])
            if not candidate_prices:
                continue
            seen_ob_ranges = set()
            for ob_match in re.finditer(
                r'(?:세력\s*매물대|\[OB\])[^0-9\n]{0,20}([\d,]+(?:\.\d+)?)'
                r'\s*(?:KRW|원|\$|USD|USDT)?\s*~\s*([\d,]+(?:\.\d+)?)',
                tf_match.group("body"),
            ):
                try:
                    ob_lo, ob_hi = (float(g.replace(",", "")) for g in ob_match.groups())
                except ValueError:
                    continue
                if (ob_lo, ob_hi) in seen_ob_ranges:
                    continue
                seen_ob_ranges.add((ob_lo, ob_hi))
                if ob_lo > ob_hi:
                    ob_lo, ob_hi = ob_hi, ob_lo
                near = any(
                    price * (1 - proximity_ratio) <= ob_hi and ob_lo <= price * (1 + proximity_ratio)
                    for price in candidate_prices
                )
                if not near:
                    warnings.append(
                        f"OB Tier1 타당성 검증 실패({tf_match.group('tf')}): 보고된 세력 매물대 "
                        f"{ob_lo}~{ob_hi}가 실측 주요 스윙 피벗 {len(candidate_prices)}개 중 어느 것과도 "
                        f"REF_PROXIMITY_LIMIT(±{REF_PROXIMITY_LIMIT_PCT}%) 이내로 근접하지 않음 — "
                        "환각 의심(참고용, OB 판정 자체의 정확성 검증은 아님)"
                    )
...
def classify_phase2_verification_warnings(warnings, structured=False):
    ...
    for warning in warnings or []:
        ...
        if "OB(" in text or "상호중첩" in text:
            rule_ids.add("P2V05")
        # [2026-09 Cowork 재검토 — 판정66번] 10장[3] 결손 데이터 신뢰도 상한 검증 전용 코드.
        if "10장[3] 결손 데이터 제한" in text:
            rule_ids.add("P2V06")
        # [2026-09 Cowork 재검토 — 판정67번] OB Tier1 최소 타당성 검증 전용 코드.
        if "OB Tier1 타당성 검증" in text:
            rule_ids.add("P2V07")
        if not any(rule_id in rule_ids for rule_id in ("P2V01", "P2V02", "P2V03", "P2V04", "P2V05", "P2V06", "P2V07")):
            rule_ids.add("P2V99")
...
def phase2_validation_subcodes(warnings):
    ...
    for warning in warnings or []:
        ...
        if "10장[3] 결손 데이터 제한" in text:
            subcodes.add("P2V06_CONFIDENCE_GRADE_CAP")
        if "OB Tier1 타당성 검증" in text:
            subcodes.add("P2V07_OB_TIER1_IMPLAUSIBLE")
...
def normalize_evidence_ledger_for_publication(text):
    ...
    # [판정70번] internal_grade = 내부 원장 "등급" 필드(Notion 추적 기록용, D_final=횡보일 때도
    # 신호충돌 여부를 "약함"으로 보존). display_grade = 사용자 표시 문구 전용(D_final=횡보이면
    # 항상 "횡보" 고정). 종전에는 이 두 용도에 동일 변수(expected_grade)를 그대로 재사용해
    # D_final=횡보 사례의 내부 판정값이 표시문구와 함께 "횡보"로 붕괴되고 있었다.
    internal_grade = _expected_final_grade(scores, net_direction, adjustment) if adjustment in ("없음", "대형임펄스반전") else None
    display_grade = _expected_display_grade(net_direction, internal_grade) if internal_grade is not None else None
    ...
    if internal_grade is not None:
        replacements["등급"] = internal_grade
    elif visible:
    ...
    if visible and display_grade is not None:
        expected_decision = f"{net_direction} 우세 (우세 등급: {display_grade})"
...
def extract_ledger_tracking_fields(text):
    """[PHASE2 실전 결과 추적용 — 2026-09 신설] 등급·최종신뢰도점수 원문을 읽기 전용으로 뽑는다.
    build_phase2_display_contract와 같은 ledger 블록을 다시 정규식으로만 읽을 뿐, 기존 검증·발행
    로직에는 관여하지 않는다(무간섭 확장)."""
    match = re.search(r"\[INTERNAL_EVIDENCE_LEDGER\](.*?)\[/INTERNAL_EVIDENCE_LEDGER\]", text or "", re.DOTALL)
    if not match:
        return {}
    ledger = match.group(1)
    return {
        "grade": _extract_ledger_value(ledger, "등급"),
        "final_confidence_score": _extract_ledger_value(ledger, "최종신뢰도점수"),
    }
...
ALT_PATH_SECTION_PATTERN = (
    r"(?ms)^(?P<header>[^\n]*📉\s*대체 시나리오 파동 경로[^\n]*)\n"
    # [결함수정-파이프라인정합성 #9] "예상 소요기간" 섹션을 엔진 스펙(CandleView_API.txt)에서
    # 통째로 삭제했으므로(정성적 3단계 봉수 추정치가 실제로는 근거 2개짜리 룩업일 뿐이고
    # 백테스트 미검증이라는 자체 고지까지 있어, 정밀해 보이는 시간 표기가 오히려 오도 소지가
    # 크다는 판단 — 사용자 승인), 그 마커를 찾던 대체 경로("|2️⃣")도 함께 걷어낸다. 2️⃣는
    # 어차피 항상 존재하므로 대체 경로 본문의 경계 인식 자체는 그대로 유지된다.
    r"(?P<body>.*?)(?=^[^\n]*2️⃣[^\n]*|\Z)"
)
...
def _is_empty_route_context_label(line):
    ...
    [실사용 관측 결함수정] "무효화 손절선"은 대체 시나리오에서 Python이 이미
    "검증된 무효화선 (수치) 이탈"로 렌더링한 값과 의미가 겹치는, 4개 허용 라벨
    (FVG·Role Reversal·돌파선·다음 매물대) 밖의 라벨이다. 기존 정규식은 모델이
    "무효화 손절선 이탈"이라고 쓴 경우만 걸러냈는데, 실제로는 "이탈" 없이
    "무효화 손절선"만 단독으로 오는 경우가 관측되어(값도 TF도 없는 채 그대로
    발행됨) "이탈"을 선택적으로 바꾼다. 값이 실제로 붙은 변형(예: "무효화 손절선
    50.3 KRW")은 숫자를 포함하므로 _is_unverified_decision_value_line이 별도로
    걸러낸다."""
    return bool(re.fullmatch(
...
def try_deterministic_phase2_repair(text, warnings):
    ...
    if len(re.findall(decision_pattern, briefing)) != 1:
        return None, metadata
    net_direction = _direction_from_net(sum(scores.values()))
    # [판정70번] internal_grade(원장 "등급" 필드) vs display_grade(사용자 표시) 이원화 —
    # 위 normalize_evidence_ledger_for_publication과 동일한 근거·동일한 분리 원칙 적용.
    internal_grade = _expected_final_grade(scores, net_direction, adjustment)
    display_grade = _expected_display_grade(net_direction, internal_grade)
    axis_sets = _derived_axis_sets(scores, net_direction)
    repaired_ledger = ledger
    replacements = {
        "결론": net_direction, "등급": internal_grade, "순합방향": net_direction,
        "지지축": ", ".join(axis_sets["지지축"]) if axis_sets["지지축"] else "없음",
        "반대축": ", ".join(axis_sets["반대축"]) if axis_sets["반대축"] else "없음",
        "상충축": ", ".join(axis_sets["상충축"]) if axis_sets["상충축"] else "없음",
        "중립축": ", ".join(axis_sets["중립축"]) if axis_sets["중립축"] else "없음",
    ...
    repaired_briefing = re.sub(decision_pattern, f"{net_direction} 우세 (우세 등급: {display_grade})", briefing, count=1)
...
def run_phase2(phase1_result, symbol, exchange_name, phase1_canonical=None, phase1_model=None,
    ...
    # [2026-09 Cowork 재검토 — 판정66번] STAGE0이 이미 확보한 TF별 데이터결손 여부(tf_quality_list)를
    # 10장[3] Silent Verification에 넘긴다. attempt 루프 내내 phase1_canonical은 불변이므로 루프 밖에서 1회만 계산한다.
    data_deficient_tfs = sorted({
        str(item.get("tf", ""))
        for item in (phase1_canonical or {}).get("data_quality", {}).get("timeframes", [])
        if item.get("status") == "데이터결손/판정불가" and item.get("tf")
    })
    # [2026-09 Cowork 재검토 — 판정67번] PHASE1이 이미 확보한 1w·1d 실측 주요 스윙 피벗 가격을
    # OB Tier1 Silent Verification에 넘긴다. 마찬가지로 attempt 루프 밖에서 1회만 조회한다.
    major_swing_pivots = (phase1_canonical or {}).get("plugins", {}).get("major_swing_pivots") or {}
    # [2026-09 Cowork 재검토 — 판정71번] Layer 5-B(Delta_Ratio/OI 변동률/Whale Wall 구조매칭)
    # 자격판정을 PHASE2 생성 이전에 확정해 프롬프트에 사실로 주입한다(근본원인 접근).
    supply_gate_facts = compute_supply_gate_facts(phase1_canonical, major_swing_pivots)
    ...
    base_prompt = (
        ...
        f"[PHASE 2 수급 플러그인 자격 판정 — PHASE 1 원시데이터로 Python이 사전판정, 판정71]\n{json.dumps(supply_gate_facts, ensure_ascii=False, sort_keys=True)}\n\n"
        ...
        f"[수급 플러그인 자격 판정 준수 — 판정71] oi_change_anomaly가 true이면 해당 OI 변동은 [데이터이상]으로만 명시하고 방향·가격 근거로 사용하지 마십시오. delta_ratio_anomaly_tfs에 열거된 TF의 Volume Delta도 동일하게 [데이터이상]으로만 언급하고 방향 근거로 쓰지 마십시오. whale_wall_structurally_unmatched에 열거된 벽은 실측 주요 구조와 근접하지 않으므로 지지·저항 근거로 언급하지 말고, whale_wall_structurally_qualified에 열거된 벽만 인용하십시오. ‘API 수급 보조 관측’이라는 이름의 별도 고정 블록·소제목을 새로 만들지 말고, 수급 근거는 반드시 1️⃣ 또는 개별 TF·상관관계 서술에 자연스럽게 결속하십시오. "
    ...
    for attempt in range(2):
        ...
        raw_json, response_meta = call_gemini_api_with_retry(
            ...
            # [PHASE2 막다른 골목 예외] phase2_run 1차 실행에는 영향 없음. PHASE 1 성공 모델
            # 외에 승인 fallback이 전혀 없던 세션이 phase2_retry(동일 세션 1회 재시도)까지
            # 도달했을 때만, 그 재시도에서 roster 앞 순위 모델을 최후 후보로 허용한다.
            allow_dead_end_reverse_fallback=(trigger_action == "phase2_retry"),
        )
        ...
        last_verified = verify_and_fix_phase2(
            normalized_result, data_deficient_tfs=data_deficient_tfs, major_swing_pivots=major_swing_pivots,
        )
        ...
        # [PHASE2 실전 결과 추적용 — 2026-09 신설, 무간섭 확장] 여기까지 왔다는 건 display_contract가
        # 이미 위 검증(축점수·가격순서·확률식)을 전부 통과했다는 뜻이다 — 검증 로직을 새로 만들지
        #않고 이미 검증된 값만 읽어 side-channel(phase2_execution_metadata)에 얹는다. 횡보는 채점할
        # 가격 목표가 없어 이번 버전 추적 대상에서 제외한다(스코프 결정, 필요시 확장 가능).
        if phase2_execution_metadata is not None and display_contract and display_contract.get("direction") != "횡보":
            tracking_fields = extract_ledger_tracking_fields(normalized_result)
            prices = display_contract.get("prices") or {}
            probabilities = display_contract.get("probabilities") or {}
            phase2_execution_metadata["tracking_payload"] = {
                "symbol": symbol,
                "exchange": exchange_name,
                "direction": display_contract["direction"],
                "grade": tracking_fields.get("grade"),
                "final_confidence_score": tracking_fields.get("final_confidence_score"),
                "main_probability_pct": probabilities.get("main"),
                "entry": prices.get("P_entry"),
                "invalidation": prices.get("P_inv"),
                "target_1": prices.get("P_target_1"),
                "target_2": prices.get("P_target_2"),
                "session_id_hash": _phase2_session_hash(session_id),
            }
        observe("PUBLISHED", attempt=attempt + 1)
...
# ============================================================
# [결함수정-파이프라인정합성] 정식 모드 보조 지표(Zone/Kz/Kp_mult) 사후검증
# 근거: Layer 0 스펙의 Zone→Kz 매핑(대칭 Zone 1/2/3은 고정값, Zone 0/4는 부호연동 2값 중 하나),
# 경계 완충(±2%p) 여부, Kp_mult 5단계 이산값, "원본×Kz×Kp_mult=보정" 산식은 전부 결정론적
# 조견표·산술이라 원칙적으로 계산이 틀릴 수 없는 영역인데도, 이 블록은 근거원장·가격경로·카드
# 완결성과 달리 지금까지 Python 사후검증이 전혀 없어 모델 산출값이 그대로 발행되고 있었다.
# 실사용 예시(UPBIT FLOCK/KRW, 4h: Pos%=57.6%→ZONE_ID=2인데 Kz=1.20 적용)로 실제 오류를
# 직접 확인했다 — 스펙은 "Zone 2 Kz는 예외 없이 항상 1.30을 사용한다"고 명시한다(위 사례는
# 완충 보간 대상도 아니었다: 57.6%는 가장 가까운 경계 60%에서 2.4%p 떨어져 있어 ±2%p 완충
# 구간 밖이다). 값을 고쳐 쓰지 않고 다른 검증 함수들과 동일하게 불일치만 경고로 남긴다.
# ============================================================
FRACTAL_ZONE_RANGES = {
    0: (Decimal("0"), Decimal("20")), 1: (Decimal("20"), Decimal("40")),
    2: (Decimal("40"), Decimal("60")), 3: (Decimal("60"), Decimal("80")),
    4: (Decimal("80"), Decimal("100")),
}
FRACTAL_ZONE_KZ_SYMMETRIC = {1: Decimal("1.20"), 2: Decimal("1.30"), 3: Decimal("1.10")}
FRACTAL_ZONE_KZ_SIGN_DEPENDENT = {0: {Decimal("0.50"), Decimal("1.50")}, 4: {Decimal("0.50"), Decimal("1.50")}}
FRACTAL_KZ_CLAMP = (Decimal("0.3"), Decimal("1.8"))
FRACTAL_KP_MULT_VALID = {Decimal(v) for v in ("1.00", "1.08", "1.16", "1.25", "1.35")}
FRACTAL_ZONE_BUFFER_MARGIN = Decimal("2.0")
FRACTAL_ZONE_BOUNDARIES = (Decimal("20"), Decimal("40"), Decimal("60"), Decimal("80"))
FRACTAL_CORRECTION_TOLERANCE = Decimal("0.015")  # 2자리 반올림 표기 오차 허용

# [결함수정-파이프라인정합성 #3] TF 블록 분리 후 필드별 개별 매칭으로 변경.
# 기존의 단일 DOTALL 정규식은 TF 헤더~위치~계수~점수를 ".*?"로 이어붙여 한 번에 매칭했는데,
# 실사용 응답(BITHUMB HEMI/KRW)으로 직접 재현해보니 다음 두 조건이 겹치면 다른 TF의 수치를
# 가져와 엉뚱한 경고를 만들어냈다:
#   1) 숫자 캡처 그룹이 "-?[0-9]+"만 허용해 양수를 "+0.48"처럼 부호를 붙여 표기하면 그 자리에서
#      매치가 실패한다.
#   2) 매치 실패 시 비탐욕 ".*?"가 실패한 지점을 건너뛰고 문서 뒤쪽에서 같은 리터럴("▸ 계산
#      보정 점수" 등)이 다시 나올 때까지 계속 전진하는데, 그 지점은 대개 "다음 TF" 블록이다.
# 그 결과 실제 사례에서 "1w" 라벨을 달고 있지만 실제로는 Kz/Kp_mult는 1d 값(1.04/1.08), 원본·
# 보정 점수는 4h 값(-0.38/-0.62)을 섞어 "1w: 원본×Kz×Kp_mult=-0.4268 ≠ 표기된 보정값 -0.62"라는
# 존재하지 않는 오류를 발행했다(정작 1w 자체 수치 +0.48→+0.52는 Kz=1.00·Kp_mult=1.08 기준
# 0.48×1.00×1.08=0.5184로 표기값과 오차 0.0016에 불과해 아무 문제가 없었다).
# 수정: TF 헤더 기준으로 본문을 먼저 통째로 잘라(다음 TF 헤더 또는 문서 끝까지) 그 안에서만
# 각 필드를 독립적으로 검색한다 — 어떤 필드가 그 블록 안에서 매칭에 실패해도 다른 TF의 텍스트로
# 절대 넘어가지 않고 "해당 블록 검증 생략"으로 끝난다. 아울러 원본/보정/Kz/Kp_mult/W_TF 숫자
# 패턴에 "+" 부호를 허용해 모델이 양수에 명시적 "+"를 붙이는 표기를 정상적으로 인식한다.
FRACTAL_TF_HEADER_PATTERN = re.compile(r"정식 모드 보조 지표\s*—\s*TF\s*:\s*(?P<tf>\S+)")
FRACTAL_POS_LINE_PATTERN = re.compile(
    r"▸\s*상위 컨테이너 위치\s*\n\s*(?P<pos>[0-9]+(?:\.[0-9]+)?)%\s*Zone\s*\(ZONE_ID\s*=\s*(?P<zone>[^,)]+),\s*완충 보간 적용 여부\s*:\s*(?P<buffer>[YN])\)"
)
FRACTAL_KZKP_LINE_PATTERN = re.compile(
    r"▸\s*계산 적용 계수\s*\n\s*Kz\s*=\s*(?P<kz>[-+]?[0-9]+(?:\.[0-9]+)?)\s*/\s*Kp_mult\s*=\s*(?P<kp>[-+]?[0-9]+(?:\.[0-9]+)?)\s*/\s*W_TF\s*=\s*(?P<wtf>[-+]?[0-9]+(?:\.[0-9]+)?)"
)
FRACTAL_SCORE_LINE_PATTERN = re.compile(
    r"▸\s*계산 보정 점수\s*\n\s*원본\s*(?P<orig>[-+]?[0-9]+(?:\.[0-9]+)?)\s*➔\s*보정\s*(?P<corr>[-+]?[0-9]+(?:\.[0-9]+)?)"
)


def _split_fractal_tf_blocks(text):
    """TF 헤더를 경계로 본문을 자른다. 각 조각은 해당 TF 자신의 텍스트만 담으므로,
    이후 필드 매칭이 절대 다른 TF 블록으로 넘어갈 수 없다."""
    text = text or ""
    headers = list(FRACTAL_TF_HEADER_PATTERN.finditer(text))
    blocks = []
    for i, header_match in enumerate(headers):
        start = header_match.end()
        end = headers[i + 1].start() if i + 1 < len(headers) else len(text)
        blocks.append((header_match.group("tf"), text[start:end]))
    return blocks


def _fractal_zone_for_pos(pos):
    for zone_id, (lo, hi) in FRACTAL_ZONE_RANGES.items():
        if zone_id == 4:
            if lo <= pos <= hi:
                return zone_id
        elif lo <= pos < hi:
...
def _fractal_expected_buffer(pos):
    """경계값 미만 쪽으로만 ±2%p 편측 완충이 적용된다(3장 3항 이상=포함/미만=비포함 마스터 규정)."""
    return any(boundary - FRACTAL_ZONE_BUFFER_MARGIN <= pos < boundary for boundary in FRACTAL_ZONE_BOUNDARIES)


def verify_fractal_supplement_math(text):
    """[정식 모드 보조 지표] 텍스트에서 TF별 블록을 찾아 Zone 분류·완충 여부·Kz·Kp_mult·
    원본→보정 산식을 스펙 SSOT와 대조한다. 값은 고치지 않고 경고 문자열 리스트만 반환한다.
    TF 블록을 먼저 분리한 뒤 그 안에서만 필드를 찾으므로, 한 블록에서 특정 줄이 없거나
    형식이 달라도 다른 TF의 값을 대신 가져오는 일은 구조적으로 불가능하다."""
    warnings = []
    for tf, body in _split_fractal_tf_blocks(text):
        kzkp_match = FRACTAL_KZKP_LINE_PATTERN.search(body)
        score_match = FRACTAL_SCORE_LINE_PATTERN.search(body)
        pos_match = FRACTAL_POS_LINE_PATTERN.search(body)
        # "상위 컨테이너 위치" 섹션(계산 적용 계수 앞부분)에 "최상위 TF" 문구가 있으면
        # Zone% 형식이 아예 없는 최상위 TF 예외 표기로 간주한다.
        pos_section = body.split("▸ 계산 적용 계수")[0] if "▸ 계산 적용 계수" in body else body
        has_top_tf_marker = "최상위 TF" in pos_section

        if kzkp_match is None or score_match is None or (pos_match is None and not has_top_tf_marker):
            warnings.append(f"{tf}: 정식모드 보조지표 수치 파싱 실패로 검증 생략")
            continue

        kz = _parse_decimal(kzkp_match.group("kz"))
        kp = _parse_decimal(kzkp_match.group("kp"))
        orig = _parse_decimal(score_match.group("orig"))
        corr = _parse_decimal(score_match.group("corr"))
        if None in (kz, kp, orig, corr):
            warnings.append(f"{tf}: 정식모드 보조지표 수치 파싱 실패로 검증 생략")
            continue

        if not (FRACTAL_KZ_CLAMP[0] <= kz <= FRACTAL_KZ_CLAMP[1]):
            warnings.append(f"{tf}: 정식모드 보조지표 Kz={kz}가 클램프 범위 [0.3,1.8]을 벗어남")
        if kp not in FRACTAL_KP_MULT_VALID:
            warnings.append(f"{tf}: 정식모드 보조지표 Kp_mult={kp}가 허용된 5단계 이산값(1.00/1.08/1.16/1.25/1.35)이 아님")

        expected_corr = orig * kz * kp
        if abs(expected_corr - corr) > FRACTAL_CORRECTION_TOLERANCE:
            warnings.append(
                f"{tf}: 정식모드 보조지표 원본×Kz×Kp_mult={expected_corr:.4f} ≠ 표기된 보정값 {corr}"
            )

        if pos_match is None:
            # 최상위 TF 예외 표기(예: "최상위 TF (ZONE_ID=None, ...)")로, Zone% 형식 자체가 없다.
            if kz != Decimal("1.00"):
                warnings.append(f"{tf}: 정식모드 보조지표 최상위 TF 예외는 Kz=1.00 고정인데 Kz={kz}로 표기됨")
            continue

        pos = _parse_decimal(pos_match.group("pos"))
        zone_raw = pos_match.group("zone").strip()
        buffer_flag = pos_match.group("buffer")
        if pos is None:
            warnings.append(f"{tf}: 정식모드 보조지표 Pos% 파싱 실패로 검증 생략")
            continue

        is_top_tf_exception = not zone_raw.lstrip("-").replace(".", "", 1).isdigit()
        if is_top_tf_exception:
            if kz != Decimal("1.00"):
                warnings.append(f"{tf}: 정식모드 보조지표 최상위 TF 예외는 Kz=1.00 고정인데 Kz={kz}로 표기됨")
            continue

        zone_id = int(Decimal(zone_raw))
        expected_zone = _fractal_zone_for_pos(pos)
        if expected_zone is None:
            warnings.append(f"{tf}: 정식모드 보조지표 Pos%={pos}가 0~100 범위를 벗어나 Zone 산정 불가")
            continue
        if zone_id != expected_zone:
            warnings.append(
                f"{tf}: 정식모드 보조지표 Pos%={pos}%는 ZONE_ID={expected_zone} 범위인데 ZONE_ID={zone_id}로 표기됨"
            )

        expected_buffer = "Y" if _fractal_expected_buffer(pos) else "N"
        if buffer_flag != expected_buffer:
            warnings.append(
                f"{tf}: 정식모드 보조지표 완충 보간 적용 여부는 '{expected_buffer}'이어야 하는데 '{buffer_flag}'로 표기됨"
            )

        if expected_buffer == "N" and zone_id in FRACTAL_ZONE_KZ_SYMMETRIC:
            expected_kz = FRACTAL_ZONE_KZ_SYMMETRIC[zone_id]
            if kz != expected_kz:
                warnings.append(
                    f"{tf}: 정식모드 보조지표 ZONE_ID={zone_id}(대칭 Zone)는 Kz={expected_kz} 고정인데 Kz={kz}로 표기됨"
                )
        elif expected_buffer == "N" and zone_id in FRACTAL_ZONE_KZ_SIGN_DEPENDENT:
            if kz not in FRACTAL_ZONE_KZ_SIGN_DEPENDENT[zone_id]:
                warnings.append(
                    f"{tf}: 정식모드 보조지표 ZONE_ID={zone_id}(부호연동 Zone)는 Kz가 0.50 또는 1.50 중 하나여야 하는데 Kz={kz}로 표기됨"
                )
    return warnings


def run_fractal_supplement(phase1_result, symbol, exchange_name, custom_tfs=None):
    ...
    [결함수정-파이프라인정합성 #4] 실사용 응답(UPBIT FLOCK/KRW)을 직접 대조하다가 4개 TF 중
    1h 블록이 "V002-17 정정 참조" 문구 중간에서 그대로 끊긴 채 발행되는 응답 절단을 확인했다.
    이 함수는 원래 max_tokens=4000이었는데, 이 블록 하나(반복되는 "이원화 트랙 설계..." 안내문
    포함)만으로도 TF당 대략 800~1000토큰이 들어 4개를 다 채우면 여유가 거의 없었다. max_tokens를
    늘리고, PHASE1 카드 완결성 검증(validate_phase1_completeness)과 동일한 방식으로 요청한 TF
    개수만큼 블록이 실제로 도착했는지도 확인해 절단을 비차단 경고로 남긴다."""
    fractal_prompt = (
    ...
    fractal_result = call_gemini_api_with_retry(fractal_prompt, max_tokens=8000)
    math_warnings = verify_fractal_supplement_math(fractal_result)
    found_tfs = {tf for tf, _ in _split_fractal_tf_blocks(fractal_result)}
    missing_tfs = [tf for tf in (custom_tfs or []) if tf not in found_tfs]
    if missing_tfs:
        math_warnings = list(math_warnings) + [
            f"정식모드 보조지표 응답 절단 또는 누락: {', '.join(missing_tfs)} 블록이 응답에 없음"
        ]
    if math_warnings:
...
FC_COMPRESSION_RANGE_MAX = 25.0  # 경로B: 최근 FC_MIN_BARS_STANDARD봉 최고가 대비 변동폭 상한(%)
# [2026-09-05 실측 정정] 기존 5.0%는 V003[C] 설계 당시 이론값 — 업비트/빗썸 실제 데이터로
# 검증한 적 없었음. 이벤트수집 데이터(터지기 전 191건, 기준값 100건)로 확인한 결과 압축도가
# 10% 밑으로 내려가는 경우가 전무(터진 코인·평범한 코인 구분 없이 전부 10%대가 최저) —
# 5%는 이 시장에서 도달 자체가 불가능한 값이라 State1이 실전에서 영구히 0건이 되는 결함이었음.
# 실측 분포: 기준값 중앙값 38.3% vs 터지기 1~7일전 중앙값 29~31%(가장 좁음, D-0엔 40.8%로
# 다시 벌어짐 — 압축→확산 방향 자체는 일치). 터지기 1~7일전 구간의 하위 25% 지점(약 25%)을
# 데이터 기반 1차 추정치로 채택 — 표본이 아직 크지 않아(이벤트191/기준100) 데이터가 더
# 쌓이면 재조정 대상(사용자 승인 완료, 2026-09-05).
...
# [Phase A — D10 실측 반영, 2026-09-04] 업비트/빗썸 공식 API 문서 확인: 캔들 조회 1회 호출당
# 최대 200개(초과 요청 시 거래소가 200으로 자동 클리핑, 에러 아님) — API 호출 횟수·요금 불변으로
# 이력을 40→200봉까지 늘릴 수 있음(fc_observation_tier1.py Tier1 지표들의 표본 기반).
# [SSOT] 이 캐시(daily_ohlcv_cache)는 fc_compute_rtm 하나에서만 fetch하고 이후 전부 재사용 —
# 여기서 한 곳만 바꾸면 됨. 단, fc_build_candidate_payload의 Gemini 원시봉 텍스트 덤프는 별도
# 소비자로 캐시 전체를 그대로 dump하므로, 그 지점에서 최근 50봉으로 슬라이스해 하류 토큰비용을
# 기존과 동일하게 고정한다(FC_GEMINI_DUMP_BARS로 SSOT 분리 — 관측용 창(200)과 Gemini용 창(50)은
# 목적이 다른 별개 값이라 하나로 통일하지 않음, 통일 시 위 토큰비용 문제 재발).
FC_OBS_HISTORY_LIMIT = 200
FC_GEMINI_DUMP_BARS = 50
...
# ============================================================
# [2026-09-05 신설] FC-2(State진단)·FC-3(8모듈 S_scout) 파이썬 전환용 상수.
# 전부 _spec_number()로 본체 스펙(CANDLEVIEW_PROMPT_FULL) 원문에서 직접 추출한다 —
# 손으로 옮겨적지 않음으로써 오타·구버전 값 혼입을 원천 차단(SSOT 유지, 판정47/52번과
# 동일 원칙). 값이 None으로 나오면(스펙 문구가 바뀌어 정규식이 못 찾은 경우) 아래
# 스타트업 검증에서 즉시 드러나도록 방치한다(조용한 실패 금지).
# ============================================================
FC_S_SCOUT_PASS = _spec_number("S_SCOUT_PASS")
FC_S_SCOUT_W_VOLSQUEEZE = _spec_number("S_SCOUT_W_VOLSQUEEZE")
# [주의] "S_SCOUT_W_TAKERBUY"는 스펙 원문에 2번 등장한다 — ①상수 정의부(0.1456) ②모듈2
# 재정규화 설명 중 "거래소 미지원 시 0으로 제외"라는 조건부 프로즈(0). _spec_number는
# re.search(첫 매칭)라 문서상 ①이 ②보다 먼저 나와 정상값(0.1456)을 잡는다 — 순서에
# 의존하는 취약점이라 여기 명시해둔다(문서 구조가 바뀌면 재확인 필요).
FC_S_SCOUT_W_TAKERBUY = _spec_number("S_SCOUT_W_TAKERBUY")
FC_S_SCOUT_W_DEPTH = _spec_number("S_SCOUT_W_DEPTH")
FC_S_SCOUT_W_ABSORPTION = _spec_number("S_SCOUT_W_ABSORPTION")
FC_S_SCOUT_W_STRUCTURE = _spec_number("S_SCOUT_W_STRUCTURE")
FC_S_SCOUT_W_VOLACCEL = _spec_number("S_SCOUT_W_VOLACCEL")
FC_S_SCOUT_W_RELSTRENGTH = _spec_number("S_SCOUT_W_RELSTRENGTH")
FC_S_SCOUT_W_BOXRANGE = _spec_number("S_SCOUT_W_BOXRANGE")
FC_S_SCOUT_W_NO_TAKER_DENOM = _spec_number("S_SCOUT_W_NO_TAKER_DENOM")
FC_VROC_ACCEL_MIN = _spec_number("VROC_ACCEL_MIN")
FC_VROC_ACCEL_MAX = _spec_number("VROC_ACCEL_MAX")
FC_RS_WINDOW_1, FC_RS_WEIGHT_1 = int(_spec_number("RS_WINDOW_1")), _spec_number("RS_WEIGHT_1")
FC_RS_WINDOW_2, FC_RS_WEIGHT_2 = int(_spec_number("RS_WINDOW_2")), _spec_number("RS_WEIGHT_2")
FC_RS_WINDOW_3, FC_RS_WEIGHT_3 = int(_spec_number("RS_WINDOW_3")), _spec_number("RS_WEIGHT_3")
FC_RS_WINDOW_4, FC_RS_WEIGHT_4 = int(_spec_number("RS_WINDOW_4")), _spec_number("RS_WEIGHT_4")
FC_VOL_ABSORPTION_MIN = _spec_number("VOL_ABSORPTION_MIN")
FC_VOL_ABSORPTION_BODY_MAX = _spec_number("VOL_ABSORPTION_BODY_MAX")
FC_BODY_RATIO_STRONG = _spec_number("BODY_RATIO_STRONG")
FC_BODY_RATIO_WEAK = _spec_number("BODY_RATIO_WEAK")
FC_VOL_AVG_LOOKBACK = int(_spec_number("VOL_AVG_LOOKBACK"))
FC_ROLE_REVERSAL_TOLERANCE = _spec_number("ROLE_REVERSAL_TOLERANCE") / 100.0  # % -> 비율
FC_CONFLUENCE_AREA_MIN = _spec_number("CONFLUENCE_AREA_MIN") / 100.0  # % -> 비율

_FC_CORE_CONST_NAMES = [
    "FC_S_SCOUT_PASS", "FC_S_SCOUT_W_VOLSQUEEZE", "FC_S_SCOUT_W_TAKERBUY", "FC_S_SCOUT_W_DEPTH",
    "FC_S_SCOUT_W_ABSORPTION", "FC_S_SCOUT_W_STRUCTURE", "FC_S_SCOUT_W_VOLACCEL",
    "FC_S_SCOUT_W_RELSTRENGTH", "FC_S_SCOUT_W_BOXRANGE", "FC_S_SCOUT_W_NO_TAKER_DENOM",
    "FC_VROC_ACCEL_MIN", "FC_VROC_ACCEL_MAX", "FC_VOL_ABSORPTION_MIN", "FC_VOL_ABSORPTION_BODY_MAX",
    "FC_BODY_RATIO_STRONG", "FC_BODY_RATIO_WEAK", "FC_VOL_AVG_LOOKBACK",
    "FC_ROLE_REVERSAL_TOLERANCE", "FC_CONFLUENCE_AREA_MIN",
]
for _name in _FC_CORE_CONST_NAMES:
    if globals().get(_name) is None:
        print(f"[ERROR] FC 순수파이썬 전환 상수 추출 실패: {_name} — 본체 스펙 문구 확인 필요")
_w_sum_check = (FC_S_SCOUT_W_VOLSQUEEZE + FC_S_SCOUT_W_TAKERBUY + FC_S_SCOUT_W_DEPTH
                + FC_S_SCOUT_W_ABSORPTION + FC_S_SCOUT_W_STRUCTURE + FC_S_SCOUT_W_VOLACCEL
                + FC_S_SCOUT_W_RELSTRENGTH + FC_S_SCOUT_W_BOXRANGE)
if abs(_w_sum_check - 1.0) > 0.001:
    print(f"[ERROR] S_scout 8모듈 가중치 합={_w_sum_check:.4f}, 1.0이어야 함 — 스펙 추출값 확인 필요")
...
def fc_prefilter_universe(exchange, quote):
    ...
    [판정60번 — Cowork 재검토, 확정결함 수정] 스펙 FC-0의 제외 대상은 ①거래정지
    ②스테이블코인 ③확보봉수<8(완전배제 아님, 태그만) 3가지뿐이며 quoteVolume은
    제외기준으로 명시돼 있지 않다. 기존 코드는 qv(티커 24h 거래대금)가 None이거나
    0이면 이 단계에서 완전 배제해, 신규상장 직후처럼 티커 24h값이 아직 None/0인
    코인이 n_total·관측대기 목록 어디에도 잡히지 않고 조용히 사라졌다 — 스펙이
    명시한 "신규 상장 코인을 배제하지 않는다" 원칙과 충돌하는 확정 결함.
    수정: qv<=0/None인 종목도 universe에는 포함하되 mode="watch_only"로 미리
    태그해, 무거운 fetch_ohlcv(FC-1 진입용) 호출은 걸지 않으면서(기존 필터의
    API 호출량 절감 의도는 그대로 보존) n_total·관측대기 목록에는 정상 노출되도록
    한다. run_findcoin_scan은 이 사전 태그를 보고 fc_compute_rtm 호출을 건너뛴다."""
    try:
    ...
    for symbol, t in tickers.items():
        ...
        item = {"symbol": symbol, "base": base, "quote_volume_24h": qv, "last": _safe_float(t.get("last"))}
        if qv is None or qv <= 0:
            # [판정60번] 완전 배제 대신 관측대기 사전태그 — 무거운 OHLCV 조회는 생략.
...
def fc_compute_rtm(exchange, item):
    ...
    try:
        ...
        # [Phase A, 2026-09-04: 50→FC_OBS_HISTORY_LIMIT(200) 추가 확장] 위 47번 근거(RTM/압축/
        # ATR_STD용 여유버퍼)는 그대로 유효 — 이번 확장은 그 목적이 아니라 fc_observation_tier1
        # Tier1 지표(자기이력 백분위 등)가 40봉보다 긴 표본을 요구하기 때문. API 호출 1회·요금
        # 불변(D10 실측, 두 거래소 모두 count=200이 하드상한). 기존 RTM/압축 계산은 전부
        # daily[:-1][-FC_MIN_BARS_STANDARD:] 슬라이싱이라 이 확장에 영향받지 않음(구문·의미
        # 이중검증 완료) — Gemini 원시봉 덤프만 별도 지점에서 FC_GEMINI_DUMP_BARS로 재슬라이스.
        ...
        # 재사용해 API 호출 중복을 없앤다.
        daily = exchange.fetch_ohlcv(symbol, timeframe="1d", limit=FC_OBS_HISTORY_LIMIT)
    except Exception as e:
...
def fc_compute_liquidity_ratio(exchange, item):
    ...
    [2026-09-05 확장] FC-3 모듈3(S_depth)이 필요로 하는 bid_depth_10/ask_depth_10을
    여기서 함께 저장 — 이미 호출 중인 동일 order book 응답을 재사용해 추가 API 호출 없음."""
    symbol = item["symbol"]
    try:
        ...
        item["bid_depth_10"] = bid_depth
        item["ask_depth_10"] = ask_depth
...
# ============================================================
# [2026-09-05 신설] FC-2(State진단)+FC-3(8모듈 S_scout) 순수파이썬 조립.
# [배경] 기존엔 Gemini가 텍스트 프롬프트 안에서 State1/2/3 진단과 8모듈 계산을 수행했다.
# 전부 본체 14장에 이미 정확한 수식으로 명시된 결정론적 연산이라 판단(judgment)이 필요한
# 부분이 아니었음을 확인 — 진짜 판단이 필요한 FC-4(진입가·목표가)는 사용자 지시로 FindCoin
# 범위에서 제외(캔들뷰 본체가 담당)했으므로, FindCoin 전체가 Gemini 없이 완결된다.
# [무간섭] 기존 FC-0/FC-1(RTM/Percentile/Liquidity) 게이트는 전혀 건드리지 않는다 —
# 이 게이트를 이미 통과한 후보에게만 적용되는 그 다음 단계다.
# ============================================================

def fc_compute_pure_python_score(item, btc_daily):
    """1차 패스: State1/2/3 진단 + 상대강도 제외 7개 모듈 계산. item은 이미 FC-1 게이트를
    통과한 후보. daily_ohlcv_cache(200봉)를 그대로 쓴다 — 라이브 스캔이라 마지막 봉이
    실제 진행봉이므로 /이벤트수집(과거재현)에서 썼던 봉복제 보정이 불필요하다(그 보정은
    과거 특정일을 흉내낼 때만 필요 — 사용자 지적으로 이미 확인된 구분)."""
    bars = item.get("daily_ohlcv_cache")
    if not bars or len(bars) < FC_MIN_BARS_STANDARD + 10:
        item["fc_core_status"] = "insufficient_data"
        return item
    at_idx = len(bars) - 1
    completed_only = bars[:-1]

    # [판정61번 신설] 20일선 돌파 양봉 — 관측용 참고표시 전용, S_scout 점수(fc_modules_raw)에는
    # 포함하지 않는다(Phase C 원칙: 실측 검증 전 임의 가중치로 편입하지 않음). 라이브 스캔이라
    # bars 그대로 사용(마지막 봉=진행봉을 "당일"로 직접 참조하는 게 의도된 설계 — fc_compute_
    # pure_python_score 상단 주석과 동일 근거, 봉복제 보정 불필요).
    ma20_cross = fc_observation_tier1.tier1_ma20_cross_bullish(bars, FC_MIN_BARS_STANDARD)
    item["fc_ma20_cross_bullish"] = bool(ma20_cross.get("signal")) if ma20_cross.get("status") == "ok" else None

    major_highs, major_lows = fc_core_scoring.find_major_swings(completed_only)

    state1 = fc_core_scoring.diagnose_state1(
        bars, at_idx, item.get("compression_pct"), FC_COMPRESSION_RANGE_MAX,
        FC_VOL_ABSORPTION_MIN, FC_VOL_ABSORPTION_BODY_MAX,
        fc_observation_tier1.tier1_spring_reclaim,
    )
    state2 = fc_core_scoring.diagnose_state2(bars, at_idx, major_highs, FC_BODY_RATIO_STRONG)
    state3 = fc_core_scoring.diagnose_state3(
        bars, at_idx, FC_BODY_RATIO_WEAK,
        fc_observation_tier1.tier1_selling_exhaustion,
        fc_state3_structure.detect_ob_fvg_confluence_rolereversal,
        fc_state3_structure.check_pullback_support_touch,
        FC_ROLE_REVERSAL_TOLERANCE,
    )
    resolved = fc_core_scoring.resolve_state(state1, state2, state3)
    if not resolved.get("diagnosed"):
        item["fc_core_status"] = "no_state_diagnosed"
        return item

    s_absorption = fc_core_scoring.module_absorption(
        bars, at_idx, FC_VOL_ABSORPTION_MIN, FC_VOL_ABSORPTION_BODY_MAX)
    s_vol_accel = fc_core_scoring.module_vol_accel(bars, at_idx, FC_VROC_ACCEL_MIN, FC_VROC_ACCEL_MAX)
    s_depth = fc_core_scoring.module_depth(item.get("bid_depth_10"), item.get("ask_depth_10"))

    rs_raw = None
    if btc_daily:
        rs_raw = fc_core_scoring.compute_rs_raw(
            bars, btc_daily, at_idx,
            [(FC_RS_WINDOW_1, FC_RS_WEIGHT_1), (FC_RS_WINDOW_2, FC_RS_WEIGHT_2),
             (FC_RS_WINDOW_3, FC_RS_WEIGHT_3), (FC_RS_WINDOW_4, FC_RS_WEIGHT_4)],
        )

    item["fc_core_status"] = "diagnosed"
    item["fc_state"] = resolved.get("state")
    item["fc_state_detail"] = resolved
    item["fc_modules_raw"] = {
        "vol_squeeze": item.get("s_vol_squeeze") or 0.0,
        "taker_buy": 0.0,  # 업비트·빗썸 미지원 — 항상 0, S_scout 집계 시 재정규화
        "depth": s_depth if s_depth is not None else 0.0,
        "absorption": s_absorption,
        "structure": resolved.get("s_structure", 0.0),
        "vol_accel": s_vol_accel,
        "relstrength": None,  # 2차 패스(교차 백분위)에서 채움
        "boxrange": item.get("s_boxrange") or 0.0,
    }
    item["_rs_raw"] = rs_raw
    return item


def fc_finalize_scoring(candidates):
    """2차 패스: 상대강도(모듈7) 교차백분위 계산 → S_scout 최종 집계 → FC-5 게이트
    (S_scout≥6.5, R:R 조건 없음 — 사용자 지시로 FindCoin은 가격분석 미수행) → Top3."""
    diagnosed = [c for c in candidates if c.get("fc_core_status") == "diagnosed"]
    if not diagnosed:
        return []
    rs_list = [(c["symbol"], c.get("_rs_raw")) for c in diagnosed]
    rs_pctile = fc_core_scoring.compute_relstrength_percentile(rs_list)

    weights = {
        "vol_squeeze": FC_S_SCOUT_W_VOLSQUEEZE, "taker_buy": FC_S_SCOUT_W_TAKERBUY,
        "depth": FC_S_SCOUT_W_DEPTH, "absorption": FC_S_SCOUT_W_ABSORPTION,
        "structure": FC_S_SCOUT_W_STRUCTURE, "vol_accel": FC_S_SCOUT_W_VOLACCEL,
        "relstrength": FC_S_SCOUT_W_RELSTRENGTH, "boxrange": FC_S_SCOUT_W_BOXRANGE,
        "no_taker_denom": FC_S_SCOUT_W_NO_TAKER_DENOM,
    }
    for c in diagnosed:
        modules = dict(c["fc_modules_raw"])
        modules["relstrength"] = rs_pctile.get(c["symbol"], 0.0)
        s_scout = fc_core_scoring.aggregate_s_scout(modules, weights, taker_supported=False)
        c["s_scout"] = s_scout
        c["fc_pass"] = s_scout >= FC_S_SCOUT_PASS

    passed = [c for c in diagnosed if c["fc_pass"]]
    passed.sort(key=lambda c: c["s_scout"], reverse=True)
    return passed[:3]
...
def run_findcoin_scan(ex_name):
    ...
    # 1단계(가벼움, 전종목): RTM 계산
    # [판정60번] fc_prefilter_universe가 qv<=0/None 종목을 이미 mode="watch_only"로
    # 사전태그해뒀으면 무거운 fetch_ohlcv(fc_compute_rtm)를 걸지 않고 바로 관측대기로 분류.
    rtm_results, watch_only = [], []
    for item in universe:
        if item.get("mode") == "watch_only":
    ...
    n_gate2 = len(final_candidates)  # 임시값 — 아래 순수파이썬 조립 단계에서 diagnosed 수로 재할당됨

    # ========================================================
    # FindCoin Phase C — Tier1 관측 산출 및 축적 (2026-09-04 신설)
    # [무간섭 확인] 이 지점은 final_candidates가 이미 확정된 뒤다 — 아래 블록이 무엇을 하든
    # 위에서 계산된 선정 결과(path_a_final/path_b_final/final_candidates)를 바꿀 수 없다.
    # 강제 게이트가 아니다 — rtm_results(전체 유효종목)에 관측값만 얹고, 선정 로직은 건드리지
    # 않는다(Phase C 설계 원칙: 전량 표시, 컷오프 없음).
    # ========================================================
    for item in rtm_results:
        fc_compute_compression(item)  # #5 VCP — 기존엔 range_ok 통과자만 계산되던 걸 전체로 확장(중복계산, API호출 없음)
        fc_compute_box_range(item)    # #15 box_range — 기존엔 최종후보(~20개)만 계산되던 걸 전체로 확장(API호출 없음)

    def _tier1_own_lookback_return(daily_bars, lookback):
        """관측 스냅샷 시점 기준 이 종목 자신의 lookback일 수익률. tier1_relative_strength_btc/
        universe_relative_return과 동일 정의(완성봉만, 시작봉 종가 대비 끝봉 종가) — 별도
        재정의 없이 여기 한 곳에서만 계산해 둘 다에 재사용."""
        if not daily_bars or len(daily_bars) < 2:
            return None
        completed = daily_bars[:-1]
        window = completed[-lookback:]
        if len(window) < lookback or window[0][4] <= 0:
            return None
        return (window[-1][4] / window[0][4]) - 1.0

    def _completed_close_snapshot(daily_bars):
        """관측가로 쓸 값 — 오늘 진행봉이 아닌 가장 최근 완성봉 종가(진행봉 처리 통일 규칙과 동일 원칙)."""
        if not daily_bars or len(daily_bars) < 2:
            return None
        return daily_bars[-2][4]

    universe_returns = []
    for item in rtm_results:
        item["_tier1_own_return"] = _tier1_own_lookback_return(item.get("daily_ohlcv_cache"), FC_MIN_BARS_STANDARD)
        if item["_tier1_own_return"] is not None:
            universe_returns.append(item["_tier1_own_return"])

    # BTC 참조 일봉 — 이 스캔 전체에서 1회만 조회(코인별 반복 아님). 실패해도 관측 자체를
    # 막지 않고 상대강도(#11) 항목만 insufficient_data로 남는다(coverage-first).
    btc_daily_for_obs = None
    try:
        btc_daily_for_obs = exchange_class.fetch_ohlcv(f"BTC/{quote}", timeframe="1d", limit=FC_OBS_HISTORY_LIMIT)
    except Exception as e:
        print(f"[WARN] FindCoin Tier1 관측용 BTC일봉 조회 실패({ex_name}): {e}")

    obs_records_to_log = []
    for item in rtm_results:
        obs = fc_observation_tier1.compute_tier1_observation(
            daily_bars=item.get("daily_ohlcv_cache"),
            lookback=FC_MIN_BARS_STANDARD,
            btc_daily_bars=btc_daily_for_obs,
            universe_returns=universe_returns,
            item_return_for_universe=item.get("_tier1_own_return"),
        )
        # #2/#8/#5/#15 — 기존 main.py 함수가 이미 계산해둔 값을 재계산 없이 그대로 병합
        # (fc_observation_tier1.py 자체 설계원칙: 이 4개는 그 모듈이 재구현하지 않음).
        obs["reused_vol_squeeze_ratio"] = item.get("vol_squeeze_ratio")
        obs["reused_s_vol_squeeze"] = item.get("s_vol_squeeze")
        obs["reused_compression_pct"] = item.get("compression_pct")
        obs["reused_s_boxrange"] = item.get("s_boxrange")
        obs["reused_box_status"] = item.get("box_status")
        item["tier1_observation"] = obs

        snapshot_price = _completed_close_snapshot(item.get("daily_ohlcv_cache"))
        if snapshot_price is not None:
            obs_records_to_log.append((item["symbol"], ex_name, obs, snapshot_price))

    # [스레드 난립 방지] 종목별로 스레드를 따로 띄우지 않는다 — 유효종목이 수백 개일 수 있어
    # 스캔 1회당 스레드 수백 개가 생기는 걸 막기 위해 배치 전체를 스레드 1개에서 순차 기록한다.
    # (알려진 한계: 유효종목이 아주 많으면 이 스레드가 한동안 Notion API를 순차 호출하며
    # 실행되지만, 백그라운드·데몬 스레드라 텔레그램 응답은 막지 않는다. Render 로그에서 지연이나
    # 429가 관측되면 Phase D 재검토 대상 — 지금은 알려진 트레이드오프로 그대로 둔다.)
    if FC_OBS_TRACKING_ENABLED and obs_records_to_log:
        threading.Thread(target=_fc_obs_record_batch, args=(obs_records_to_log,), daemon=True).start()

    # ========================================================
    # [2026-09-05 신설] FC-2/FC-3 순수파이썬 조립 — 여기서 실행하는 이유: 위 Phase C
    # 블록에서 이미 BTC 일봉(btc_daily_for_obs)을 조회해뒀으므로 그대로 재사용(추가
    # API호출 0). compression_pct/s_boxrange/s_vol_squeeze도 이미 rtm_results 전체에
    # 계산되어 있고, final_candidates는 그 rtm_results와 객체를 공유하므로(동일 dict
    # 참조) 이 시점엔 전부 채워진 상태다 — 경로A/B 구분 없이 그대로 사용 가능.
    # [무간섭 재확인] Phase C 관측 축적(obs_records_to_log)은 이미 위에서 끝났다 — 아래
    # 블록은 그 결과에 전혀 영향을 주지 않고 final_candidates만 재구성한다.
    # ========================================================
    for item in final_candidates:
        fc_compute_pure_python_score(item, btc_daily_for_obs)
    top3 = fc_finalize_scoring(final_candidates)
    n_diagnosed = sum(1 for it in final_candidates if it.get("fc_core_status") == "diagnosed")
    n_gate2 = n_diagnosed  # State1/2/3 중 하나라도 진단된 후보 수(기존 'LLM 입력수' 의미를
    # 대체 — Gemini 위임을 없앴으므로 이제 "옥석검증 통과 전 State 진단 성공 수"를 나타냄)
    final_candidates = top3

    return final_candidates, n_total, n_valid, n_gate1, n_gate2, watch_only


# ============================================================
# FindCoin — Layer2~3 (V003[C] 챕터14 FC-2~FC-3, 판정57번 갱신)
# [2026-09-05부로 갱신] 이전엔 이 지점에서 이중경로(경로A/경로B) 통과 후보에 상세
# 캔들데이터를 붙여 Gemini에 전달해 State 진단·옥석검증·S_scout·Top3 선정을 맡겼으나,
# 본체 스펙에 이미 결정론적 수식으로 명시돼 있음을 확인해 Gemini 위임을 제거하고
# fc_compute_pure_python_score/fc_finalize_scoring(위에서 정의)으로 순수 파이썬 직접
# 조립한다. 손익비(R:R)·진입가·Top3 선정은 FC-4/FC-5에서 상속하지 않는다 — 사용자
# 지시로 FindCoin 범위에서 제외(스펙 판정56번 참조, 가격분석은 캔들뷰 본체 담당).
# 아래 FC_MAX_CANDIDATES_TO_LLM·FC_GEMINI_DUMP_BARS 등 변수명은 구 아키텍처(Gemini
# 위임) 명칭이 그대로 남아있으나, 값 자체(상한 개수·덤프 봉수)는 지금도 유효해 이름만
# 구식일 뿐 기능에는 영향이 없다(판정57번 — 리네이밍은 하류 참조 전수스캔 부담 대비
# 실익이 낮아 보류).
# ============================================================
    ...
    for c in candidates:
        ...
        try:
            ...
            if not ohlcv:
                ...
                # 조용히 분석되는 회귀위험 방지)
                ohlcv = exchange_class.fetch_ohlcv(c["symbol"], timeframe="1d", limit=FC_GEMINI_DUMP_BARS)
            else:
                # [Phase A, 2026-09-04] 캐시는 관측모듈용으로 FC_OBS_HISTORY_LIMIT(200)까지 보유하지만,
                # 이 지점은 Gemini 프롬프트에 원시봉을 그대로 텍스트 dump하는 별도 소비자라 창 크기가
                # 다르다 — 슬라이스 없이 그대로 넘기면 토큰비용이 4배로 뛴다(360도 스캔에서 확인된
                # 실제 리스크). 기존 운영 시 받던 봉수(FC_GEMINI_DUMP_BARS=50)와 동일하게 최근 것만
                # 잘라 하류 비용을 정확히 보존한다. 캐시가 오름차순(과거→최근)이므로 뒤에서부터 슬라이스.
                ohlcv = ohlcv[-FC_GEMINI_DUMP_BARS:]
...
def _fc_module_strengths_kr(fc_modules_raw):
    """8모듈 중 상대적으로 강한 것 상위 2~3개를 한글로 뽑아 근거문에 사용."""
    kr_names = {
        "vol_squeeze": "변동성압축", "taker_buy": "체결강도(미지원)", "depth": "매수벽우위",
        "absorption": "수급흡수", "structure": "구조신뢰도", "vol_accel": "거래량가속",
        "relstrength": "BTC대비강도", "boxrange": "박스권",
    }
    scored = [(kr_names.get(k, k), v) for k, v in fc_modules_raw.items()
              if k != "taker_buy" and v is not None]
    scored.sort(key=lambda x: x[1], reverse=True)
    top = [name for name, v in scored[:3] if v >= 0.3]
    return top


def _fc_format_result_text(top3, ex_display, quote, scan_time_kst, n_total, n_valid, n_gate1, n_gate2):
    """[2026-09-05 신설] Gemini 없이 파이썬이 직접 서식화. 사용자 지시로 진입가·손절선·
    목표가·손익비는 포함하지 않는다 — 상세 가격분석은 캔들뷰 본체(/거래소 코인명)가 담당,
    FindCoin은 코인명+근거까지만 출력한다."""
    if not top3:
        return None
    lines = [
        f"🔎 스캔 결과 최종 합격코인 : {len(top3)}개\n",
        f"대상 : {ex_display} {quote} 마켓",
        f"스캔 시각: {scan_time_kst} (KST)",
        f"총 스캔 종목: {n_total}개 (유효 {n_valid}개)",
        f"\n🧭 스캔 단계별 현황",
        f"➔ 1차 경로 통과 {n_gate1}개",
        f"➔ State 진단 성공 {n_gate2}개",
        f"➔ 최종 합격(S_scout≥{FC_S_SCOUT_PASS}) {len(top3)}개\n",
    ]
    medal = ["🥇", "🥈", "🥉"]
    state_kr = {"State1": "매집/응축", "State2": "돌파초입", "State3": "눌림목"}
    for i, c in enumerate(top3):
        strengths = _fc_module_strengths_kr(c.get("fc_modules_raw", {}))
        path_tag = f"[경로{c.get('fc_path', '?')}]"
        # [판정61번 신설] 20일선 돌파 양봉 — 참고표시 전용(S_scout 미반영, 있음일 때만 노출).
        ma20_line = "   참고: 20일선 돌파 양봉 감지\n" if c.get("fc_ma20_cross_bullish") is True else ""
        lines.append(
            f"{medal[i] if i < 3 else '▪️'} {c['symbol']} {path_tag}\n"
            f"   State: {c.get('fc_state')}({state_kr.get(c.get('fc_state'), '')})\n"
            f"   S_scout: {c.get('s_scout', 0):.2f} / 10\n"
            f"   근거: {', '.join(strengths) if strengths else '복합모듈 종합'}\n"
            f"{ma20_line}"
        )
    lines.append(
        "\n💡 진입가·손절선·목표가는 표시하지 않습니다 — 위 코인을 선택해 캔들뷰 상세분석을 "
        "받아보세요(아래 버튼 또는 /거래소 코인명)."
    )
    return "\n".join(lines)


def run_findcoin(ex_name):
    """FindCoin 전체 실행: Layer0~1(Python 게이트) → Layer2~3(2026-09-05부터 순수 파이썬,
    Gemini 위임 제거) → Top3 직접 서식화.
    [배경] FC-2(State진단)·FC-3(8모듈)는 본체 스펙에 이미 결정론적 수식으로 명시돼 판단이
    필요하지 않았음을 확인(main.py _spec_number로 상수 직접 추출, fc_core_scoring.py에서
    구현·검증 완료). FC-4(진입가·손익비)는 사용자 지시로 FindCoin 범위에서 제외(캔들뷰
    본체가 담당) — 이로써 FindCoin 전체가 Gemini 없이 완결된다.
    반환: (display_text, n_total, n_valid, n_gate1, n_gate2, error_message, top_symbols, n_watch)"""
    quote = SUPPORTED_EXCHANGES[ex_name]["quote"]
    ...
    if n_gate2 == 0 or not candidates:
    ...
    display_text = _fc_format_result_text(candidates, ex_display, quote, scan_time_kst,
                                           n_total, n_valid, n_gate1, n_gate2)
    top_symbols = [c["symbol"] for c in candidates]
...
# ============================================================
# PHASE 2 실전 결과 추적 — Notion 영속 저장 (신규, 2026-09)
# ============================================================
# [설계 배경] 판정이력 45·50번: "일단 로그만 남겨서 관찰하자"는 관측모드는 실제로 켰었지만,
# 그 관측값을 담아둘 영속 저장소 자체가 없어 무기한 방치됐다고 자체 인정한 전례가 있다.
# Render free 플랜은 persistent disk를 지원하지 않고 파일시스템이 재배포·재시작·spin-down마다
# 초기화된다(2026-09 공식문서 확인) — 판정 시각과 결과 확인 시각 사이(최소 며칠)에 반드시
# 최소 1회 재시작이 끼므로 로컬 CSV/SQLite는 이 용도로 쓸 수 없다. Notion을 영속 계층으로
# 쓴다 — 개인 워크스페이스는 무료로 페이지 무제한(Render 무료 Postgres처럼 30일 뒤 만료되지
# 않음), Render 플랜·디스크 변경이 전혀 필요 없다. NOTION_TOKEN/NOTION_DATA_SOURCE_ID가
# 비어 있으면 TRACKING_ENABLED=False로 전 기능이 조용히 꺼지고 기존 텔레그램 발행 경로에는
# 어떤 영향도 주지 않는다.
#
# [범위] published_briefing이 성공 발행되고 direction≠횡보(가격경로 존재)인 건만 기록한다.
# 횡보는 채점할 가격 목표가 없어 이번 버전 추적 대상에서 제외했다(스코프 결정).
#
# [판정 방식 — 고정 N봉이 아니라 first-touch를 쓴 이유] backtest_framework의
# FORWARD_WINDOW=20봉은 과거 슬라이딩 윈도우 배치 처리라 봉 인덱스가 자연스럽지만, 이 시스템은
# 여러 타임프레임(1d/4h/1h)을 함께 쓰고 PHASE2 가격경로가 어느 타임프레임 기준인지 코드로
# 고정돼 있지 않다. "N봉 뒤 종가"보다 "목표가·무효화가 중 어느 쪽을 먼저 터치했는가"가 이
# 시스템이 실제로 하는 예측(가격경로)의 정의에 더 맞는 채점 방식이라 판단했다 — 이건 설계
# 선택이라 사용자 확인 필요. 같은 1시간봉 안에서 무효화·목표가 동시에 걸치면 낙관적 왜곡을
# 피하기 위해 무효화를 우선한다.
#
# [OUTCOME_MAX_HOLD_DAYS — 확인 필요, 임의값] 20일은 backtest_framework의
# FORWARD_WINDOW=20(일봉 기준)을 그대로 유추한 것뿐, PHASE2의 실제 정책값이 아니다.
# 실제 목표 도달까지 걸리는 시간대를 관찰한 뒤 조정 대상.
# ============================================================
NOTION_API_VERSION = "2026-03-11"  # [확인: 2026-09-04 공식문서] 최신 버전. 이 DB는 신형
                                     # 멀티 데이터소스 구조로 생성되어 구버전(2022-06-28) 대신 이걸 쓴다.
TRACKING_ENABLED = bool(NOTION_TOKEN and NOTION_DATA_SOURCE_ID)
OUTCOME_CHECK_INTERVAL_SECONDS = 60 * 60  # 1시간마다 미결 건 점검
OUTCOME_MAX_HOLD_DAYS = 20  # 위 주석 참조 — 확정값 아님

EXCHANGE_CCXT_ID = {"UPBIT": "upbit", "BITHUMB": "bithumb", "COINBASE": "coinbase"}


def _resolve_ccxt_exchange_id(exchange_label):
    """main.py 다른 곳의 거래소 표기(대소문자 혼재)를 ccxt 소문자 id로 흡수한다."""
    key = str(exchange_label or "").strip()
    return EXCHANGE_CCXT_ID.get(key.upper(), key.lower())
...
# [수정 — 2026-09-04] _safe_float는 main.py 1414행에 이미 (x, default=None) 시그니처로 정의돼
# 있었다. 여기서 동명 함수를 (value)만 받는 시그니처로 다시 정의하면서 모듈 전역에서 나중 정의가
# 이전 정의를 덮어써, Whale Wall(1700행)을 비롯해 기존 _safe_float(x, 0.0) 2-인자 호출부 전체가
# "takes 1 positional argument but 2 were given"로 깨졌다(배포 후 실제 로그로 확인). 새로 정의하지
# 않고 기존 _safe_float(x, default=None)를 그대로 재사용한다 — default가 이미 None이라 아래
# 단일 인자 호출부와 동작이 동일하다.


def notion_create_prediction_row(payload):
    """PHASE 2 판정 1건을 Notion DB에 '대기중' 상태로 기록한다. 실패해도 예외를 올리지 않는다
    (호출부가 별도 스레드이므로 여기서 죽어도 텔레그램 발행에는 이미 영향이 없지만, 로그는 남긴다)."""
    if not TRACKING_ENABLED:
        return None
    # [결함수정 — 2026-09-04] ledger의 등급 enum은 강/보통/약함/횡보 네 가지지만, Notion DB의
    # 등급 select는 강/보통/약함/미상만 옵션으로 만들어뒀다(추적 대상 자체가 direction≠횡보인
    # 건만이라 "횡보" 등급 옵션을 안 만들었음). 이론상 이 조합(방향은 상방/하방인데 등급만 횡보)이
    # 실제로 나오는지 코드로 못 찾았고, Notion API가 미리 없는 select 옵션 값을 어떻게 처리하는지도
    # (자동 생성/거부) 확인 못 했다 — 확인 안 된 두 가지를 전제로 짜지 않고, 화이트리스트 밖 값은
    # 전부 "미상"으로 방어적으로 치환한다.
    _valid_grades = {"강", "보통", "약함"}
    grade_value = payload.get("grade")
    grade_value = grade_value if grade_value in _valid_grades else "미상"
    props = {
        "심볼": {"title": [{"text": {"content": str(payload.get("symbol") or "")}}]},
        "거래소": {"select": {"name": str(payload.get("exchange") or "")}},
        "방향": {"select": {"name": str(payload.get("direction") or "")}},
        "등급": {"select": {"name": grade_value}},
        "최종신뢰도점수": {"number": _safe_float(payload.get("final_confidence_score"))},
        "메인확률pct": {"number": _safe_float(payload.get("main_probability_pct"))},
        "진입가": {"number": _safe_float(payload.get("entry"))},
        "무효화가": {"number": _safe_float(payload.get("invalidation"))},
        "목표가1": {"number": _safe_float(payload.get("target_1"))},
        "목표가2": {"number": _safe_float(payload.get("target_2"))},
        "판정시각": {"date": {"start": payload.get("created_at_iso")}},
        "상태": {"select": {"name": "대기중"}},
        "세션ID": {"rich_text": [{"text": {"content": str(payload.get("session_id_hash") or "")}}]},
    }
    body = {"parent": {"data_source_id": NOTION_DATA_SOURCE_ID}, "properties": props}
    try:
        res = requests.post("https://api.notion.com/v1/pages", headers=_notion_headers(), json=body, timeout=15)
        if res.status_code >= 300:
            print(f"[NOTION_TRACK] 기록 실패 status={res.status_code} body={res.text[:300]}")
            return None
        return res.json().get("id")
    except Exception as e:
        print(f"[NOTION_TRACK] 기록 예외: {e}")
        return None


def record_phase2_tracking_prediction(payload):
    """백그라운드 스레드 진입점. created_at_iso를 여기서 확정해 기록한다."""
    try:
        payload = dict(payload)
        payload["created_at_iso"] = datetime.now(timezone.utc).isoformat()
        notion_create_prediction_row(payload)
    except Exception as e:
        print(f"[NOTION_TRACK] 기록 스레드 예외: {e}")


def _notion_num(props, key):
    return (props.get(key) or {}).get("number")


def _notion_select(props, key):
    return ((props.get(key) or {}).get("select") or {}).get("name")


def _notion_date_start(props, key):
    return ((props.get(key) or {}).get("date") or {}).get("start")


def _notion_title(props, key):
    title = (props.get(key) or {}).get("title") or []
    return title[0]["plain_text"] if title else ""


def notion_query_pending_predictions():
    """상태=대기중인 행 전체를 페이지네이션으로 모아 반환한다. 실패 시 빈 리스트(다음 주기에 재시도)."""
    if not TRACKING_ENABLED:
        return []
    url = f"https://api.notion.com/v1/data_sources/{NOTION_DATA_SOURCE_ID}/query"
    body = {"filter": {"property": "상태", "select": {"equals": "대기중"}}, "page_size": 100}
    results, cursor = [], None
    while True:
        if cursor:
            body["start_cursor"] = cursor
        try:
            res = requests.post(url, headers=_notion_headers(), json=body, timeout=15)
        except Exception as e:
            print(f"[NOTION_TRACK] 대기중 조회 예외: {e}")
            break
        if res.status_code >= 300:
            print(f"[NOTION_TRACK] 대기중 조회 실패 status={res.status_code} body={res.text[:300]}")
            break
        data = res.json()
...
def notion_update_prediction_outcome(page_id, status, realized_price, resolved_at_iso):
    if not TRACKING_ENABLED:
        return False
    props = {
        "상태": {"select": {"name": status}},
        "실현가": {"number": _safe_float(realized_price)},
        "결과확정시각": {"date": {"start": resolved_at_iso}},
    }
    try:
        res = requests.patch(
            f"https://api.notion.com/v1/pages/{page_id}",
            headers=_notion_headers(), json={"properties": props}, timeout=15,
        )
        if res.status_code >= 300:
            print(f"[NOTION_TRACK] 갱신 실패 status={res.status_code} body={res.text[:300]}")
            return False
        return True
    except Exception as e:
        print(f"[NOTION_TRACK] 갱신 예외: {e}")
        return False


def determine_first_touch_outcome(exchange_label, symbol, direction, invalidation, target_1, target_2, since_ms,
                                   timeframe="1h", per_call_limit=200, max_calls=5):
    """판정 시각 이후 1시간봉을 페이지네이션으로 모아 목표가·무효화가 중 먼저 터치한 쪽을 찾는다.

    [결함수정 — 2026-09-04] 최초 버전은 fetch_ohlcv(since=..., limit=1000) 단일 호출이었다.
    ccxt upbit.py 소스를 직접 확인한 결과, limit(=count)은 검증 없이 그대로 거래소 요청에
    실리고 거래소 1회 호출당 반환 캔들수 자체가 제한돼 있다(backtest_framework.py의
    fetch_ohlcv_range 주석과 동일 문제 — "거래소 1회 호출당 캔들 반환 개수 제한(대부분 200개
    내외)"). upbit.py는 since로 받은 값을 그대로 시작점으로 안 쓰고 `to = since + 캔들간격*limit`을
    계산해 그 시각까지의 최근 count개를 요청하는 구조라, limit=1000처럼 크게 잡으면 to가 아득히
    미래로 밀려나 거래소가 "지금 시점 기준 최근 것"을 돌려주고, since 직후~최근 사이 구간이
    통째로 빠질 수 있었다(며칠 지난 대기중 건일수록 위험 — 실제 목표 도달을 놓치고 나중에
    엉뚱하게 만료 처리될 수 있었음). backtest_framework.fetch_ohlcv_range와 동일하게
    since부터 페이지네이션으로 순서대로 모은다."""
    ex_id = _resolve_ccxt_exchange_id(exchange_label)
    try:
    ...
    for _ in range(max_calls):
        ...
        bars.extend(batch)
        last_ts = batch[-1][0]
        if last_ts <= since:  # 진행 없음 — 무한루프 방지 (backtest_framework와 동일 가드)
            break
        since = last_ts + 1
        if len(batch) < per_call_limit:
            break  # 마지막 페이지(더 이상 데이터 없음)
    if not bars:
        return {"resolved": False, "error": "no_bars"}
    for _, o, h, l, c, v in bars:
        if direction == "상방":
            if l <= invalidation:
                return {"resolved": True, "status": "무효화", "price": invalidation}
            if h >= target_2:
                return {"resolved": True, "status": "목표2도달", "price": target_2}
            if h >= target_1:
                return {"resolved": True, "status": "목표1도달", "price": target_1}
        else:
            if h >= invalidation:
                return {"resolved": True, "status": "무효화", "price": invalidation}
            if l <= target_2:
            ...
            if l <= target_1:
                return {"resolved": True, "status": "목표1도달", "price": target_1}
    return {"resolved": False, "last_price": bars[-1][4]}


def _check_pending_predictions_once():
    now = datetime.now(timezone.utc)
    for page in notion_query_pending_predictions():
        props = page.get("properties", {})
        page_id = page.get("id")
        symbol = _notion_title(props, "심볼")
        exchange_label = _notion_select(props, "거래소")
        direction = _notion_select(props, "방향")
        invalidation = _notion_num(props, "무효화가")
        target_1 = _notion_num(props, "목표가1")
        target_2 = _notion_num(props, "목표가2")
        judged_at_str = _notion_date_start(props, "판정시각")
        if not all([symbol, exchange_label, direction, invalidation, target_1, target_2, judged_at_str]):
            continue
        try:
            judged_at = datetime.fromisoformat(judged_at_str.replace("Z", "+00:00"))
        except ValueError:
            continue
        since_ms = int(judged_at.timestamp() * 1000)
        outcome = determine_first_touch_outcome(exchange_label, symbol, direction, invalidation, target_1, target_2, since_ms)
        if outcome.get("resolved"):
            notion_update_prediction_outcome(page_id, outcome["status"], outcome["price"], now.isoformat())
        elif (now - judged_at).days >= OUTCOME_MAX_HOLD_DAYS:
            notion_update_prediction_outcome(page_id, "만료(미결)", outcome.get("last_price"), now.isoformat())


def _run_outcome_check_loop():
    while True:
        time.sleep(OUTCOME_CHECK_INTERVAL_SECONDS)
        if not TRACKING_ENABLED:
            continue
        try:
            _check_pending_predictions_once()
        except Exception as e:
            print(f"[NOTION_TRACK] 점검 스레드 오류: {e}")


if TRACKING_ENABLED:
    threading.Thread(target=_run_outcome_check_loop, daemon=True).start()
    print("[INFO] PHASE2 실전 결과 추적(Notion) 활성화")
else:
    print("[INFO] PHASE2 실전 결과 추적 비활성화 — NOTION_TOKEN/NOTION_DATA_SOURCE_ID 미설정")


# ============================================================
# FindCoin Phase B — Tier1 관측 사후채점 루프 (2026-09-04 신설)
# [모듈식 무간섭 확장] FC-0~FC-5 코어 게이트(run_findcoin_scan/run_findcoin)를 전혀 수정하지
# 않는다. 아래 함수들은 현재 어디에서도 호출되지 않는 대기 상태(Phase E에서 연결 예정) —
# 배포해도 기존 FindCoin/지정코인 산출물에 영향이 없다.
#
# [목적] Phase A(fc_observation_tier1.py)가 산출하는 13개 지표 각각이 baseline 대비 실제
# 예측력이 있는지 사후채점하기 위한 축적 전용 루프. PHASE2와 달리 "진입가/목표가/무효화가"
# 예측이 아니라 "이 지표값이 관측된 뒤 N일 뒤 가격이 어떻게 움직였는가"만 기록한다 —
# backtest_framework.py의 forward-return 방법론(baseline vs pass_avg_return, win_rate)을
# 라이브 환경에 그대로 적용한 것. 컷오프 판정(합격/불합격)은 여기서 하지 않는다 — 원값만
# 쌓고, 실제 baseline 비교·컷오프 결정은 별도 분석 단계(Phase D)의 몫이다.
#
# [저장소] PHASE2와 별도의 Notion 데이터소스를 쓴다(스키마가 다름 — 진입/목표가가 아니라
# 13개 지표 원값). 같은 Notion 워크스페이스면 통합 토큰(NOTION_TOKEN)을 그대로 재사용하고,
# 데이터소스 ID만 별도 환경변수로 받는다 — 이 값이 없으면 FC_OBS_TRACKING_ENABLED=False로
# 전 기능이 조용히 꺼진다(PHASE2와 동일한 안전 폴백 패턴).
#
# [중복기록 방지] 같은 코인을 하루에 여러 번 스캔해도(FindCoin은 텔레그램 명령 기반이라
# 스캔 빈도가 불규칙) 심볼+거래소+날짜 단위로 하루 1행만 남긴다 — title 필드를
# "{symbol}_{exchange}_{date}"로 고정해 조회 1회로 존재여부를 판별한다.
#
# [forward window] 20일 — backtest_framework.py의 FORWARD_WINDOW와 동일값 재사용(신규
# 임의값 발명 아님). PHASE2의 OUTCOME_MAX_HOLD_DAYS(같은 20이지만 "예측 만료" 의미)와는
# 목적이 달라 상수를 공유하지 않고 별도 이름(FC_OBS_FORWARD_WINDOW_DAYS)으로 분리한다 —
# 두 20이 우연히 같은 값이라는 이유로 하나의 상수로 묶으면, 이후 둘 중 하나만 조정해야 할
# 때(예: PHASE2는 유지하되 FindCoin 관측 window만 늘리는 경우) SSOT 결합이 오히려 발목을
# 잡는다 — 값이 같다고 개념이 같은 건 아니다(구문·의미 이중검증 상 별도 상수가 맞음).
# ============================================================
NOTION_FC_OBS_DATA_SOURCE_ID = os.environ.get("NOTION_FC_OBS_DATA_SOURCE_ID", "")
FC_OBS_TRACKING_ENABLED = bool(NOTION_TOKEN and NOTION_FC_OBS_DATA_SOURCE_ID)
FC_OBS_FORWARD_WINDOW_DAYS = 20  # backtest_framework.py FORWARD_WINDOW 재사용값, 신규 아님
FC_OBS_CHECK_INTERVAL_SECONDS = 60 * 60 * 6  # 일봉 기준 채점이라 PHASE2(1시간)보다 느슨하게
FC_OBS_RICH_TEXT_CHUNK = 1900  # Notion rich_text 블록당 상한(2000) 아래 여유폭


def _fc_obs_notion_headers():
    """PHASE2의 _notion_headers()와 동일 토큰 재사용(같은 워크스페이스, 다른 데이터소스).
    NOTION_API_VERSION도 위 PHASE2 섹션에서 이미 정의된 값을 그대로 재사용한다."""
    return {
        "Authorization": f"Bearer {NOTION_TOKEN}",
        "Notion-Version": NOTION_API_VERSION,
        "Content-Type": "application/json",
...
def _fc_obs_chunk_rich_text(text):
    """Notion rich_text 배열은 블록당 2000자 상한 — 관측 JSON이 길어질 경우를 대비해
    청크로 분할한다(완전 무삭제 원칙 — 원문을 자르지 않고 여러 블록에 나눠 담음)."""
    chunks = [text[i:i + FC_OBS_RICH_TEXT_CHUNK] for i in range(0, len(text), FC_OBS_RICH_TEXT_CHUNK)] or [""]
    return [{"text": {"content": c}} for c in chunks]


def fc_obs_row_key(symbol, exchange, date_str):
    """중복기록 방지용 고유 title. 예: 'BTC/KRW_upbit_2026-09-04'."""
    return f"{symbol}_{exchange}_{date_str}"


def fc_obs_notion_row_exists(row_key):
    """title(row_key) 완전일치 조회로 오늘자 기록 존재 여부만 확인한다(복합필터 대신 title
    equals 단일조건 — 스키마 단순화로 조회 실패 표면적을 줄임). 조회 자체가 실패하면
    '없다고 보고 새로 기록 시도'가 아니라 안전하게 None(판단불가)을 반환해 중복기록보다
    누락을 택한다(관측 데이터는 하루 늦게 못 쌓이는 것이 중복보다 덜 해로움)."""
    if not FC_OBS_TRACKING_ENABLED:
        return None
    url = f"https://api.notion.com/v1/data_sources/{NOTION_FC_OBS_DATA_SOURCE_ID}/query"
    body = {"filter": {"property": "행키", "title": {"equals": row_key}}, "page_size": 1}
    try:
        res = requests.post(url, headers=_fc_obs_notion_headers(), json=body, timeout=15)
        if res.status_code >= 300:
            print(f"[FC_OBS] 중복조회 실패 status={res.status_code} body={res.text[:200]}")
            return None
        return len(res.json().get("results", [])) > 0
    except Exception as e:
        print(f"[FC_OBS] 중복조회 예외: {e}")
        return None


def fc_obs_notion_create_row(symbol, exchange, snapshot_date_str, snapshot_price, observation_dict):
    """Tier1 관측 1건을 '관측중' 상태로 기록한다. observation_dict는
    fc_observation_tier1.compute_tier1_observation()의 반환값을 JSON으로 직렬화해 그대로 담는다
    (필드별 컬럼화 대신 단일 rich_text 블롭 — 13개 지표×여러 필드를 전부 컬럼화하면 스키마가
    과도하게 비대해지고, Phase D 분석은 어차피 프로그램적으로 JSON을 파싱하므로 컬럼 분리의
    실익이 없다). 실패해도 예외를 올리지 않는다(PHASE2 notion_create_prediction_row와 동일
    방어 패턴 — 기록 실패가 FindCoin 본 기능에 영향을 주지 않아야 함)."""
    if not FC_OBS_TRACKING_ENABLED:
        return None
    row_key = fc_obs_row_key(symbol, exchange, snapshot_date_str)
    try:
        payload_json = json.dumps(observation_dict, ensure_ascii=False, default=str)
    except Exception as e:
        print(f"[FC_OBS] 관측값 직렬화 실패({symbol}): {e}")
        return None
    props = {
        "행키": {"title": [{"text": {"content": row_key}}]},
        "심볼": {"rich_text": [{"text": {"content": str(symbol)}}]},
        "거래소": {"select": {"name": str(exchange)}},
        "관측일": {"date": {"start": snapshot_date_str}},
        "관측시각": {"date": {"start": datetime.now(timezone.utc).isoformat()}},
        "관측가": {"number": _safe_float(snapshot_price)},
        "상태": {"select": {"name": "관측중"}},
        "tier1_raw": {"rich_text": _fc_obs_chunk_rich_text(payload_json)},
    }
    body = {"parent": {"data_source_id": NOTION_FC_OBS_DATA_SOURCE_ID}, "properties": props}
    try:
        res = requests.post("https://api.notion.com/v1/pages", headers=_fc_obs_notion_headers(), json=body, timeout=15)
        if res.status_code >= 300:
            print(f"[FC_OBS] 기록 실패 status={res.status_code} body={res.text[:300]}")
            return None
        return res.json().get("id")
    except Exception as e:
        print(f"[FC_OBS] 기록 예외: {e}")
        return None


def record_fc_tier1_observation(symbol, exchange, observation_dict, snapshot_price):
    """백그라운드 스레드 진입점(PHASE2의 record_phase2_tracking_prediction과 동일 패턴).
    중복조회 결과가 True(이미 존재)일 때만 기록을 건너뛴다 — 조회 실패(None)는 '중복인지
    모르니 기록 강행'을 택한다(과다기록이 결손보다 Phase D 분석에서 걸러내기 쉬움 — 중복은
    날짜 기준 groupby로 사후 제거 가능하지만, 결손은 복구 불가)."""
    try:
        today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        row_key = fc_obs_row_key(symbol, exchange, today_str)
        if fc_obs_notion_row_exists(row_key) is True:
            return
        fc_obs_notion_create_row(symbol, exchange, today_str, snapshot_price, observation_dict)
    except Exception as e:
        print(f"[FC_OBS] 기록 스레드 예외: {e}")


def _fc_obs_record_batch(records):
    """run_findcoin_scan이 한 스캔에서 모은 (symbol, exchange, observation, snapshot_price)
    묶음을 스레드 1개에서 순차 기록한다. [설계근거] 종목별로 스레드를 띄우면 유효종목이
    수백 개일 때 스캔 1회당 스레드 수백 개가 생겨 프로세스에 부담을 준다 — 배치 전체를
    스레드 하나로 묶어 순차 처리(동시성 없음, 대신 안전)."""
    for symbol, exchange, obs, snapshot_price in records:
        try:
            record_fc_tier1_observation(symbol, exchange, obs, snapshot_price)
        except Exception as e:
            print(f"[FC_OBS] 배치 기록 예외({symbol}): {e}")


def fc_obs_notion_query_scoreable():
    """상태=관측중 이면서 관측일이 FC_OBS_FORWARD_WINDOW_DAYS일 이전인 행만 페이지네이션으로
    모은다(PHASE2 notion_query_pending_predictions와 동일 페이지네이션 구조 재사용)."""
    if not FC_OBS_TRACKING_ENABLED:
        return []
    cutoff_date = (datetime.now(timezone.utc) - timedelta(days=FC_OBS_FORWARD_WINDOW_DAYS)).strftime("%Y-%m-%d")
    url = f"https://api.notion.com/v1/data_sources/{NOTION_FC_OBS_DATA_SOURCE_ID}/query"
    body = {
        "filter": {
            "and": [
                {"property": "상태", "select": {"equals": "관측중"}},
                {"property": "관측일", "date": {"on_or_before": cutoff_date}},
            ]
        },
        "page_size": 100,
    }
    ...
    while True:
        ...
        try:
            ...
            print(f"[FC_OBS] 채점대상 조회 예외: {e}")
            break
        if res.status_code >= 300:
            print(f"[FC_OBS] 채점대상 조회 실패 status={res.status_code} body={res.text[:300]}")
            break
        data = res.json()
        results.extend(data.get("results", []))
        if not data.get("has_more"):
...
def fc_obs_notion_update_outcome(page_id, forward_return_pct, realized_price, resolved_at_iso):
    if not FC_OBS_TRACKING_ENABLED:
        return False
    props = {
        "상태": {"select": {"name": "채점완료"}},
        "forward_return_pct": {"number": _safe_float(forward_return_pct)},
        "실현가": {"number": _safe_float(realized_price)},
        "채점시각": {"date": {"start": resolved_at_iso}},
    }
    try:
        res = requests.patch(
            f"https://api.notion.com/v1/pages/{page_id}",
            headers=_fc_obs_notion_headers(), json={"properties": props}, timeout=15,
        )
        if res.status_code >= 300:
            print(f"[FC_OBS] 채점갱신 실패 status={res.status_code} body={res.text[:300]}")
            return False
        return True
    except Exception as e:
        print(f"[FC_OBS] 채점갱신 예외: {e}")
        return False


def fc_obs_compute_forward_return(exchange_label, symbol, snapshot_date_str, snapshot_price, forward_days):
    """관측일 종가(snapshot_price) 대비 forward_days일 뒤 종가의 수익률을 계산한다.
    [페이지네이션] determine_first_touch_outcome과 동일하게 200개/회, 최대 5회, 무진행 가드
    포함 — main.py에 이미 문서화된 동일 결함(limit 과대요청 시 최근구간 누락)을 재발시키지
    않기 위해 그 수정 패턴을 그대로 재사용한다(신규 로직 발명 아님)."""
    ex_id = _resolve_ccxt_exchange_id(exchange_label)
    try:
        ex = getattr(ccxt, ex_id)({"enableRateLimit": True, "timeout": 8000})
    except Exception as e:
        return {"resolved": False, "error": str(e)}
    try:
        snapshot_dt = datetime.strptime(snapshot_date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    except ValueError:
        return {"resolved": False, "error": "bad_snapshot_date"}
    since_ms = int(snapshot_dt.timestamp() * 1000)
    bars, since, per_call_limit, max_calls = [], since_ms, 200, 5
    for _ in range(max_calls):
        try:
            batch = ex.fetch_ohlcv(symbol, timeframe="1d", since=since, limit=per_call_limit)
        except Exception as e:
            if not bars:
                return {"resolved": False, "error": str(e)}
            break
    ...
    if not bars:
        return {"resolved": False, "error": "no_bars"}
    # 관측일 자체 봉(index 0 근방)은 스냅샷 가격의 기준일이므로 건너뛰고, forward_days번째
    # "그 이후" 완성봉을 목표로 삼는다. 요청 이후 시장이 휴장 없이 매일 봉을 만드는
    # 암호화폐 특성상 인덱스=날짜수로 근사해도 안전(주식시장의 휴장일 보정 불필요).
    if len(bars) <= forward_days:
        return {"resolved": False, "last_price": bars[-1][4], "bars_available": len(bars)}
    target_bar = bars[forward_days]
    forward_price = target_bar[4]
    if not snapshot_price or snapshot_price <= 0:
        return {"resolved": False, "error": "bad_snapshot_price"}
    forward_return_pct = (forward_price / snapshot_price - 1.0) * 100.0
    return {"resolved": True, "forward_return_pct": forward_return_pct, "price": forward_price}


def _check_fc_obs_scoreable_once():
    now = datetime.now(timezone.utc)
    for page in fc_obs_notion_query_scoreable():
        props = page.get("properties", {})
        page_id = page.get("id")
        symbol = ((props.get("심볼") or {}).get("rich_text") or [{}])[0].get("plain_text")
        exchange_label = _notion_select(props, "거래소")
        snapshot_date_str = _notion_date_start(props, "관측일")
        snapshot_price = _notion_num(props, "관측가")
        if not all([symbol, exchange_label, snapshot_date_str, snapshot_price]):
            continue
        outcome = fc_obs_compute_forward_return(exchange_label, symbol, snapshot_date_str, snapshot_price, FC_OBS_FORWARD_WINDOW_DAYS)
        if outcome.get("resolved"):
            fc_obs_notion_update_outcome(page_id, outcome["forward_return_pct"], outcome["price"], now.isoformat())
        # resolved=False(봉 부족)면 이번 주기는 건너뛰고 다음 주기(6시간 뒤)에 재시도 —
        # PHASE2의 '만료(미결)' 같은 강제종료 상태를 여기서는 두지 않는다: forward window가
        # 지나면 거래소에 해당 봉이 이미 존재하는 게 정상이라 재시도로 대부분 해결되고,
        # 영구 결손이면 Phase D 분석에서 관측중 상태로 남은 행 자체가 이상신호로 드러난다.


def _run_fc_obs_check_loop():
    while True:
        time.sleep(FC_OBS_CHECK_INTERVAL_SECONDS)
        if not FC_OBS_TRACKING_ENABLED:
            continue
        try:
            _check_fc_obs_scoreable_once()
        except Exception as e:
            print(f"[FC_OBS] 점검 스레드 오류: {e}")


if FC_OBS_TRACKING_ENABLED:
    threading.Thread(target=_run_fc_obs_check_loop, daemon=True).start()
    print("[INFO] FindCoin Tier1 관측 사후채점(Notion) 활성화")
else:
    print("[INFO] FindCoin Tier1 관측 사후채점 비활성화 — NOTION_FC_OBS_DATA_SOURCE_ID 미설정")


# ============================================================
# FindCoin 이벤트수집 — 사후사건연구용 수동 데이터 적재 (2026-09-04 신설)
# [배경] 사용자가 직접 "이 코인이 이 날짜에 시세분출했다"를 식별해 텔레그램 명령으로
# 넘기면, 그 기준일 이전 여러 시점(D-0/1/2/3/5/7/10/15/20)의 Tier1 관측치를 계산해
# Notion에 쌓는다. 분석(여러 사건을 겹쳐보고 공통점 찾기)은 사용자가 Notion에서 직접
# 수행한다 — 이 기능은 수집만 담당한다.
# [무간섭] FC-0~FC-5 코어, Phase B(FC_OBS) 스키마 어느 쪽도 건드리지 않는다. 별도
# Notion 데이터소스(NOTION_FC_EVENT_DATA_SOURCE_ID)를 새로 쓴다 — 목적(사후관측 대기중
# vs 이미 확정된 과거사건)이 다른 데이터를 한 스키마에 억지로 합치지 않기 위함.
# [재사용] fc_compute_range_expansion/compression/box_range는 main.py 기존 함수 그대로,
# fc_observation_tier1.compute_tier1_observation은 Phase A 모듈 그대로, 페이지네이션은
# fc_obs_compute_forward_return과 동일 패턴, 헤더는 PHASE2의 _notion_headers() 그대로,
# rich_text 청크는 Phase B의 _fc_obs_chunk_rich_text 그대로 — 신규 로직은 날짜→인덱스
# 매칭과 오프셋 순회뿐이다.
# [명령 제한] /backtest와 동일하게 소유자 chat_id 전용 — 일반 사용자 명령 아님.
# ============================================================
NOTION_FC_EVENT_DATA_SOURCE_ID = os.environ.get("NOTION_FC_EVENT_DATA_SOURCE_ID", "")
FC_EVENT_TRACKING_ENABLED = bool(NOTION_TOKEN and NOTION_FC_EVENT_DATA_SOURCE_ID)
FC_EVENT_COLLECT_OFFSETS = [0, 1, 2, 3, 5, 7, 10, 15, 20]  # 리포트 시점 목록 — 판정기준 아님, 표시용
FC_EVENT_BASELINE_SAMPLES = 3  # 이벤트 1건당 자동 수집할 무작위 기준값 개수
FC_EVENT_BASELINE_EXCLUDE_DAYS = 25  # 기준일 앞뒤 이 범위는 기준값 후보에서 제외(이벤트 영향권 회피, 최대오프셋 20+여유5)
FC_EVENT_BASELINE_FETCH_MARGIN = 120  # 제외구간 밖에도 기준값 후보가 실제로 남도록 추가 확보하는 조회기간(일)
FC_EVENT_COLLECT_LOOKBACK_BUFFER = FC_MIN_BARS_STANDARD + 20  # range_pctile_self_history 요구사항 재사용


def _fc_event_find_date_index(daily_bars, target_date_str):
    """daily_bars(오름차순)에서 target_date_str(YYYY-MM-DD, UTC 자정 기준)에 해당하거나
    그 직전인 가장 가까운 봉의 인덱스를 찾는다. 암호화폐는 휴장이 없어 보통 정확히 일치하지만,
    거래소 데이터결손 등 예외 상황을 대비해 '이전 가장 가까운 봉'으로 안전하게 폴백한다."""
    target_ts = int(datetime.strptime(target_date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc).timestamp() * 1000)
    best_idx = None
...
def _fc_event_structural_features(daily_bars, as_of_idx):
    """구조적 변곡점(RSI 다이버전스)·보조지표(RSI, RSI모멘텀) 산출.
    [2026-09-04 신설 — 사용자 지적 반영] 신규 지표 발명이 아니라 main.py 지정코인 분석에서
    이미 검증되어 쓰이는 calculate_rma_rsi/detect_regular_divergence/
    calculate_rsi_momentum_slope를 그대로 재사용한다(SSOT — RSI 정의가 두 곳에 따로
    존재하지 않음). look-ahead 방지 위해 as_of_idx까지만 슬라이스해서 전달."""
    sliced = daily_bars[: as_of_idx + 1]
    if len(sliced) < 20:
        return {"status": "insufficient_data"}
    df = pd.DataFrame(sliced, columns=["timestamp", "open", "high", "low", "close", "volume"])
    df["rsi"] = calculate_rma_rsi(df["close"])
    divergence_tag = detect_regular_divergence(df)
    rsi_delta, rsi_state = calculate_rsi_momentum_slope(df)
    current_rsi = df["rsi"].iloc[-1]
    return {
        "status": "ok",
        "rsi": None if pd.isna(current_rsi) else round(float(current_rsi), 2),
        "divergence": divergence_tag,
        "rsi_momentum_delta": rsi_delta,
        "rsi_momentum_state": rsi_state,
    }


def _fc_event_trendline_break(daily_bars, as_of_idx):
    """장기 하락추세선 이탈 여부(#18, 재테스트 시퀀스까지는 다루지 않음 — 과적합 위험으로
    Phase A에서 보류했던 범위를 '이탈 여부'로만 좁혀 재도입).
    [재사용] main.py 기존 스윙탐지(_find_pivots/_filter_major_price_pivots, RSI다이버전스에
    이미 쓰이는 함수) 그대로 사용 — 새 스윙탐지 로직 발명 아님."""
    sliced = daily_bars[: as_of_idx + 1]
    completed = sliced[:-1] if len(sliced) >= 2 else []
    if len(completed) < 30:
        return {"status": "insufficient_data"}
    highs = np.array([b[2] for b in completed])
    lows = np.array([b[3] for b in completed])
    _, lows_raw = _find_pivots(highs, lows, left=2, right=2)
    _, lows_major = _filter_major_price_pivots([], lows_raw, highs, lows)
    if len(lows_major) < 2:
        return {"status": "insufficient_data"}
    i1, i2 = lows_major[-2], lows_major[-1]
    if i2 <= i1:
        return {"status": "insufficient_data"}
    slope = (lows[i2] - lows[i1]) / (i2 - i1)
    is_downtrend = bool(slope < 0)
    # [2026-09-04 수정 — 1일 밀림 결함] 기존엔 completed[-1](as_of_idx 전날)을 '오늘 종가'로
    # 오인 비교했음(사용자 지적으로 발견). sliced[-1]이 실제 as_of_idx 당일 — 그것으로 교체.
    # 투영 x좌표도 completed 인덱싱 기준으로 sliced[-1]의 위치인 len(completed)로 보정
    # (기존 len(completed)-1은 completed[-1]=전날의 위치였음).
    today_idx = len(completed)
    projected = float(lows[i2] + slope * (today_idx - i2))
    today_close = float(sliced[-1][4])
    broke_above = bool(is_downtrend and today_close > projected)
    return {
        "status": "ok",
        "is_downtrend_line": is_downtrend,
        "trendline_projected": round(projected, 8),
        "today_close": today_close,
        "broke_above": broke_above,
        "normalized": 1.0 if broke_above else 0.0,
    }


def _fc_event_snapshot_full(daily_bars, as_of_idx, lookback, btc_daily_bars=None):
    """as_of_idx 시점까지만 잘라(look-ahead 방지) 기존 main.py 함수(range_expansion/
    compression/box_range)와 Phase A 모듈(compute_tier1_observation)을 그대로 호출한다.
    fc_event_study.snapshot_at과 동일한 look-ahead 방지 원칙(마지막 봉=오늘 진행봉 취급).
    [2026-09-04 확장] 구조적변곡(RSI다이버전스)·추세선이탈도 함께 산출해 병합한다.
    [2026-09-05 제거] 체결데이터(trade_tape)·매집판정(quiet_accumulation)은 업비트 공식
    API 확인 결과 체결이력이 최근 7일치만 제공됨이 확인되어 제거했다 — 기준값 제외구간
    (±FC_EVENT_BASELINE_EXCLUDE_DAYS=25일)과 구조적으로 양립 불가능해(기준값은 항상 사건일
    에서 25일 이상 떨어져야 하는데 체결데이터는 오늘로부터 7일 이내만 가능 — 두 조건을
    동시에 만족하는 날짜가 존재할 수 없음) 비교 자체가 성립하지 않았다(사용자 승인 하 제거).

    [2026-09-04 수정 — 1일 밀림 결함, 사용자 지적으로 발견]
    fc_compute_range_expansion/compression/box_range와 Phase A 지표 다수(17개)는 "마지막
    봉=오늘 진행봉(미종료)"으로 취급해 내부적으로 제외한다(실시간 스캔에서는 의도된 설계
    — 시간대 편향 방지). 그런데 이 함수는 과거 특정일(as_of_idx)을 "그날"로 지정해 재현하는
    용도라, 그 규칙을 그대로 적용하면 실제로는 as_of_idx 전날을 계산하게 된다 — 나머지
    지표(spring_reclaim·이동평균지지반등 등)는 마지막 봉을 직접 참조해 정확히 as_of_idx를
    보는 것과 하루 어긋남.
    해결(당시): as_of_idx 봉을 복제해 맨 뒤에 한 번 더 붙인다. "완성봉 제외" 규칙 함수는
    복제본만 제외해 진짜 as_of_idx가 남고, "마지막봉 직접참조" 규칙 함수는 복제본이
    as_of_idx와 값이 동일해 결과가 같다 — 양쪽 다 정확히 as_of_idx를 보게 통일된다는
    설계였음.

    [판정59번 — 위 해결책 자체의 결함, Cowork 재검토] 위 "해결"은 잘못된 전제에 기반함:
    "마지막봉 직접참조" 함수들은 daily_bars[-1]"만" 직접 참조하는 게 아니라, 그와 별개로
    daily_bars[:-1](=_completed_bars)을 "그 이전 이력 창"으로도 함께 쓴다(예:
    spring_reclaim의 SL=이전 lookback봉 최저가, ma_bounce의 ma_support=이전 20일 평균).
    복제 패딩을 적용하면 daily_bars[:-1]이 원본 sliced 그대로 남아(패딩분 1개만 잘려
    나가고) as_of_idx 당일 봉 자신이 "이전 이력 창" 안에 들어가 버린다. 실측(격리
    재현스크립트): 오늘 실제로 고점(112)을 돌파한 합성봉을 넣었을 때 —
      · 패딩본(기존): compression_pct=13.39%, box_status="computed" (돌파를 놓침 —
        오늘 자신의 고가 112가 창에 섞여 판단기준(period_high)이 112까지 부풀려짐)
      · 비패딩(정답): compression_pct=999.0, box_status="already_broken" (정상 감지)
    같은 원인으로 spring_reclaim.strict_reclaim은 SL 계산에 오늘 저가가 섞여 수학적으로
    항상 False(사용자가 handoff 문서에서 이미 지적한 결함, 여기서 근본원인 재확인).
    영향 범위 재조사 결과 spring_reclaim 외에도 3곳이 동일 결함: fc_compute_compression
    (already_broken 오탐지 실패), fc_compute_box_range(동일), tier1_moving_average_
    structure의 ma_bounce_strict/loose 서브필드(golden_cross/ma_spread_pctile은 반대로
    "완성봉 제외" 패턴이라 패딩이 필요 — 같은 함수 안에 두 패턴이 섞여 있음).
    해결: 패딩본(padded)과 비패딩본(sliced) 두 벌을 모두 계산해, 함수별 실제 참조 패턴에
    맞는 쪽 결과만 채택한다. moving_average_structure는 필드 단위로 쪼개 병합 후
    normalized를 원래 가중식 그대로 재계산(둘 중 하나를 통째로 버리지 않음 — 완전무삭제
    원칙과 별개로, 두 계산 다 유효한 부분이 있어 병합이 유일하게 정보손실 없는 방법)."""
    sliced = daily_bars[: as_of_idx + 1]
    padded = sliced + [sliced[-1]]

    # BTC 쪽도 동일 원리로 정렬 — 코인·BTC 둘 다 run_event_collection에서 같은 since_ms로
    # 조회돼 인덱스가 날짜 단위로 대략 정렬된다는 전제(암호화폐는 휴장 없어 안전) 위에서,
    # as_of_idx까지만 자르고 동일하게 패딩해 날짜가 다시 어긋나지 않게 한다.
    # relative_strength_btc는 "완성봉 제외" 패턴(패딩 필요)만 쓰므로 비패딩본 BTC는 불필요.
    btc_padded = None
    if btc_daily_bars:
        btc_sliced = btc_daily_bars[: as_of_idx + 1]
        if btc_sliced:
            btc_padded = btc_sliced + [btc_sliced[-1]]

    # [판정59번] "완성봉 제외 창" 패턴(range_expansion 등 다수) → padded로 계산(기존과 동일).
    temp_item = {"daily_ohlcv_cache": padded}
    fc_compute_range_expansion(temp_item)
    obs = fc_observation_tier1.compute_tier1_observation(
        daily_bars=padded, lookback=lookback, btc_daily_bars=btc_padded,
    )

    # [판정59번] "당일봉 직접참조 + 별도 이전이력창" 패턴(compression/box_range/spring_
    # reclaim/ma_bounce) → sliced(비패딩)로 다시 계산해 해당 결과만 교체한다.
    temp_item_unpadded = {"daily_ohlcv_cache": sliced}
    fc_compute_compression(temp_item_unpadded)
    fc_compute_box_range(temp_item_unpadded)
    obs_unpadded = fc_observation_tier1.compute_tier1_observation(
        daily_bars=sliced, lookback=lookback, btc_daily_bars=None,
    )
    obs["spring_reclaim"] = obs_unpadded["spring_reclaim"]
    # [판정61번] ma20_cross_bullish도 spring_reclaim과 동일한 "당일봉 직접참조 + 별도
    # 이전이력창" 패턴이라 sliced(비패딩) 결과로 전량 교체(필드분할 불필요 — 전 필드가
    # 동일 패턴에서 나옴).
    obs["ma20_cross_bullish"] = obs_unpadded["ma20_cross_bullish"]

    mas_padded = obs.get("moving_average_structure", {})
    mas_unpadded = obs_unpadded.get("moving_average_structure", {})
    if mas_padded.get("status") == "ok" and mas_unpadded.get("status") == "ok":
        golden_cross = mas_padded["golden_cross"]
        days_since_cross = mas_padded["days_since_cross"]
        ma_spread_pctile = mas_padded["ma_spread_pctile"]
        ma_bounce_strict = mas_unpadded["ma_bounce_strict"]
        ma_bounce_loose = mas_unpadded["ma_bounce_loose"]
        obs["moving_average_structure"] = {
            "status": "ok",
            "golden_cross": golden_cross,
            "days_since_cross": days_since_cross,
            "ma_spread_pctile": ma_spread_pctile,
            "ma_bounce_strict": ma_bounce_strict,
            "ma_bounce_loose": ma_bounce_loose,
            # tier1_moving_average_structure 원식 그대로 재계산(가중치 0.4/0.3/0.3) —
            # 신규 가중치 발명 아님, fc_observation_tier1.py의 normalized 공식 재사용.
            "normalized": round(
                (1.0 if golden_cross else 0.0) * 0.4
                + (1 - ma_spread_pctile) * 0.3
                + (1.0 if ma_bounce_strict else (0.5 if ma_bounce_loose else 0.0)) * 0.3,
                4,
            ),
        }
    # 둘 중 하나라도 insufficient_data면 병합 불가 — 기존(패딩본) 결과를 그대로 둔다
    # (일부 필드가 부정확할 수 있음을 감수하는 안전 폴백, 크래시 방지 우선).

    obs["reused_vol_squeeze_ratio"] = temp_item.get("vol_squeeze_ratio")
    obs["reused_s_vol_squeeze"] = temp_item.get("s_vol_squeeze")
    obs["reused_compression_pct"] = temp_item_unpadded.get("compression_pct")
    obs["reused_s_boxrange"] = temp_item_unpadded.get("s_boxrange")
    obs["reused_box_status"] = temp_item_unpadded.get("box_status")

    obs["structural"] = _fc_event_structural_features(daily_bars, as_of_idx)
    obs["trendline_break"] = _fc_event_trendline_break(daily_bars, as_of_idx)

    return obs


def _fc_notion_diagnose(chat_id):
    """[2026-09-04 신설 — 실사용 결함 대응, 1차 수정] 사용자가 페이지 URL에서 직접 뽑은
    ID로 NOTION_FC_EVENT_DATA_SOURCE_ID를 설정했으나 404(object_not_found)가 발생함을
    실제 Render 로그로 확인. 원인: Notion 신형 멀티 데이터소스 구조에서 URL의 페이지/블록
    ID와 API가 요구하는 data_source_id가 다를 수 있음.
    [2차 수정 — 실사용 400 오류로 확인] 최초 구현은 search filter.value를 'database'로
    보냈으나, 이 API버전(NOTION_API_VERSION 참조)은 'page' 또는 'data_source'만 허용한다는
    검증 오류를 실제로 반환받아 확인함 — 문서 추측이 아닌 실제 오류 메시지 기반으로 정정.
    이 버전은 database가 아닌 data_source를 최상위 검색 대상으로 직접 반환한다."""
    if not NOTION_TOKEN:
        send_telegram_message(chat_id, "[오류] NOTION_TOKEN이 설정되지 않았습니다.")
        return
    try:
        res = requests.post(
        ...
        send_telegram_message(chat_id, f"[오류] Notion 검색 실패: {e}")
        return
    if res.status_code >= 300:
        send_telegram_message(chat_id, f"[오류] Notion 검색 실패 status={res.status_code} body={res.text[:500]}")
        return
    ...
    if not results:
        send_telegram_message(
            chat_id,
            "연결된 데이터소스가 하나도 없습니다.\n"
            "노션 표 페이지에서 공유(Share) → candleview-bot 연결이 실제로 됐는지 다시 확인해 주세요."
        )
        return
    lines = [f"🔎 candleview-bot이 접근 가능한 데이터소스 {len(results)}개\n"]
    for ds in results:
        # 응답 스키마 방어적 파싱 — 'name'(문자열) 또는 'title'(rich_text 배열) 둘 다 대비
        title = ds.get("name")
        if not title:
        ...
        ds_id = ds.get("id", "?")
        lines.append(f"\n📄 제목: {title}")
        lines.append(f"  ✅ data_source_id: {ds_id}")
    lines.append("\n\n환경변수 NOTION_FC_EVENT_DATA_SOURCE_ID엔 위 ✅ 값을 넣으세요.")
    full_text = "\n".join(lines)
    for chunk in smart_chunk(full_text, ["📄"]):
        send_telegram_message(chat_id, chunk)


def fc_event_notion_create_row(symbol, exchange, ref_date_str, offset_days, snapshot_date_str,
                                snapshot_price, observation_dict):
    """이벤트 1건의 오프셋 1개를 Notion에 기록. 실패해도 예외를 올리지 않는다
    (PHASE2/Phase B와 동일한 방어 패턴)."""
    if not FC_EVENT_TRACKING_ENABLED:
        return None
    row_key = f"{symbol}_{exchange}_{ref_date_str}_D-{offset_days}"
    try:
        payload_json = json.dumps(observation_dict, ensure_ascii=False, default=str)
    except Exception as e:
        print(f"[FC_EVENT] 직렬화 실패({symbol}): {e}")
        return None
    props = {
        "이벤트키": {"title": [{"text": {"content": row_key}}]},
        "심볼": {"rich_text": [{"text": {"content": str(symbol)}}]},
        "거래소": {"select": {"name": str(exchange)}},
        "기준일": {"date": {"start": ref_date_str}},
        "오프셋일": {"number": offset_days},
        "스냅샷일": {"date": {"start": snapshot_date_str}},
        "스냅샷가": {"number": _safe_float(snapshot_price)},
        "tier1_raw": {"rich_text": _fc_obs_chunk_rich_text(payload_json)},
    }
    body = {"parent": {"data_source_id": NOTION_FC_EVENT_DATA_SOURCE_ID}, "properties": props}
    try:
        res = requests.post("https://api.notion.com/v1/pages", headers=_notion_headers(), json=body, timeout=15)
        if res.status_code >= 300:
            print(f"[FC_EVENT] 기록 실패 status={res.status_code} body={res.text[:300]}")
            return None
        return res.json().get("id")
    except Exception as e:
        print(f"[FC_EVENT] 기록 예외: {e}")
        return None


def run_event_collection(chat_id, ex_name, symbol, ref_date_str):
    """텔레그램 /이벤트수집 명령의 실행 본체. 별도 스레드에서 호출된다(텔레그램 응답 대기 없음).
    데이터 자체는 Notion에 쌓고, 텔레그램에는 결과 요약(저장/실패 건수)만 짧게 보낸다 —
    분석은 사용자가 Notion에서 직접 수행하므로 여기서 원문을 텔레그램에 풀어놓지 않는다."""
    if not FC_EVENT_TRACKING_ENABLED:
        send_telegram_message(chat_id, "[안내] 이벤트수집 저장소가 아직 설정되지 않았습니다(NOTION_FC_EVENT_DATA_SOURCE_ID 필요).")
        return
    ...
    try:
        ...
        send_telegram_message(chat_id, f"[오류] 거래소 초기화 실패: {e}")
        return

    full_symbol = symbol if "/" in symbol else f"{symbol}/{quote}"
    if full_symbol not in exchange_class.markets:
        send_telegram_message(chat_id, f"[오류] {full_symbol}을 {ex_name}에서 찾을 수 없습니다.")
        return

    try:
        ref_dt = datetime.strptime(ref_date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    except ValueError:
        send_telegram_message(chat_id, "[오류] 날짜 형식은 YYYY-MM-DD 입니다. 예: 2026-08-15")
        return

    max_offset = max(FC_EVENT_COLLECT_OFFSETS)
    # [2026-09-04 버그수정] 기존엔 offset 수집에 필요한 만큼만 조회해(max_offset+버퍼+5)
    # 기준값 무작위 샘플링의 제외구간(±FC_EVENT_BASELINE_EXCLUDE_DAYS)이 버퍼 범위를 통째로
    # 삼켜버려 후보가 0개가 되는 결함이 있었음(실사용 "저장 9건, 기준값 누락"으로 확인).
    # 기준값 후보가 실제로 남도록 여유분(FC_EVENT_BASELINE_FETCH_MARGIN)을 추가로 확보한다.
    fetch_span_days = (max_offset + FC_EVENT_COLLECT_LOOKBACK_BUFFER
                        + FC_EVENT_BASELINE_EXCLUDE_DAYS + FC_EVENT_BASELINE_FETCH_MARGIN + 5)
    since_ms = int((ref_dt - timedelta(days=fetch_span_days)).timestamp() * 1000)

    # 페이지네이션 — fc_obs_compute_forward_return과 동일 패턴(200개/회, 최대5회, 무진행가드)
    bars, since, per_call_limit, max_calls = [], since_ms, 200, 5
    for _ in range(max_calls):
        try:
            batch = exchange_class.fetch_ohlcv(full_symbol, timeframe="1d", since=since, limit=per_call_limit)
        except Exception as e:
            send_telegram_message(chat_id, f"[오류] 캔들 조회 실패: {e}")
            return
        if not batch:
            break
        bars.extend(batch)
    ...
    if not bars:
        send_telegram_message(chat_id, "[오류] 캔들 데이터를 가져오지 못했습니다.")
        return

    ref_idx = _fc_event_find_date_index(bars, ref_date_str)
    if ref_idx is None or ref_idx < FC_EVENT_COLLECT_LOOKBACK_BUFFER:
        send_telegram_message(
            chat_id,
            f"[오류] 기준일({ref_date_str}) 이전 데이터가 부족합니다"
            f"(확보 {ref_idx if ref_idx is not None else 0}봉, 필요 {FC_EVENT_COLLECT_LOOKBACK_BUFFER}봉 이상)."
        )
        return

    # [2026-09-04 버그수정] fetch_span_days가 200일을 넘을 수 있게 됐는데, BTC 조회는
    # limit=200 단일호출이라 넓어진 범위의 최근 구간(ref_date 근접분)이 잘려나가는 결함이
    # 있었음 — 코인 조회와 동일한 페이지네이션으로 교체.
    btc_daily_bars = []
    try:
        btc_since, btc_calls = since_ms, 0
        while btc_calls < max_calls:
            btc_batch = exchange_class.fetch_ohlcv(f"BTC/{quote}", timeframe="1d", since=btc_since, limit=per_call_limit)
            btc_calls += 1
            if not btc_batch:
                break
            btc_daily_bars.extend(btc_batch)
            btc_last_ts = btc_batch[-1][0]
            if btc_last_ts <= btc_since:
                break
            btc_since = btc_last_ts + 1
            if len(btc_batch) < per_call_limit:
                break
    except Exception as e:
        print(f"[WARN] 이벤트수집 BTC참조 실패: {e}")
        btc_daily_bars = None

    saved, failed = 0, 0
    for offset in FC_EVENT_COLLECT_OFFSETS:
        snap_idx = ref_idx - offset
        if snap_idx < 0:
            continue
        obs = _fc_event_snapshot_full(bars, snap_idx, FC_MIN_BARS_STANDARD, btc_daily_bars)
        snap_date = datetime.fromtimestamp(bars[snap_idx][0] / 1000, tz=timezone.utc).strftime("%Y-%m-%d")
        snapshot_price = bars[snap_idx][4]
        page_id = fc_event_notion_create_row(full_symbol, ex_name, ref_date_str, offset, snap_date, snapshot_price, obs)
        if page_id:
            saved += 1
        else:
            failed += 1

    # [2026-09-04 신설 — 사용자 지적 반영] "평범한 날"을 사람이 고르면 무의식적 편향이 생긴다
    # (정말 평범해 보이는 날만 고르게 됨). 같은 코인의 과거 이력에서 이벤트 window(±
    # FC_EVENT_BASELINE_EXCLUDE_DAYS)를 제외한 나머지 구간에서 시스템이 무작위로 골라
    # 기준값을 자동 수집한다. 사람 선택 없음 — 난수만 개입.
    # [오프셋일=-1] 신규 Notion 컬럼을 만들지 않기 위해 기존 '오프셋일' 필드를 재사용한다.
    # 실제 오프셋(0~20)은 항상 0 이상이므로 -1은 "무작위 기준값"이라는 의미로 안전하게
    # 구분 가능한 sentinel — 값 재정의가 아니라 도메인 밖 값을 표식으로 쓰는 것뿐이다.
    baseline_saved = 0
    exclude_lo = max(0, ref_idx - FC_EVENT_BASELINE_EXCLUDE_DAYS)
    exclude_hi = min(len(bars), ref_idx + FC_EVENT_BASELINE_EXCLUDE_DAYS + 1)
    candidate_pool = [i for i in range(FC_EVENT_COLLECT_LOOKBACK_BUFFER, len(bars) - 1)
                       if not (exclude_lo <= i < exclude_hi)]
    if candidate_pool:
        n_pick = min(FC_EVENT_BASELINE_SAMPLES, len(candidate_pool))
        picked = random.sample(candidate_pool, n_pick)
        for idx in picked:
            obs_b = _fc_event_snapshot_full(bars, idx, FC_MIN_BARS_STANDARD, btc_daily_bars)
            snap_date_b = datetime.fromtimestamp(bars[idx][0] / 1000, tz=timezone.utc).strftime("%Y-%m-%d")
            snapshot_price_b = bars[idx][4]
            page_id_b = fc_event_notion_create_row(full_symbol, ex_name, snap_date_b, -1, snap_date_b, snapshot_price_b, obs_b)
            if page_id_b:
                baseline_saved += 1

    status_note = f"저장 {saved}건" + (f", 실패 {failed}건" if failed else "")
    baseline_note = f"\n기준값(무작위) {baseline_saved}건 추가 저장" if baseline_saved else ""
    send_telegram_message(
        chat_id,
        f"📍 이벤트수집 완료 — {ex_name.upper()} {full_symbol} 기준일 {ref_date_str}\n{status_note}{baseline_note}\nNotion에서 확인해 주세요."
    )


# ============================================================
# [판정61번 신설] 신규지표(ma20_cross_bullish) 즉시검증 — /지표검증
# [설계배경] 새 지표를 검증하려고 앞으로의 신규 /이벤트수집을 기다릴 필요는 없다.
# 이미 Notion에 쌓인 "어떤 코인이 언제 시세분출했는지" 사건목록(거래소·심볼·스냅샷일·
# 오프셋일)은 그대로 재사용하고, 그 목록이 가리키는 날짜들의 원본 일봉만 거래소에서
# 다시 받아와 신규지표를 그 시점 기준으로 재계산한다 — fc_event_study.py의 사후사건연구와
# 동일 방법론(이미 벌어진 과거 데이터 재사용, 대기시간 없음). Notion에 저장된 tier1_raw
# JSON 자체는 이 신규지표를 담고 있지 않으므로(당시엔 존재하지 않던 지표) 원문을 읽지
# 않고 거래소 원본 데이터만 새로 조회한다.
# ============================================================

def _fc_event_notion_query_all(page_size=100, max_pages=50):
    """NOTION_FC_EVENT_DATA_SOURCE_ID의 전체 행을 페이지네이션으로 수집해 (거래소·심볼·
    스냅샷일·오프셋일)만 추출한다. tier1_raw(관측값 JSON)는 이 신규지표 검증엔 불필요해
    읽지 않는다 — 필요한 건 '어떤 코인, 어느 날짜가 사건/기준값이었는지' 목록뿐."""
    if not FC_EVENT_TRACKING_ENABLED:
        return []
    url = f"https://api.notion.com/v1/data_sources/{NOTION_FC_EVENT_DATA_SOURCE_ID}/query"
    rows, cursor = [], None
    for _ in range(max_pages):
        ...
        if cursor:
            body["start_cursor"] = cursor
        try:
            res = requests.post(url, headers=_notion_headers(), json=body, timeout=20)
        except Exception as e:
            print(f"[지표검증] Notion 조회 예외: {e}")
            break
        if res.status_code >= 300:
            print(f"[지표검증] Notion 조회 실패 status={res.status_code} body={res.text[:200]}")
            break
        data = res.json()
        for page in data.get("results", []):
            props = page.get("properties", {})
            try:
                symbol = props["심볼"]["rich_text"][0]["text"]["content"]
                exchange = props["거래소"]["select"]["name"]
                snap_date = props["스냅샷일"]["date"]["start"][:10]
                offset = props["오프셋일"]["number"]
            except (KeyError, IndexError, TypeError):
                continue
            rows.append({"symbol": symbol, "exchange": exchange, "snapshot_date": snap_date, "offset": offset})
        if not data.get("has_more"):
            break
        cursor = data.get("next_cursor")
    return rows
...
def run_ma20_cross_validation(chat_id):
    """/지표검증 명령 실행 본체. 위 설계배경 참조 — 새 데이터 수집을 기다리지 않고
    이미 확보된 사건목록을 재사용해 원본 일봉만 다시 받아 검증한다."""
    if not FC_EVENT_TRACKING_ENABLED:
        send_telegram_message(chat_id, "[안내] 이벤트수집 저장소가 설정되지 않아 검증할 데이터가 없습니다(NOTION_FC_EVENT_DATA_SOURCE_ID 필요).")
        return

    send_telegram_message(chat_id, "🔍 20일선 돌파 양봉 지표 검증을 시작합니다 — Notion 사건목록 조회 중...")
    rows = _fc_event_notion_query_all()
    if not rows:
        send_telegram_message(chat_id, "[안내] Notion에 저장된 이벤트수집 데이터가 없습니다. /이벤트수집으로 먼저 데이터를 쌓아 주세요.")
        return

    by_symbol = {}
    for r in rows:
        key = (r["exchange"], r["symbol"])
        bucket = by_symbol.setdefault(key, {"event": [], "baseline": []})
        label = "event" if (r["offset"] is not None and r["offset"] >= 0) else "baseline"
        bucket[label].append(r["snapshot_date"])

    event_hit, event_total = 0, 0
    baseline_hit, baseline_total = 0, 0
    symbols_checked, symbols_failed = 0, 0

    for (ex_name, symbol), dates in by_symbol.items():
        all_dates = dates["event"] + dates["baseline"]
        if not all_dates:
            continue
        try:
            exchange_class = getattr(ccxt, ex_name)({"enableRateLimit": True, "timeout": 8000})
            ...
            print(f"[지표검증] 거래소 초기화 실패({ex_name}): {e}")
            symbols_failed += 1
            continue

        earliest_dt = datetime.strptime(min(all_dates), "%Y-%m-%d").replace(tzinfo=timezone.utc)
        since_ms = int((earliest_dt - timedelta(days=FC_MIN_BARS_STANDARD + 30)).timestamp() * 1000)

        bars, since, per_call_limit, max_calls = [], since_ms, 200, 5
        try:
            for _ in range(max_calls):
                batch = exchange_class.fetch_ohlcv(symbol, timeframe="1d", since=since, limit=per_call_limit)
                if not batch:
                    break
                bars.extend(batch)
                last_ts = batch[-1][0]
            ...
            print(f"[지표검증] 일봉 조회 실패({symbol}@{ex_name}): {e}")
            symbols_failed += 1
            continue
        ...
        for date_str in dates["event"]:
            idx = _fc_event_find_date_index(bars, date_str)
            if idx is None or idx < FC_MIN_BARS_STANDARD:
                continue
            r = fc_observation_tier1.tier1_ma20_cross_bullish(bars[: idx + 1], FC_MIN_BARS_STANDARD)
            if r.get("status") != "ok":
                continue
            event_total += 1
            if r.get("signal"):
        ...
        for date_str in dates["baseline"]:
            idx = _fc_event_find_date_index(bars, date_str)
            if idx is None or idx < FC_MIN_BARS_STANDARD:
                continue
            r = fc_observation_tier1.tier1_ma20_cross_bullish(bars[: idx + 1], FC_MIN_BARS_STANDARD)
    ...
    if event_total == 0 or baseline_total == 0:
        send_telegram_message(
            chat_id,
            f"[결과 부족] 검증 가능한 표본이 부족합니다(사건 {event_total}건 / 기준값 {baseline_total}건, "
            f"조회성공 {symbols_checked}종목 / 실패 {symbols_failed}종목)."
        )
        return

    event_rate = event_hit / event_total * 100.0
    baseline_rate = baseline_hit / baseline_total * 100.0
    send_telegram_message(
        chat_id,
        "📊 20일선 돌파 양봉 지표 검증 결과\n\n"
        f"사건(펌프 전 0~20일) 표본: {event_total}건 중 {event_hit}건 발생 ({event_rate:.1f}%)\n"
        f"기준값(무작위 평범한 날) 표본: {baseline_total}건 중 {baseline_hit}건 발생 ({baseline_rate:.1f}%)\n"
        f"차이: {event_rate - baseline_rate:+.1f}%p\n\n"
        f"조회 성공 {symbols_checked}종목 / 실패 {symbols_failed}종목\n\n"
        "※ 참고용 통계입니다. 표본 수와 차이 크기를 함께 보고 S_scout 편입 여부를 판단해 주세요."
    )
        ...
        for update in updates.get("result", []):
            ...
            if callback:
                ...
                if action == "phase1_view":
                    ...
                    model_line = format_phase2_model_provenance(
                        phase2_meta or (cached.get("supplement") or {}).get("phase1_model"),
                        (cached.get("supplement") or {}).get("phase1_model"),
                    )
                    ...
                    for chunk in smart_chunk(full, PHASE2_BOUNDARY_MARKERS):
                        send_telegram_message(chat_id, chunk)
                    # [PHASE2 실전 결과 추적용 — 2026-09 신설] 텔레그램 발행은 이미 끝났으므로,
                    # Notion 기록은 별도 스레드로 비동기 처리해 API 지연/실패가 사용자 응답을
                    # 절대 막지 않게 한다(backtest_framework와 동일하게 daemon=True로 격리).
                    tracking_payload = (phase2_meta or {}).get("tracking_payload")
                    if tracking_payload and TRACKING_ENABLED:
                        threading.Thread(
                            target=record_phase2_tracking_prediction,
                            args=(tracking_payload,), daemon=True,
                        ).start()
            ...
            if cmd == "notion진단" and chat_id == 517008099:
                threading.Thread(target=_fc_notion_diagnose, args=(chat_id,), daemon=True).start()
                continue

            if cmd == "이벤트수집" and chat_id == 517008099:
                if len(parts) < 4:
                    send_telegram_message(
                        chat_id,
                        "사용법: /이벤트수집 <거래소> <코인명또는티커> <기준일YYYY-MM-DD>\n"
                        "예: /이벤트수집 업비트 리플 2026-08-15"
                    )
                    continue
                ev_ex_name = resolve_exchange(parts[1])
                if ev_ex_name is None:
                    send_telegram_message(chat_id, UNAUTHORIZED_INPUT_GUIDE)
                    continue
                ev_symbol = resolve_korean_symbol(parts[2], ev_ex_name)
                ev_ref_date = parts[3]
                send_telegram_message(chat_id, f"📍 이벤트 데이터 수집 중... ({ev_ex_name} {ev_symbol} {ev_ref_date})")
                threading.Thread(
                    target=run_event_collection,
                    args=(chat_id, ev_ex_name, ev_symbol, ev_ref_date), daemon=True
                ).start()
                continue

            if cmd == "지표검증" and chat_id == 517008099:
                threading.Thread(target=run_ma20_cross_validation, args=(chat_id,), daemon=True).start()
                continue
            ...
            if len(parts) == 1:
                ...
                if n_gate2 == 0:
                    ...
                    send_telegram_message(
                        ...
                        f"RTM ≥ 3.0 AND Liquidity_Ratio ≥ 1.0) 및 경로B(변동폭 ≤ {FC_COMPRESSION_RANGE_MAX:.1f}% AND "
                        f"Liquidity_Ratio ≥ 1.0) 중 어느 쪽도 통과하지 못했거나, PA-VSA 옥석 검증·"
                        f"S_scout(≥{FC_S_SCOUT_PASS}) 조건을 동시에 충족하는 고신뢰 분출 후보가 0개입니다. "
